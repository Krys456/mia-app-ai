/**
 * #337B — Google Gmail Settings toggle model (pure).
 *
 * Visual ON/OFF reflects server connection status only (not a local preference).
 * Toggle is UX — server EMAIL_ENABLED / Edge auth remain the security gates.
 * Mirrors src/lib/calendarToggleModel.ts.
 */

export type EmailToggleVisual = 'on' | 'off'

export type EmailServiceState =
  | 'unknown'
  | 'available'
  | 'disabled'
  | 'auth_unavailable'
...
export type EmailUiPhase = 'idle' | 'loading' | 'connecting' | 'busy'

export type EmailToggleModel = {
  visual: EmailToggleVisual
  statusCode:
    | 'connected'
    | 'disconnected'
    | 'connecting'
