...
export const PRIVACY_DISCLOSURE = {
  ...
  googleCalendar:
    'Puoi collegare opzionalmente Google Calendar da Impostazioni → Integrazioni. ShinkAIdo chiede solo il permesso di lettura (sola lettura): non può creare, modificare o eliminare eventi. I token di accesso Google sono crittografati lato server e non vengono salvati nel browser. Quando fai domande sui tuoi impegni, ShinkAIdo può leggere un intervallo limitato del calendario e inviare a OpenAI solo dati sanificati e delimitati per quel turno (titoli e orari; non descrizioni, partecipanti o link Meet di default). Non viene creato un archivio a lungo termine degli eventi. Puoi scollegare in qualsiasi momento; Memoria, promemoria e notifiche restano indipendenti. Con una sessione anonima su questo dispositivo, cancellare i dati del sito può richiedere di collegare di nuovo Google.',
  anonymousSession:
