...
/**
 * #337B — Settings → Integrazioni → Gmail (read-only), rendered below
 * CalendarIntegrationsSettings under the same "Integrazioni" heading.
 * Does NOT auto-start OAuth — ON only starts the flow on explicit click.
 */
export function EmailIntegrationsSettings() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (result.code === 'email_disabled') {
      ...
      setService('disabled')
      setNote(
        'Il collegamento Gmail non è attivato su questo ambiente. Memoria e Google Calendar non sono interessati.',
      )
      return
    }
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (returned === 'connected') {
        setNote(
          'Gmail collegato in sola lettura. Puoi chiedere in chat: «Ho email non lette?»',
        )
  ...
  const setToggle = async (next: EmailToggleVisual) => {
    ...
    try {
      ...
      if (result.ok) {
        setConnection(result.connection)
        setService('available')
        setNote('Gmail scollegato. Memoria e Google Calendar non sono stati modificati.')
  ...
  return (
    <div className="settings-integration-block">
      ...
      <p className="settings-note settings-note--tight">
        ShinkAIdo legge Gmail solo dopo il consenso, in sola lettura: non invia, non risponde e
        non elimina messaggi. ON avvia Google OAuth; OFF scollega e revoca i token lato server.
        Scollegare Gmail non modifica Memoria né Google Calendar.
      </p>
