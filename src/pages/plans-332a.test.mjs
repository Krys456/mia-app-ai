...
assert.match(plansTsx, /createCheckoutSession|IdentityAccountPanel|showIdentityGate/)
assert.match(plansTsx, /role="status"/)
assert.doesNotMatch(plansTsx, /STRIPE_SECRET|sk_live|sk_test|price_[A-Za-z0-9]+/)
...
// Free is current; durable upgrade starts Checkout (never local grant)
...
assert.match(plansTsx, /createCheckoutSession/)
assert.match(plansTsx, /Attivazione del piano in corso/)
assert.doesNotMatch(plansTsx, /setVerifiedPlanId\('pro'\)|setVerifiedPlanId\("pro"\)|setVerifiedPlanId\('base'\)/)
