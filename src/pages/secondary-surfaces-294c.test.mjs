...
// —— Vision state machine + immersive layout ——
...
assert.match(visionTsx, /laife-vision--immersive/)
assert.match(visionTsx, /laife-vision__dock/)
...
assert.match(visionCss, /overflow-y:\s*hidden/)
...
// —— Exclusive Vision AppView (chat must not stack underneath) ——
assert.match(appCss, /\.app-view--chat\[hidden\]/)
assert.match(appCss, /display:\s*none\s*!important/)
assert.match(appTsx, /app-view--vision/)
