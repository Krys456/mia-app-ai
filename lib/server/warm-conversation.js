...
const TRANSACTIONAL_FORBIDDEN =
  /\b(how\s+can\s+i\s+help|come\s+posso\s+(aiutarti|esserti\s+utile)|what\s+can\s+i\s+do\s+for\s+you|in\s+cosa\s+posso\s+aiutarti|how\s+may\s+i\s+assist|what\s+would\s+you\s+like\s+to\s+(discuss|talk\s+about|know)|di\s+cosa\s+(vuoi|preferisci)\s+parlare|cosa\s+vuoi\s+sapere|what\s+is\s+your\s+priority(\s+today)?|qual\s+[eè]\s+la\s+(tua\s+)?priorit[aà](\s+di\s+oggi)?|tell\s+me\.?|dimmi\s+pure|any\s+questions|hai\s+domande|let\s+me\s+know|fammi\s+sapere|i'?m\s+here\s+if\s+you\s+need|sono\s+qui\s+se\s+ti\s+serve)\b/i
...
function buildBrief(trigger, ctx = {}) {
  ...
  const lines = [
    'Warm Conversation: sei un partner di conversazione, non una macchina Q&A.',
    'Goditi la conversazione. Guida in modo naturale. Porta idee.',
    'Tratta la chiacchiera come valore, non come rumore prima del “task”.',
    'Fai sentire la persona benvenuta; non restare in attesa passiva di istruzioni.',
    'Aperture a basso valore — evitarle salvo necessità assoluta: “Dimmi pure.”, “Come posso aiutarti?”, “Qual è la tua priorità?”, “Cosa vuoi sapere?”, “Hai domande?”, “Fammi sapere.”, “Sono qui se ti serve.”',
  ...
  if (trigger === 'greeting') {
    lines.push(
      'Trigger: saluto. Calore + responsabilità: apri con un’osservazione/idea/curiosità (non un’intervista). L’utente deve sentire che hai già idee da condividere.',
    )
  } else if (trigger === 'casual_start') {
    lines.push(
      'Trigger: inizio casuale / incertezza. Accogli, poi parti tu con una direzione ad alto valore — non “dimmi tu di cosa parlare”.',
    )
  } else if (trigger === 'casual_continue') {
    lines.push(
      `Trigger: conversazione su «${topic}». Costruisci sul contesto; porta un’idea, un collegamento o un fatto sorprendente — senza form da sportello.`,
    )
  } else if (trigger === 'warm_handoff') {
    lines.push(
      'Trigger: saluto + richiesta. Un cenno caldo, poi sostanza fluida — niente checklist da receptionist.',
    )
  }

  if (ctx.softStyleOnly || ctx.welcomeActive) {
    lines.push(
      'Welcome/altro advisor può possedere l’apertura: rafforza partner-energy e anti-intervista, senza competere.',
    )
  }
...
function hintsFor(trigger) {
  ...
  const hints = [
    ...
    'Evita aperture a basso valore: “Dimmi pure.”, “Come posso aiutarti?”, “Qual è la tua priorità?”, “Cosa vuoi sapere?”, “Hai domande?”, “Fammi sapere.”, “Sono qui se ti serve.”',
    'Preferisci: osservazioni, idee, curiosità, storie, esperimenti mentali, insight, fatti sorprendenti, collegamenti.',
  ]
  if (trigger === 'greeting' || trigger === 'casual_start') {
...
export function softenTransactionalOpening(text, plan) {
  if (!plan?.active) return text
  const raw = String(text || '')
  if (!raw.trim()) return raw
  if (!TRANSACTIONAL_FORBIDDEN.test(raw.slice(0, 180))) return raw

  // Strip a leading transactional / interview opener; leave the rest.
  const softened = raw
    .replace(
      /^(how\s+can\s+i\s+help(\s+you)?[?.!]+\s*|come\s+posso\s+(aiutarti|esserti\s+utile)[?.!]+\s*|what\s+can\s+i\s+do\s+for\s+you[?.!]+\s*|in\s+cosa\s+posso\s+aiutarti[?.!]+\s*|what\s+would\s+you\s+like\s+to\s+(discuss|talk\s+about|know)[?.!]+\s*|di\s+cosa\s+(vuoi|preferisci)\s+parlare[?.!]+\s*|cosa\s+vuoi\s+sapere[?.!]+\s*|what\s+is\s+your\s+priority(\s+today)?[?.!]+\s*|qual\s+[eè]\s+la\s+(tua\s+)?priorit[aà](\s+di\s+oggi)?[?.!]+\s*|tell\s+me[.!]+\s*|dimmi\s+pure[.!]+\s*|any\s+questions[?.!]+\s*|hai\s+domande[?.!]+\s*|let\s+me\s+know[.!]+\s*|fammi\s+sapere[.!]+\s*|i'?m\s+here\s+if\s+you\s+need(\s+me)?[.!]+\s*|sono\s+qui\s+se\s+ti\s+serve[.!]+\s*)/i,
      '',
    )
    .trim()
  return softened || raw
