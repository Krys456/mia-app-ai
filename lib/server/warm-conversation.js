...
const TRANSACTIONAL_FORBIDDEN =
  /(how\s+can\s+i\s+help|come\s+posso\s+(aiutarti|esserti\s+utile)|what\s+can\s+i\s+do\s+for\s+you|in\s+cosa\s+posso\s+aiutarti|how\s+may\s+i\s+assist|what\s+would\s+you\s+like\s+to\s+(discuss|talk\s+about|know)|di\s+cosa\s+(vuoi|preferisci)\s+parlare|cosa\s+vuoi\s+sapere|what\s+is\s+your\s+priority(\s+today)?|qual\s+\S+\s+la\s+(tua\s+)?priorit\S*|tell\s+me\.?|dimmi\s+pure|any\s+questions|hai\s+domande|let\s+me\s+know|fammi\s+sapere|i'?m\s+here\s+if\s+you\s+need|sono\s+qui\s+se\s+ti\s+serve)/i
