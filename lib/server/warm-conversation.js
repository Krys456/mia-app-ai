...
const TRANSACTIONAL_FORBIDDEN =
  /\b(how\s+can\s+i\s+help|come\s+posso\s+(aiutarti|esserti\s+utile)|what\s+can\s+i\s+do\s+for\s+you|in\s+cosa\s+posso\s+aiutarti|how\s+may\s+i\s+assist)\b/i
...
function buildBrief(trigger, ctx = {}) {
  ...
  const lines = [
    'Warm Conversation: goditi davvero la conversazione — non sei uno sportello ticket.',
    ...
    'Transizioni naturali; tono di qualcuno a cui piace pensare insieme.',
    'Vietato: “How can I help?”, “Come posso aiutarti?”, aperture da helpdesk, tono transazionale.',
  ]

  if (trigger === 'greeting') {
    lines.push(
      'Trigger: saluto. Rispondi caldo e presente; a volte basta stare nella conversazione senza spingere un task.',
    )
  } else if (trigger === 'casual_start') {
    lines.push(
      'Trigger: inizio casuale. Accogli con calore; rispondi al contenuto umano; non dirottare subito sul “come posso aiutarti”.',
    )
  } else if (trigger === 'casual_continue') {
    lines.push(
      `Trigger: conversazione casuale in corso su «${topic}». Resta umano, curioso quanto basta, senza diventare un form.`,
    )
  } else if (trigger === 'warm_handoff') {
    lines.push(
      'Trigger: saluto + richiesta. Prima un cenno caldo (una frase), poi passa in modo naturale alla sostanza — niente checklist da receptionist.',
    )
  }

  if (ctx.softStyleOnly || ctx.welcomeActive) {
    lines.push(
      'Welcome/altro advisor può possedere l’apertura: rafforza solo il calore e l’anti-transazionale, senza competere.',
    )
  }
...
function structureFor(trigger) {
  if (trigger === 'warm_handoff') {
    return 'Warm Conversation: un cenno di benvenuto → poi la richiesta, senza “How can I help?”'
  }
  if (trigger === 'greeting' || trigger === 'casual_start') {
    return 'Warm Conversation: presenza calda prima di ogni problema; conversazione come valore'
  }
  return 'Warm Conversation: resta nel tono di chi pensa volentieri insieme'
}
...
function hintsFor(trigger) {
  /** @type {string[]} */
  const hints = [
    'Calore genuino, non teatralità.',
    'Niente “How can I help you?” / “Come posso aiutarti?”.',
    'Piacere di pensare insieme — non solo eseguire ticket.',
  ]
  if (trigger === 'greeting' || trigger === 'casual_start') {
    hints.push('Può bastare una risposta umana; non forzare un next step da produttività.')
  }
  if (trigger === 'warm_handoff') {
    hints.push('Una frase di benvenuto, poi sostanza fluida.')
  }
  if (trigger === 'casual_continue') {
    hints.push('Transizioni naturali; ascolta il filo umano.')
  }
  return hints
}
...
Regole: calore genuino · conversazione come valore · benvenuto prima dei problemi · niente helpdesk · non citare il motore.`.trim()
}

/**
 * Guard: Writer text must not open with transactional helpdesk lines.
...
export function softenTransactionalOpening(text, plan) {
  ...
  if (!TRANSACTIONAL_FORBIDDEN.test(raw.slice(0, 160))) return raw

  // Strip a leading transactional question; leave the rest.
  const softened = raw
    .replace(
      /^(how\s+can\s+i\s+help(\s+you)?[?.!]+\s*|come\s+posso\s+(aiutarti|esserti\s+utile)[?.!]+\s*|what\s+can\s+i\s+do\s+for\s+you[?.!]+\s*|in\s+cosa\s+posso\s+aiutarti[?.!]+\s*)/i,
      '',
    )
    .trim()
  return softened || raw
