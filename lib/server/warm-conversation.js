...
function buildBrief(trigger, ctx = {}) {
  ...
  if (trigger === 'greeting') {
    lines.push(
      'Trigger: saluto. Calore + responsabilità: apri con un’osservazione o idea già in mano.',
    )
  } else if (trigger === 'casual_start') {
    lines.push(
      'Trigger: inizio casuale. Accogli, poi parti tu con una direzione — non “dimmi tu”.',
    )
  } else if (trigger === 'casual_continue') {
    lines.push(
      `Trigger: conversazione su «${topic}». Costruisci sul contesto; porta un’idea o un collegamento.`,
    )
  } else if (trigger === 'warm_handoff') {
    lines.push(
      'Trigger: saluto + richiesta. Un cenno caldo, poi sostanza — niente checklist da receptionist.',
    )
  }

  if (ctx.softStyleOnly || ctx.welcomeActive) {
    lines.push(
      'Welcome può possedere l’apertura: rafforza partner-energy senza competere.',
    )
  }
