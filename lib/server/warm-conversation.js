...
const TRANSACTIONAL_FORBIDDEN =
  /\b(how\s+can\s+i\s+help|come\s+posso\s+(aiutarti|esserti\s+utile)|what\s+can\s+i\s+do\s+for\s+you|in\s+cosa\s+posso\s+aiutarti|how\s+may\s+i\s+assist|what\s+would\s+you\s+like\s+to\s+(discuss|talk\s+about)|di\s+cosa\s+(vuoi|preferisci)\s+parlare|what\s+is\s+your\s+priority(\s+today)?|qual\s+[eè]\s+la\s+priorit[aà](\s+di\s+oggi)?|tell\s+me\.?$|dimmi\s+pure)\b/i
...
function buildBrief(trigger, ctx = {}) {
  ...
  const lines = [
    ...
    'Vietato di default: “How can I help?”, “Tell me.”, “What would you like to discuss?”, “What is your priority today?”, “Dimmi pure.”, “Di cosa vuoi parlare?” — salvo se sono davvero la migliore continuazione.',
  ]

  if (trigger === 'greeting') {
    lines.push(
      'Trigger: saluto. Calore + responsabilità: avvia o proponi UN filo interessante (non un’intervista). L’utente deve sentire che hai già idee da condividere.',
    )
  } else if (trigger === 'casual_start') {
    lines.push(
      'Trigger: inizio casuale / incertezza. Accogli, poi parti tu con una direzione — non “dimmi tu di cosa parlare”.',
    )
  } else if (trigger === 'casual_continue') {
    lines.push(
      `Trigger: conversazione su «${topic}». Costruisci sul contesto; porta un’idea o un angolo — senza form da sportello.`,
    )
...
function structureFor(trigger) {
  if (trigger === 'warm_handoff') {
    return 'Warm Conversation: benvenuto breve → sostanza, senza helpdesk/interview'
  }
  if (trigger === 'greeting' || trigger === 'casual_start') {
    return 'Warm Conversation: calore + UNA idea/direzione — non “How can I help?” / non “Dimmi pure.”'
  }
  return 'Warm Conversation: partner che pensa insieme e costruisce sul filo'
}
...
function hintsFor(trigger) {
  ...
  const hints = [
    ...
    'Niente “How can I help?”, “Tell me.”, “What would you like to discuss?”, “What is your priority today?”.',
    'Piacere di pensare insieme; hai già idee da condividere.',
  ]
  if (trigger === 'greeting' || trigger === 'casual_start') {
    hints.push('Prendi responsabilità: inizia una conversazione interessante, non un’intervista.')
    hints.push('Una direzione proposta e sviluppata batte un menu di scelte.')
  }
  if (trigger === 'warm_handoff') {
...
export function softenTransactionalOpening(text, plan) {
  ...
  const softened = raw
    .replace(
      /^(how\s+can\s+i\s+help(\s+you)?[?.!]+\s*|come\s+posso\s+(aiutarti|esserti\s+utile)[?.!]+\s*|what\s+can\s+i\s+do\s+for\s+you[?.!]+\s*|in\s+cosa\s+posso\s+aiutarti[?.!]+\s*|what\s+would\s+you\s+like\s+to\s+(discuss|talk\s+about)[?.!]+\s*|di\s+cosa\s+(vuoi|preferisci)\s+parlare[?.!]+\s*|what\s+is\s+your\s+priority(\s+today)?[?.!]+\s*|qual\s+[eè]\s+la\s+priorit[aà](\s+di\s+oggi)?[?.!]+\s*|tell\s+me[.!]+\s*|dimmi\s+pure[.!]+\s*)/i,
      '',
    )
    .trim()
  return softened || raw
