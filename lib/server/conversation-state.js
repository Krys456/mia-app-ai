...
export const CONVERSATION_STATE_BUILD = '328-1'
...
export const CONVERSATION_STATE_APPENDIX_MAX_CHARS = 1750
...
export function computeConversationState(input = {}) {
  ...
  const shortFollowUp =
    isShortFollowUp(userMessage) || continueCue || ordinalFollowUp || ellipsisFollowUp
  ...
  let responsePurpose = inferResponsePurpose(userMessage, conversationMode, {
    isCorrection,
    shortFollowUp,
    continueCue: continueCue || ordinalFollowUp || dimensionContinuation,
    stopDecline,
    completionCue,
  })
  ...
  // Soft completion: close cleanly — no new workflow / keep-alive (#330).
...
export function buildConversationStateAppendix(state) {
  ...
  if (state.stopSignalDetected) {
    lines.push(
      'stop_signal: true — ≤1 brief ack then STOP. Forbidden: alternatives, menus, "what next?", Se vuoi…, Fammi sapere…, Se hai bisogno…, sono qui se…, Vuoi parlare di altro?, service offers, questions.',
    )
  }
  if (state.completionCueDetected && !state.stopSignalDetected) {
    lines.push(
      'completion_signal: true — brief close then STOP. No new agenda, Fammi sapere…, or keep-alive questions.',
    )
  }
  ...
  lines.push(
    '',
    'RESPONSE GUIDANCE:',
    '- Follow these fields for current-turn presentation unless conflicting with safety, factual correctness, required clarification, or an explicit user instruction.',
    ...
    '- question_needed=false → no reciprocal/engagement/service-offer questions (no "E tu?", "Cosa ne pensi?", "Vuoi che…?", "Would you like…?"). Exceptions: missing required info, blocking ambiguity, safety-critical clarify.',
    '- question_needed=true → at most ONE short earned question (e.g. one social reciprocal on a simple greeting). Never stack.',
    '- acknowledgement=false → start with substance (no default Certo/Capisco/Perfetto/Assolutamente). If true → ≤1 brief ack then substance.',
    '- confidence=medium/high + purpose=recommend → choose clearly, brief reason, stop — do not reopen the choice.',
    '- emoji level is permission, not a mandate. State controls HOW to answer, not factual/tool truth.',
    '- STYLE_AVOID never outranks emotion, explicit user instruction, desiredDepth, or structurePreference.',
    '- Momentum: low initiative / stop / completion → react and stop. purpose=continue or active brainstorm/debug/teach/decision → advance exactly one useful layer; no roadmap dump. High initiative → contribute now, not a service offer.',
  )
...
export function looksLikeEllipsisFollowUp(message) {
  ...
  if (
    /\b(?:spiega(?:melo|lo|mi)?\s+(?:meglio|semplice|bene)|pi[uù]\s+semplice|pi[uù]\s+dettagliat\w*|ora\s+(?:pi[uù]\s+)?dettagliat\w*|approfondisci|explain\s+(?:it\s+)?(?:better|more\s+simply)|more\s+detail)\b/i.test(
      t,
    )
  ) {
    return true
...
export function looksLikeCompletionCue(message) {
  ...
  // Soft completion / "enough for now" — close cleanly (#330). Celebration phrases separate.
  return /^(fatto|risolto|funziona|ok\s+cos[iì]|basta\s+cos[iì]|va\s+bene\s+cos[iì]|cos[iì]\s+va\s+bene|ci\s+siamo|done|all\s+good|that'?s\s+it|that'?s\s+enough\s+for\s+now)[.!]*$/i.test(
    t,
  )
}
