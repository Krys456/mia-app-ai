...
export const CONVERSATION_STATE_BUILD = '367b-1'
...
/**
 * Completion / understanding / soft-close cues (#330, expanded #367B micro-beats).
...
export function looksLikeCompletionCue(message) {
  ...
  if (
    /^(?:fatto|risolto|funziona|done|ok\s+cos[iì]|basta\s+cos[iì]|va\s+bene\s+cos[iì]|cos[iì]\s+va\s+bene|ci\s+siamo|all\s+good|that'?s\s+it|that'?s\s+enough(?:\s+for\s+now)?)[.!…]*$/i.test(
      t,
    )
  ) {
    return true
  ...
  if (
    ...
    /^(?:ah+|ahh+|ahhh+|oh+)[,.]?\s*(?:(?:ora|adesso)\s+)?(?:ho\s+)?capito[.!…]*$/i.test(t)
  ) {
    return true
  }
