...
export const CONVERSATION_STATE_BUILD = '362c-1'
...
export function buildConversationStateAppendix(state) {
  ...
  if (state.completionCueDetected && !state.stopSignalDetected) {
    lines.push('completion_signal: true — brief close then STOP (NRP; no new agenda).')
  }
...
function inferPriorModeFromHistory(recent) {
  ...
  for (let i = slice.length - 1; i >= 0; i--) {
    ...
    if (looksLikeBinaryChoice(c) || looksLikeDecisionAsk(c)) return 'decision_support'
    if (
    ...
    if (/\b(?:safe\s+to\s+merge|pronto\s+al\s+merge|ci\s+green|tests?\s+pass)/i.test(c)) {
