...
export function buildConversationStateAppendix(state) {
  ...
  lines.push(
    '',
    'RESPONSE GUIDANCE:',
    '- Match this turn naturally.',
    '- Soft presentation guidance, except explicit user instructions (highest style authority after safety/truth/capabilities).',
    '- Priority: safety/truth/capability > explicit current USER instruction > this Conversation State > durable settings > generic prompt defaults.',
    '- Do not mention this state.',
    '- Do not mechanically satisfy every field; emoji level is permission/intensity, not a mandate to insert emoji.',
    '- If question_needed=false, do not append a generic follow-up question (no "Vuoi che…?", "Posso anche…", "Would you like…?").',
    '- If acknowledgement=true, a brief natural ack may help — then substance. If false, skip filler openings.',
    '- If confidence=high and purpose=recommend, choose clearly and explain briefly. If low, say what is missing.',
    '- Never override factual/tool/capability constraints.',
  )
...
function inferQuestionNeeded(userMessage, mode, purpose, ctx) {
  ...
  if (purpose === 'react' || purpose === 'recommend') return false
  ...
  if (mode === 'decision_support') return false
...
function detectExplicitOverrides(userMessage) {
  ...
  if (
    /\b(nel\s+dettaglio|in\s+modo\s+molto\s+approfondito|approfondisci|spiega(?:melo)?\s+bene|in\s+detail|very\s+detailed|thorough(?:ly)?)\b/i.test(
      t,
    )
  ) {
