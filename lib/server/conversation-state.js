 ...
 * #371B — Late tease invitations, topic-return depth cap, tech frustration→debug.
 *
...
export const CONVERSATION_STATE_BUILD = '371c-1'
...
export function computeConversationState(input = {}) {
  ...
  // #371B — resume shared context; do NOT auto-promote to a detailed re-tutorial.
  if (topicReturnCue && !stopDecline && !completionCue && !playfulBanter) {
    const substantivePrior =
      ...
      [
        ...
        ? priorMode
        : null
    const reteachDetail = looksLikeExplicitReteachDetail(userMessage)
    if (reteachDetail || overrides.forceTeaching) {
      conversationMode = 'teaching'
      responsePurpose = 'explain'
      if (!overrides.desiredDepth) desiredDepth = 'detailed'
    } else {
      conversationMode = substantivePrior || priorMode || 'informational'
      responsePurpose = 'continue'
      // Cap depth: teaching prior must not force a full re-explain by default.
      if (!overrides.desiredDepth) {
        if (desiredDepth === 'detailed') desiredDepth = 'medium'
        else if (desiredDepth !== 'short' && desiredDepth !== 'medium') desiredDepth = 'medium'
        // Specific slice ("…: quella cosa del token?") stays short.
        if (
          /:\s*\S+|quella\s+cosa|that\s+thing|il\s+punto\s+(?:su|del|della|di)|the\s+bit\s+about/i.test(
            userMessage,
          )
        ) {
          desiredDepth = 'short'
...
export function buildConversationStateAppendix(state) {
  ...
  if (state.completionCueDetected && !state.stopSignalDetected) {
    lines.push(
      'completion_signal: true — micro close OK (a few words / one short beat); then STOP. Conversational understanding/closure only — does not imply external/system success. No recap, no new explanation, no service offer, no keep-alive question.',
    )
  }
...
export function looksLikeCompletionCue(message) {
  ...
  // #371B — "Ahhh ora sì" / "Ahhh sì" (end-anchored; ", ma perché?" stays open).
  if (
    /^(?:a{1,8}h{1,6}|ah+|ahh+|ahhh+|oh+)[,.]?\s*(?:ora\s+|adesso\s+)?s[iì][.!…]*$/i.test(t)
  ) {
    return true
  }
...
/**
 * #371B — explicit playful tease / callback invitation (job = react, not explain).
 * Does NOT require recent celebration in State lookback.
 * Does NOT prescribe which joke/callback to make.
 * @param {string} message
 * @returns {boolean}
 */
export function looksLikePlayfulTeaseInvitation(message) {
  const raw = String(message || '')
  const t = normalizeText(raw.replace(/\p{Extended_Pictographic}/gu, ' '))
  if (!t || t.length > 120) return false
  if (looksLikeHarmDistressCue(raw) || looksLikeSeriousCue(raw)) return false
  if (looksLikeSubstantiveQuestion(raw)) return false
  // "Indovina perché OAuth…" / "Guess why tokens…" = real info ask, not banter-only.
  if (
    /\b(?:indovina|guess)\b[\s\S]{0,48}\b(?:perch[eé]|why|come\s+(?:funziona|mai)|how\s+(?:does|do|is)|cosa\s+[eè]|what\s+is)\b/i.test(
      t,
    )
  ) {
    return false
  ...
  if (
    /^(?:indovina\b|guess\b|te\s+l['']avevo\s+detto\b|told\s+you(?:\s+so)?\b|sempre\s+l[iì]\s+torniamo\b|ci\s+risiamo\b|here\s+we\s+go\s+again\b)/i.test(
      t,
    )
  ) {
    return true
  ...
  if (
    /\b(?:sempre\s+l[iì]\s+torniamo|ci\s+risiamo|here\s+we\s+go\s+again|te\s+l['']avevo\s+detto|told\s+you(?:\s+so)?|ricordi\s+(?:i\s+)?\d+\s+tentativ)\b/i.test(
      t,
    )
  ) {
    return true
...
/**
 * #371B — frustration + concrete CI/Preview/deploy/runtime markers → debugging job.
 * Bare "Che palle." alone is NOT debugging.
 * @param {string} message
 * @returns {boolean}
 */
export function looksLikeTechnicalFrustration(message) {
  const t = normalizeText(String(message || '').replace(/\p{Extended_Pictographic}/gu, ' '))
  if (!t || t.length > 180) return false
  if (looksLikeDecisionAsk(t)) return false
  if (
    /\b(?:cos[''][eè]\s+|che\s+cos[''][eè]\s+|what\s+is\s+|spiega|spiegami|teach\s+me)\b/i.test(t)
  ) {
    return false
  }
  const frustr =
    /\b(?:che\s+palle|che\s+nervi|di\s+nuovo|ancora|damn|shit|cavolo|odio|frustrat|rotto|broken|keeps\s+failing)\b/i.test(
      t,
    ) || /!{2,}/.test(t)
  // Avoid bare Italian "ci" — require CI + status/verb, or other concrete markers.
  const tech =
    /\b(?:preview|deploy(?:ment)?|builds?|tests?|errori?|errors?|crash(?:es|ed)?|vercel|runtime|stack\s*trace)\b/i.test(
      t,
    ) ||
    /\bci\s+(?:[eè]\s+|is\s+)?(?:verde|green|ross[ao]|red|ok|fail(?:ed|ing|s)?|fallisce|fallit[oaie]?|passing)\b/i.test(
      t,
    ) ||
    /\b(?:fail(?:ed|ing|s)?|fallisce|fallit[oaie]?)\b/i.test(t) ||
    /\b(?:preview|deploy(?:ment)?|build|test|ci)\b[\s\S]{0,24}\b(?:ross[ao]|red)\b/i.test(t) ||
    /\b(?:ross[ao]|red)\b[\s\S]{0,24}\b(?:preview|deploy(?:ment)?|build|test|ci)\b/i.test(t)
  return frustr && tech
}

/**
 * #371B — user explicitly wants a full re-teach on topic return.
 * @param {string} message
 * @returns {boolean}
 */
export function looksLikeExplicitReteachDetail(message) {
  const t = normalizeText(String(message || '').replace(/\p{Extended_Pictographic}/gu, ' '))
  if (!t) return false
  return /\b(?:spieg(?:a|ami|amelo|alo|alo)\w*.{0,24}(?:dettagl|da\s+(?:capo|zero)|bene|in\s+detail)|rispieg\w*|da\s+capo|dall['']inizio|from\s+scratch|in\s+detail(?:\s+again)?|detailed(?:ly)?|ripartiamo\s+dall|explain.{0,20}(?:again|in\s+detail|from\s+(?:scratch|the\s+start)))\b/i.test(
    t,
  )
}
 ...
 * #371B — late tease invitations do not need recent celebration in the 8-lookback.
 * @param {string} message
...
export function looksLikePlayfulBanterBeat(message, ctx = {}) {
  ...
  if (looksLikeContinueCue(raw)) return false
  if (looksLikeTopicReturnCue(raw)) return false
  if (looksLikeSoftDiscourseParticle(raw)) return false
  if (looksLikeTechnicalFrustration(raw)) return false

  // #371B — current message is itself a clear tease invitation.
  if (looksLikePlayfulTeaseInvitation(raw)) return true
...
function inferConversationMode(userMessage, ctx) {
  ...
  // #371B — frustration + concrete Preview/CI/deploy/runtime markers → debugging.
  if (looksLikeTechnicalFrustration(t)) {
    return 'debugging'
  }
