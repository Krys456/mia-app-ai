...
import {
  inspectCalendarChatIntent,
} from './calendar-chat-intent.js'
...
export async function maybeBuildCalendarChatEnrichment(opts) {
  ...
  const inspected = inspectCalendarChatIntent(opts.userMessage)
  const intent = inspected.intent
  /** #310F — safe text-trace fields (max 80-char previews; no history). */
  const textTrace = {
  ...
  try {
    ...
    return {
      ...
      durationMs: Date.now() - started,
      code,
      tokenDecrypt: mapTokenDecrypt(code),
      preGoogleFailureCode: code,
      ...textTrace,
    }
  }
...
/**
 * @param {string} code
 * @returns {'NOT_REACHED'|'PASS'|'FAIL'}
 */
function mapTokenDecrypt(code) {
  if (code === 'encryption_failure') return 'FAIL'
  if (
    code === 'not_connected' ||
    code === 'calendar_disabled' ||
    code === 'owner_required' ||
  ...
  // refresh/google failures imply decrypt of access or refresh was attempted
  if (
