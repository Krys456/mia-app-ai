...
function scoreDirection(direction, ctx) {
  ...
  if (
    ...
    scores.usefulness = Math.min(5, scores.usefulness + 0.45)
    scores.naturalness = Math.min(5, scores.naturalness + 0.35)
    reasons.push('care_first')
  }

  if (EMOTIONAL.test(text) || emo === 'venting' || emo === 'anxious_reassurance' || expects === 'presence') {
    if (
      direction === 'surprising_insight' ||
      direction === 'concise_story' ||
      direction === 'meaningful_comparison' ||
      direction === 'memorable_example' ||
      direction === 'elegant_explanation'
    ) {
      ...
      scores.usefulness = Math.max(1, scores.usefulness - 0.4)
