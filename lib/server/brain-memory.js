...
/** TEMP Preview forensic flag — remove after PR #246 invariant is proven in real DB. */
export function isMemoryTraceEnabled(input = {}) {
  if (input?.memoryTrace === true) return true
  const env = String(process.env.LAIFE_MEMORY_TRACE || '')
    .trim()
    .toLowerCase()
  if (env === '1' || env === 'true' || env === 'yes') return true
  // Preview-only auto-enable (never Production). Vercel sets VERCEL_ENV=preview|production|development.
...
/**
 * Exact PostgREST filter shape used by supabase-js `.contains('tags', [tag])`.
 * Does not alter the query — forensics only.
 *
 ...
 *   encodedQueryParam: string | null
 *   quotedAlternativeFilter: string | null
 *   fullRelativePathExample: string | null
 * }}
 */
export function describeFactKeyContainsFilter(tag, userId = '<user_id>') {
  const value = typeof tag === 'string' && tag.trim() ? tag.trim() : null
  if (!value) {
    return {
      method: "contains('tags', [tag])",
      column: 'tags',
      tagArgument: null,
      rawFilter: null,
      encodedQueryParam: null,
      quotedAlternativeFilter: null,
      fullRelativePathExample: null,
    }
  }
  // Observed from supabase-js: tags=cs.%7B...%7D  i.e. cs.{tag} WITHOUT quotes around the element.
  const rawFilter = `tags=cs.{${value}}`
  const encodedQueryParam = `tags=cs.${encodeURIComponent(`{${value}}`)}`
  const quotedAlternativeFilter = `tags=cs.${encodeURIComponent(`{"${value}"}`)}`
  ...
  return {
    ...
    rawFilter,
    encodedQueryParam,
    quotedAlternativeFilter,
    fullRelativePathExample: `/rest/v1/memories?select=id&user_id=eq.${uid}&${encodedQueryParam}`,
  }
}

/**
 * Sanitize a memory row for forensic traces (no secrets).
 * @param {any} row
 */
export function sanitizeMemoryTraceRow(row) {
  if (!row || typeof row !== 'object') return null
  return {
    id: row.id != null ? String(row.id) : null,
    user_id: row.user_id != null ? String(row.user_id) : row.userId != null ? String(row.userId) : null,
    category: row.category != null ? String(row.category) : null,
    title: row.title != null ? String(row.title) : null,
    content: row.content != null ? String(row.content).slice(0, 240) : null,
    status: row.status != null ? String(row.status) : null,
    tags: Array.isArray(row.tags) ? row.tags.map(String) : row.tags == null ? null : [String(row.tags)],
    source: row.source != null ? String(row.source) : null,
    updated_at: row.updated_at != null ? String(row.updated_at) : row.updatedAt != null ? String(row.updatedAt) : null,
    parsed_fact_key: readFactKeyFromTags(row.tags),
  }
}

/**
 * Ground-truth snapshot: ALL rows for user_id (no tags contains filter).
 * Classifies exact fact_key matches in JS so contains quirks cannot hide peers.
 *
 * @param {any} supabase
 * @param {string} userId
 * @param {string} factKey
 */
export async function snapshotFactKeyRowsForTrace(supabase, userId, factKey) {
  const { data, error } = await supabase
    .from('memories')
    .select('id, user_id, category, title, content, status, tags, source, updated_at')
    .eq('user_id', userId)
  ...
  if (error) {
    return {
      ...
      exactFactKey: [],
      exactFactKeyActive: [],
      encodedTagUsed: encodeFactKeyTag(factKey),
      factKeyUsed: factKey,
    }
  }

  const allForUser = (data || []).map(sanitizeMemoryTraceRow).filter(Boolean)
  const exactFactKey = allForUser.filter((r) => r.parsed_fact_key === factKey)
  const exactFactKeyActive = exactFactKey.filter(
    (r) => String(r.status || 'active').toLowerCase() === 'active',
  )

  return {
    error: null,
    allForUserCount: allForUser.length,
    exactFactKey,
    exactFactKeyActive,
    exactFactKeyActiveCount: exactFactKeyActive.length,
    encodedTagUsed: encodeFactKeyTag(factKey),
    factKeyUsed: factKey,
  }
}

function supabaseProjectRefFromEnv() {
  ...
  const refOf = (raw) => {
    try {
      ...
      return host.replace(/\.supabase\.co$/i, '') || host
    } catch {
      return null
    }
  }
  const serverRef = refOf(serverUrl)
  const viteRef = refOf(viteUrl)
  return {
    serverProjectRef: serverRef,
    viteProjectRef: viteRef,
    sameProject: Boolean(serverRef && viteRef && serverRef === viteRef),
    serverUrlConfigured: Boolean(serverUrl),
    viteUrlConfigured: Boolean(viteUrl),
    usedViteUrlFallback: !serverUrl && Boolean(viteUrl),
  }
}
...
export function extractDurableFacts(userMessage) {
  ...
  // —— Preferenze ——
  // Dislike first so "non mi piace X" cannot also emit a positive like.
  const dislike =
  ...
  // Open favorites: require clear subject + value (not bare "preferito" chatter).
  const favorite =
    user.match(
  ...
  if (favorite?.[1] && favorite?.[2]) {
    const subject = cleanCapture(favorite[1])
    const value = cleanCapture(favorite[2])
    const subjectOk =
      subject.length >= 2 &&
      !/^(un|una|il|la|lo|gli|le|a|an|the|my|mio|mia)\b/i.test(subject)
    if (subjectOk && value.length >= 2) {
      favoriteMatched = true
      preferenceMatched = true
      if (/^(tema|theme)$/i.test(subject) || /\b(scuro|chiaro|dark|light)\b/i.test(value)) {
        push(
          'settings',
          'Theme preference',
        ...
        push(
          'preferences',
          'Favorite',
          `User's favorite ${subject.toLowerCase()}: ${value}.`,
          6,
          {
            factKey: `preferences.favorite.${normalizeFavoriteSubjectKey(subject)}`,
          },
        )
      }
    }
  ...
  // Skip when a structured favorite or like already covered the turn's preference.
  if (!favoriteMatched && !dislikeMatched) {
    const interest =
  ...
  // —— Progetti (before habits so "lavoro su X" is not also a habit) ——
  const project =
    user.match(
  ...
  if (project?.[1]) {
    ...
    if (value.length >= 2 && !/^(questo|quello|this|that|it)\b/i.test(value)) {
      projectMatched = true
      const singularPrimary =
        /\bprogetto\s+principale\b|\bmain\s+project\b|\bprimary\s+project\b/i.test(user)
      push('projects', 'Project', `User's project: ${value}.`, 7, {
        factKey: singularPrimary
          ? 'projects.primary'
          : `projects.${slugifyFactKeyPart(value)}`,
      })
    }
  }
 ...
 *   rows: any[]
 *   error: string | null
 *   containsError: string | null
 *   containsFilter: ReturnType<typeof describeFactKeyContainsFilter> | null
 *   containsRequestUrl: string | null
 *   containsRowCount: number | null
 * }>}
 */
export async function listActiveRowsForFactKey(supabase, userId, factKey, category = 'preferences') {
  ...
  /** @type {string | null} */
  let containsRequestUrl = null
  /** @type {number | null} */
  let containsRowCount = null
  const tag = encodeFactKeyTag(factKey)
  const containsFilter = describeFactKeyContainsFilter(tag, userId)
  ...
  if (tag) {
    ...
    // Capture the literal URL supabase-js constructed (includes cs.{...} encoding).
    try {
      const rawUrl = builder?.url
      containsRequestUrl =
        typeof rawUrl === 'string'
          ? rawUrl
          : rawUrl && typeof rawUrl.href === 'string'
            ? rawUrl.href
            : rawUrl && typeof rawUrl.toString === 'function'
              ? String(rawUrl.toString())
              : null
    } catch {
      containsRequestUrl = null
    }

    const keyed = await builder

    if (keyed.error) {
      containsError = keyed.error.message || String(keyed.error)
    } else if (Array.isArray(keyed.data)) {
      containsRowCount = keyed.data.length
      for (const row of keyed.data) {
        if (!row?.id) continue
...
export async function obsoleteConflictingSlotRows(supabase, userId, category, factKey, keepId) {
  const empty = {
    ...
    keyedPeersObsoleted: [],
    legacyPeersFound: [],
    legacyPeersObsoleted: [],
    containsFilter: null,
    containsRequestUrl: null,
    containsRowCount: null,
    containsError: null,
  }
  ...
  /** @type {any[]} */
  const keyedPeersFound = []
  /** @type {any[]} */
  const legacyPeersFound = []

  for (const row of keyed.rows) {
    if (String(row.id) === String(keepId)) continue
    toObsolete.set(String(row.id), { row, kind: 'keyed' })
    keyedPeersFound.push(row)
  }

  for (const row of peers.rows) {
    if (!isConflictingActiveSlotRow(row, factKey, keepId)) continue
    if (toObsolete.has(String(row.id))) continue
    const kind = readFactKeyFromTags(row.tags) === factKey ? 'keyed' : 'legacy'
    toObsolete.set(String(row.id), { row, kind })
    if (kind === 'keyed') keyedPeersFound.push(row)
    else legacyPeersFound.push(row)
  }

  const conflicts = [...toObsolete.values()].map((entry) => entry.row)
  const conflictKinds = new Map(
    [...toObsolete.entries()].map(([id, entry]) => [id, entry.kind]),
  )
  ...
  const keyedPeersObsoleted = obsoleteResult.obsoletedIds.filter(
    (id) => conflictKinds.get(String(id)) === 'keyed',
  )
  const legacyPeersObsoleted = obsoleteResult.obsoletedIds.filter(
    (id) => conflictKinds.get(String(id)) === 'legacy',
  )
  ...
  return {
    ...
    activeCountAfter,
    cleanupOk,
    keyedPeersFound: keyedPeersFound.map(sanitizeMemoryTraceRow),
    keyedPeersObsoleted,
    legacyPeersFound: legacyPeersFound.map(sanitizeMemoryTraceRow),
    legacyPeersObsoleted,
    containsFilter: keyed.containsFilter || null,
    containsRequestUrl: keyed.containsRequestUrl || null,
    containsRowCount: keyed.containsRowCount ?? null,
    containsError: keyed.containsError || null,
  }
}
...
export async function upsertMemory(input, deps = {}) {
  ...
  if (existing) {
    ...
    return { action: 'updated', memory: mapMemoryRow(data), slotCleanup: null }
  }
...
async function upsertSingleValuedMemory(supabase, userId, input, factKey, tags) {
  const traceEnabled = isMemoryTraceEnabled(input)
  /** @type {any} */
  const trace = traceEnabled
    ? {
        factKeyUsed: factKey,
        encodedTagUsed: encodeFactKeyTag(factKey),
        factKeyBytes: [...Buffer.from(String(factKey || ''), 'utf8')],
        encodedTagBytes: [...Buffer.from(String(encodeFactKeyTag(factKey) || ''), 'utf8')],
        userId: String(userId),
        supabaseProjects: supabaseProjectRefFromEnv(),
        errors: [],
      }
    : null

  if (trace) {
    const before = await snapshotFactKeyRowsForTrace(supabase, userId, factKey)
    trace.before = before
    if (before.error) trace.errors.push(`before snapshot: ${before.error}`)
  }
  ...
  if (trace) {
    trace.containsError = keyed.containsError || null
    trace.containsFilter = keyed.containsFilter || null
    trace.containsRequestUrl = keyed.containsRequestUrl || null
    trace.containsRowCount = keyed.containsRowCount ?? null
    trace.keyedLookupRows = keyed.rows.map(sanitizeMemoryTraceRow)
  }

  let existing = keyed.rows[0] || null
  /** @type {'keyed' | 'legacy' | 'insert'} */
  let canonicalSource = existing ? 'keyed' : 'insert'
  ...
  if (!existing) {
    ...
    if (existing) canonicalSource = 'legacy'
  }
  ...
  if (trace) {
    trace.canonicalId = memory?.id != null ? String(memory.id) : null
    trace.canonicalSource = canonicalSource
    trace.canonicalWrite = {
      action,
      row: sanitizeMemoryTraceRow({
        ...memory,
        user_id: userId,
        tags: memory?.tags,
  ...
  if (trace) {
    const after = await snapshotFactKeyRowsForTrace(supabase, userId, factKey)
    trace.after = after
    if (after.error) trace.errors.push(`after snapshot: ${after.error}`)
    trace.keyedPeersFound = slotCleanup?.keyedPeersFound ?? []
    trace.keyedPeersObsoleted = slotCleanup?.keyedPeersObsoleted ?? []
    trace.legacyPeersFound = slotCleanup?.legacyPeersFound ?? []
    trace.legacyPeersObsoleted = slotCleanup?.legacyPeersObsoleted ?? []
    // Prefer cleanup-stage contains forensics when present (same helper path).
    if (slotCleanup?.containsFilter) trace.containsFilter = slotCleanup.containsFilter
    if (slotCleanup?.containsRequestUrl) trace.containsRequestUrl = slotCleanup.containsRequestUrl
    if (slotCleanup?.containsRowCount != null) trace.containsRowCount = slotCleanup.containsRowCount
    if (slotCleanup?.containsError) trace.containsError = slotCleanup.containsError
    trace.cleanupOk = slotCleanup?.cleanupOk ?? null
    trace.failedIds = slotCleanup?.failedIds ?? []
    trace.slotCleanup = {
      predecessorsFound: slotCleanup?.predecessorsFound ?? null,
      obsoletedIds: slotCleanup?.obsoletedIds ?? [],
      failedIds: slotCleanup?.failedIds ?? [],
      activeCountAfter: slotCleanup?.activeCountAfter ?? null,
      cleanupOk: slotCleanup?.cleanupOk ?? null,
      error: slotCleanup?.error ?? null,
      groundTruthActiveCount: after.exactFactKeyActiveCount,
      groundTruthActiveRows: after.exactFactKeyActive,
      keyedPeersFound: slotCleanup?.keyedPeersFound ?? [],
      keyedPeersObsoleted: slotCleanup?.keyedPeersObsoleted ?? [],
      legacyPeersFound: slotCleanup?.legacyPeersFound ?? [],
      legacyPeersObsoleted: slotCleanup?.legacyPeersObsoleted ?? [],
    }
    // Flatten project refs at top level for easy mobile inspection
    trace.serverProjectRef = trace.supabaseProjects?.serverProjectRef ?? null
    trace.viteProjectRef = trace.supabaseProjects?.viteProjectRef ?? null
    trace.sameProject = trace.supabaseProjects?.sameProject ?? null
    if (
      typeof after.exactFactKeyActiveCount === 'number' &&
      after.exactFactKeyActiveCount !== slotCleanup?.activeCountAfter
    ) {
      trace.errors.push(
        `verify mismatch: listActiveRowsForFactKey activeCountAfter=${slotCleanup?.activeCountAfter} but ground-truth wide select activeCount=${after.exactFactKeyActiveCount}`,
      )
    }
    console.warn('[brain-memory] MEMORY TRACE', JSON.stringify(trace).slice(0, 6000))
  }

  return { action, memory, slotCleanup, memoryTrace: trace }
}
...
export async function runMemoryPipeline(input) {
  ...
  const items = collapseItemsBySingleValuedFactKey(rawItems)
  const memoryTrace = isMemoryTraceEnabled(input)
  ...
  for (const item of items) {
    ...
    upsertCalls.push({
      ...
      factKey: item.factKey || readFactKeyFromTags(item.tags) || null,
    ...
    if (result.memoryTrace) memoryTraces.push(result.memoryTrace)
  }
  ...
  return {
    ...
    ...(memoryTrace
      ? {
          memoryTrace: {
            supabaseProjects: supabaseProjectRefFromEnv(),
            candidates: upsertCalls,
            upsertCallCount: upsertCalls.length,
            traces: memoryTraces,
          },
        }
      : {}),
