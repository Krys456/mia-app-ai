...
/**
 * #332E2 — Sync recoverable email onto public.users after durable link.
 * Never changes id. Ignores placeholder auth:*@laife.local emails.
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} authUserId
 * @param {{ email?: string | null }} [profile]
 * @returns {Promise<string>}
 */
export async function syncPublicUserProfile(supabase, authUserId, profile = {}) {
  const id = await ensureAuthUserRow(supabase, authUserId)
  const email =
    typeof profile.email === 'string' ? profile.email.trim().toLowerCase() : ''
  if (!email || email.includes('@laife.local') || !email.includes('@')) {
    return id
  }

  const { error } = await supabase.from('users').update({ email }).eq('id', id)
  if (error) {
    // Unique collision (email already on another public.users row) — do not crash chat.
    console.warn(
      '[syncPublicUserProfile] email sync skipped',
      JSON.stringify({ code: error.code || 'update_failed' }),
    )
    return id
  }
  return id
...
export function isEphemeralNoise(userMessage, options = {}) {
  ...
  // #357B — reminder mutation / scheduling turns are DB-backed, not Memory facts.
  if (
    /\b(ricordami|remind\s+me|promemoria|cancella\s+(il\s+)?promemoria|segna\s+.*come\s+fatto)\b/i.test(
      text,
    )
  ) {
    return true
