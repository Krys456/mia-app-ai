...
export function extractDurableFacts(userMessage) {
  ...
  const pushCofavorite = (subjectRaw, valueRaw) => {
    ...
    if (facts.some((f) => f.factKey === factKey && f.operation !== 'revoke')) return true
    push(
      'preferences',
      'Co-favorite',
  ...
  if (!favoriteLanguageNegated && !favoriteIsQuestion) {
  ...
  for (const fact of unique.sort((a, b) => b.importance - a.importance)) {
    ...
    const nearDup = collapsed.some((kept) => {
      // Never collapse revoke ops into positive writes (or vice versa).
      if (
        String(kept.operation || '').toLowerCase() === 'revoke' ||
        String(fact.operation || '').toLowerCase() === 'revoke'
      ) {
        return (
          String(kept.operation || '') === String(fact.operation || '') &&
          kept.factKey &&
          fact.factKey &&
...
export async function upsertMemory(input, deps = {}) {
  ...
  if (existing) {
    if (normalizeText(existing.content) === normalizeText(input.content)) {
      if (factKey && factKey.startsWith('preferences.cofavorite.')) {
...
export async function runMemoryPipeline(input) {
  ...
  // Belt-and-suspenders: never revoke favorites from interrogative / unsafe turns.
  const skipFavoriteRevoke = shouldSkipFavoriteRevocation(input.userMessage || '')
  const guardedItems = skipFavoriteRevoke
    ? filteredItems.filter((item) => String(item?.operation || '').toLowerCase() !== 'revoke')
    : filteredItems
  ...
  // Revoke items keep their own operation and are not collapsed into upserts.
  const upsertCandidates = guardedItems.filter(
  ...
  // Resolve owner once when revokes need it.
  let revokeUserId = explicitUserId
  let revokeSupabase = input.supabase || null
  if (revokeCandidates.length > 0) {
    revokeSupabase = input.supabase ?? (await getServiceSupabase())
    revokeUserId = await resolveMemoryUserId(
      {
  ...
  for (const item of revokeCandidates) {
    ...
    const result = await applyFavoriteRevocation(revokeSupabase, revokeUserId, item)
  ...
  return {
    saved: created + updated > 0 || revoked > 0,
    updated: updated > 0,
    revoked: revoked > 0,
    skipped: created + updated + revoked === 0 && skipped > 0,
    decision: decisionResult,
    memory: lastMemory,
    stats: { created, updated, skipped, revoked },
    slotCleanups,
    revokeResults,
  }
}
