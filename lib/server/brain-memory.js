 ...
 * @returns {string}
 */
function favoriteValueSlugFromContent(content) {
  const text = String(content || '')
  const afterColon = text.match(/:\s*(.+?)(?:\.|$)/)
  const raw = cleanCapture(afterColon?.[1] || text)
    .replace(/^(?:il|lo|la|i|gli|le|un|uno|una|the|a|an)\s+/i, '')
    .trim()
...
export function extractDurableFacts(userMessage) {
  ...
  // Order: interrogative gate → negation gate → partitive → plural → singular.
  ...
  // Questions about favorites must never persist as declarations (Preview regression).
  ...
  const pushCofavorite = (subjectRaw, valueRaw) => {
    ...
    let value = cleanCapture(valueRaw).replace(
      /^(?:il|lo|la|i|gli|le|un|uno|una|the|a|an)\s+/i,
      '',
    )
    ...
    if (facts.some((f) => f.factKey === factKey)) return true
    push(
      'preferences',
      'Co-favorite',
