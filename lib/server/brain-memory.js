...
export function rewriteFavoriteRevokeSuccessor(text) {
  ...
  // Right must look like a positive singular favorite assert (not another revoke).
  if (extractFavoriteRevokeCandidates(right).length > 0) return null
  const subject = revokes[0].subject
  const hasPositive =
    new RegExp(
      `(?:(?:il|la)\\s+mi[oa]\\s+${subject}\\s+preferit[oa]\\s+[eè]\\s+|my\\s+favorite\\s+${subject}\\s+is\\b)`,
      'i',
    ).test(right) ||
...
export async function runMemoryPipeline(input) {
  ...
  if (skipFavoriteRevoke) {
    guardedItems = guardedItems.filter((item) => {
      ...
      // Keep identity.name revokes; drop favorite/cofavorite revokes only.
      const isIdentityNameRevoke =
        String(item?.targetType || '').toLowerCase() === 'name' ||
        String(item?.factKey || '') === 'identity.name' ||
        String(item?.category || '').toLowerCase() === 'identity'
      return isIdentityNameRevoke
    })
  }
