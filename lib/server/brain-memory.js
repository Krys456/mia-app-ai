...
async function findUpsertTarget(supabase, userId, category, title, content, factKey) {
  ...
  const peers = await loadSameUserCategoryPeers(supabase, userId, category)
  if (peers.error) {
...
/**
 * Load same-user category peers for conflict detection (active + non-archived).
 ...
 *
 * Verifies each UPDATE via SELECT representation — never treats zero-row updates as success.
 *
 * @param {any} supabase
 * @param {string} userId
 * @param {string} category
...
export async function obsoleteConflictingSlotRows(supabase, userId, category, factKey, keepId) {
  ...
  const conflicts = peers.rows.filter((row) => isConflictingActiveSlotRow(row, factKey, keepId))
 ...
 * Returns { action: 'created' | 'updated' | 'skipped', memory?, slotCleanup? }
 *
 * @param {any} input
 * @param {{ supabase?: any }} [deps] injectable supabase for tests
 */
export async function upsertMemory(input, deps = {}) {
  ...
  if (existing) {
    if (normalizeText(existing.content) === normalizeText(input.content)) {
      ...
      if (factKey && isSingleValuedFactKey(factKey)) {
        slotCleanup = await obsoleteConflictingSlotRows(
          supabase,
          userId,
      ...
      return { action: 'skipped', memory, slotCleanup }
    }
    ...
    return { action: 'updated', memory: mapMemoryRow(data), slotCleanup }
  }
  ...
  return { action: 'created', memory, slotCleanup }
}
