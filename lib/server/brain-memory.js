 ...
 * @param {string} factKey
 * @param {string} [category]
 * @returns {Promise<{ rows: any[], error: string | null, containsError: string | null }>}
 */
export async function listActiveRowsForFactKey(supabase, userId, factKey, category = 'preferences') {
  ...
  if (peers.error && byId.size === 0) {
    return { rows: [], error: peers.error, containsError }
  }
...
async function upsertSingleValuedMemory(supabase, userId, input, factKey, tags) {
  ...
  if (trace) {
    ...
    trace.keyedPeersFound = (slotCleanup?.obsoletedIds || []).length
    trace.slotCleanup = {
      ...
      // Ground-truth from wide user_id select (no contains):
    ...
    console.warn('[brain-memory] MEMORY TRACE', JSON.stringify(trace).slice(0, 4000))
  }
