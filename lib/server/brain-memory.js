...
function extractLegacyPreferenceValue(content) {
  ...
  const favorite = text.match(/user'?s\s+favorite(?:\s+[^:]+)?:\s*(.+?)(?:\.|$)/i)
...
function isLegacyFavoriteColorPredecessor(title, content, category) {
  ...
  if (/user'?s\s+favorite\s+(?:colore|color|colour)\s*:/i.test(content)) return true

  // Historical bare Favorite: User's favorite: il blu
  if (/^favorite$/i.test(title) && /user'?s\s+favorite\s*:/i.test(content)) {
  ...
  if (/^preference$/i.test(title) && /likes\s*\/\s*prefers\s*:/i.test(content)) {
    const value = extractLegacyPreferenceValue(content)
    return Boolean(value && valueMatchesVocab(value, FAVORITE_COLOR_VALUES))
  }
...
function isLegacyFavoriteAnimalPredecessor(title, content, category) {
  ...
  if (/user'?s\s+favorite\s+(?:animale|animal)\s*:/i.test(content)) return true
...
function isLegacyFavoriteSubjectPredecessor(title, content, category, subjectKey) {
  ...
  const subjectRe = new RegExp(`user'?s\\s+favorite\\s+(?:${forms})\\s*:`, 'i')
...
async function findUpsertTarget(supabase, userId, category, title, content, factKey) {
  ...
  return selectUpsertTarget(data ?? [], { factKey, category, title, content })
}

/**
 * After a single-valued fact_key write, mark other same-user conflicting actives obsolete:
 * duplicate keyed rows and high-confidence untagged legacy predecessors.
 *
 * @param {any} supabase
 * @param {string} userId
 * @param {string} category
 ...
 * @param {string} keepId
 */
async function obsoleteConflictingSlotRows(supabase, userId, category, factKey, keepId) {
  if (!isSingleValuedFactKey(factKey) || !keepId) return
  ...
  const conflicts = data.filter((row) => isConflictingActiveSlotRow(row, factKey, keepId))
  ...
  for (const row of conflicts) {
    ...
    if (updateError) {
      // Best-effort: canonical write already succeeded; do not fail the turn.
      console.warn(
        `[brain-memory] failed to obsolete legacy slot peer ${row.id} for ${factKey}:`,
        updateError.message,
      )
    }
  }
...
export async function upsertMemory(input) {
  ...
  if (existing) {
    if (normalizeText(existing.content) === normalizeText(input.content)) {
      // Ensure legacy rows pick up fact_key tags even on skip.
      const existingKey = readFactKeyFromTags(existing.tags)
      let memory = mapMemoryRow(existing)
      if (factKey && existingKey !== factKey) {
      ...
      if (factKey && isSingleValuedFactKey(factKey)) {
        await obsoleteConflictingSlotRows(
          supabase,
          userId,
  ...
  if (factKey && isSingleValuedFactKey(factKey) && memory?.id) {
    await obsoleteConflictingSlotRows(
      supabase,
      userId,
