...
export function normalizeLifecycleEntityValue(value) {
  ...
  v = v
    ...
    .replace(/^(?:adesso|ora|now|anzi|actually)\s+/i, '')
    .trim()
  v = cleanCapture(v)
  if (!v) return ''
  if (/^(?:no\s+longer|non\s+pi[uù]|pi[uù]|anymore|item)$/i.test(v)) return ''
  return v
}
...
export function hasPositivePrimaryProjectAssert(message) {
  ...
  return (
    /(?:(?:il|la)\s+mi[oa]\s+progetto\s+principale\s+(?:[eè]|si\s+chiama)\s+)(?!non\b)([^,.!?\n]{2,90})/i.test(
      user,
    ) ||
    /(?:il\s+progetto\s+principale(?:\s+su\s+cui\s+sto\s+lavorando)?\s+[eè]\s+)(?!non\b)([^,.!?\n]{2,90})/i.test(
      user,
    ) ||
    /(?:my\s+(?:main|primary)\s+project\s+(?:is(?:\s+called)?)\s+)(?!no\s+longer\b)([^,.!?\n]{2,90})/i.test(
      user,
    ) ||
    /(?:the\s+main\s+project\s+i(?:'m|\s+am)\s+working\s+on\s+is\s+)(?!no\s+longer\b)([^,.!?\n]{2,90})/i.test(
      user,
    ) ||
    /\b([A-Za-zÀ-ÖØ-öø-ÿ0-9][^,.!?\n]{0,80}?)\s+[eè]\s+(?:il\s+)?(?:mi[oa]\s+)?progetto\s+principale\b/i.test(
      user,
    ) ||
    /\b([A-Za-z0-9][^,.!?\n]{0,80}?)\s+is\s+my\s+(?:main|primary)\s+project\b/i.test(user)
  )
}
...
export function isSingleValuedFactKey(factKey) {
  ...
  // Conditioned reply-style slots are single-valued per condition (#280).
...
export function isTemporaryInstructionCue(message) {
  ...
  return /\b(?:for\s+the\s+next\s+step|for\s+this\s+(?:step|test|turn|task)|just\s+for\s+now|for\s+now|temporarily|this\s+turn|in\s+this\s+message|per\s+il\s+prossimo\s+step|per\s+questo\s+(?:step|test|turno)|solo\s+per\s+ora|temporaneamente)\b/i.test(
    t,
  )
}
...
export function classifyReplyBrevity(text) {
  ...
  if (
    /\b(detailed|dettagliat|in[- ]?depth|lungh[ei]|verbose|thorough)\b/i.test(t)
  ) {
    return 'detailed'
  }
  if (
    /\b(concise|short|brief|brevi|concis|sintetic)\b/i.test(t) &&
    /\b(answer|answers|reply|replies|response|responses|rispost)/i.test(t)
  ) {
    return 'concise'
  }
  if (
 ...
 */
export function buildReplyStyleFactKey(style, condition) {
  const base = 'settings.reply_style'
...
export function extractProjectNamingCandidates(message) {
  ...
  let m =
    user.match(
      /(?:(?:the|il|la)\s+)?(?:(\w+)\s+)?(?:shop|negozio|store)\s+(?:is\s+)?(?:called|named|si\s+chiama)\s+([^,.!?\n]{2,60})/i,
    ) ||
    user.match(
      /(?:(?:the|il|la)\s+)?(?:(\w+)\s+)?(?:shop|negozio|store)\s+[eè]\s+(?:called|named)?\s*([^,.!?\n]{2,60})/i,
    )
  if (m?.[2]) {
    const shopKind = cleanCapture(m[1] || 'shop').toLowerCase()
    const label = shopKind && shopKind !== 'shop' ? `${shopKind} shop` : 'shop'
    pushNamed(
      'shop',
      m[2],
      'Shop',
      `User's ${label}`,
      `projects.shop.${slugifyFactKeyPart(shopKind || 'shop')}`,
      shopKind || 'shop',
    )
  }
...
export function extractDurableFacts(userMessage) {
  ...
  // Unconditional / conditioned reply-style (prefer / keep answers brief / risposte brevi)
  {
    ...
    if (leadingWhen?.[1] && leadingWhen?.[2]) {
      forcedCondition = cleanCapture(leadingWhen[1]).replace(/^facciamo\s+/i, '')
      ...
      const scope =
        leadingScope.scopeKind === 'project'
          ...
          : leadingScope.scopeKind === 'this_project'
            ? 'this_project'
            : null
      const factKey = buildReplyStyleFactKey(style, condition)
  ...
  if (leadingScope.scopeKind === 'project' && leadingScope.scope) {
    ...
    for (const clause of clauseList) {
      ...
      if (brevity) {
        const factKey = buildReplyStyleFactKey(brevity, condition)
        push(
          'settings',
          'Reply preference',
      ...
      const content = `For ${leadingScope.scope}: user prefers ${cleanCapture(value)}.`
      const factKey = `projects.${slugifyFactKeyPart(leadingScope.scope)}.preferences.${slugifyFactKeyPart(value)}`
  ...
  if (!skipPolarity && !dislikeMatched && !replyStyleMatched) {
    ...
    if (likeValue && likeValue.length >= 2 && !isMomentaryInterestObject(likeValue)) {
      ...
      if (brevity) {
        ...
        const style = /\b(dettagliat|detailed|lungh)\b/i.test(likeValue) ? 'detailed' : 'concise'
        const factKey = buildReplyStyleFactKey(style, condition)
        push('settings', 'Reply preference', `User prefers: ${likeValue}.`, 8, { factKey })
        replyStyleMatched = true
        preferenceMatched = true
  ...
  const primaryProjectPatterns = [
    ...
    // EN: My main/primary project is / is called X  (not "is no longer")
    /(?:my\s+(?:main|primary)\s+project\s+(?:is(?:\s+called)?)\s+)(?!no\s+longer\b)([^,.!?\n]{2,90})/i,
  ...
  if (!primaryRevokeMatched) {
    for (const pattern of primaryProjectPatterns) {
      const match = user.match(pattern)
      if (!match?.[1]) continue
      const value = stripNamingVerbPrefix(match[1])
  ...
  if (renamePair?.oldName && renamePair?.newName) {
    ...
    if (
      ...
      projectMatched = true
      push('projects', 'Primary project', `User's primary project: ${newName}.`, 8, {
        factKey: 'projects.primary',
      })
      push('projects', 'Project', `User's project: ${newName}.`, 7, {
        factKey: `projects.${slugifyFactKeyPart(newName)}`,
      })
    }
  }
