...
export function isEphemeralNoise(userMessage, options = {}) {
  ...
  const durableCue =
    /\b(mi\s+chiamo|il\s+mio\s+nome|sono\s+(?:un|una)|preferisco|mi\s+piace|non\s+mi\s+piace|odio|amo|adoro|appassionat|hobby|lavoro|studio|imparando|imparare|obiettivo|progetto|sto\s+(?:sviluppando|creando|lavorando)|ricorda|sempre|mai|uso|utilizzo|abito|vivo|cane|gatto|my\s+name|i\s+am|i'm|i\s+prefer|i\s+like|i\s+love|i\s+hate|i\s+work|i\s+study|i\s+use|learning|studying|my\s+goal|my\s+favorite|my\s+dog|my\s+cat|remember\s+that|always|never|developing|building|working\s+on)\b/i.test(
      text,
    )
...
export function extractDurableFacts(userMessage) {
  ...
  /** @type {Array<{category:string,title:string,content:string,importance:number,source?:string,confidence?:number}>} */
  const facts = []
  ...
  const push = (category, title, content, importance) => {
    ...
    facts.push(decision(category, title, cleaned, importance, factMeta))
  }
