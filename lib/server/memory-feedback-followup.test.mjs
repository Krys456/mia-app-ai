...
describe('cofavorite member removal (#281)', () => {
  it('A–E: create three colors, remove one → removed event + recall excludes it', async () => {
    const db = createFakeSupabase()
    const created = await pipeline(db, 'I miei colori preferiti sono rosso, blu e viola.')
    assert.equal(created.stats.created, 3)

    const remove = await pipeline(db, 'Il viola non è più il mio colore preferito.')
    assert.ok(remove.stats.revoked >= 1)
    assert.equal(remove.stats.created, 0)
    assert.equal(remove.stats.updated, 0)
    ...
    const obsolete = db.rows
      .filter((r) => r.status === 'obsolete')
      .map((r) => readFactKeyFromTags(r.tags))
    assert.ok(obsolete.includes('preferences.cofavorite.color.viola'))
  ...
  it('F: remove nonexistent cofavorite → no mutation / null', async () => {
    const db = createFakeSupabase()
    await pipeline(db, 'I miei colori preferiti sono rosso e blu.')
    const remove = await pipeline(db, 'Il viola non è più il mio colore preferito.')
    assert.equal(remove.stats.revoked, 0)
    assert.equal(mapMemoryPipelineToFeedbackEvent(remove), null)
    assert.equal(
      db.rows.filter((r) => r.status === 'active').length,
      2,
    )
  })

  it('G: mixed remove+add turn → updated (not removed-only)', async () => {
    const db = createFakeSupabase()
    await pipeline(db, 'I miei colori preferiti sono rosso, blu e viola.')
    // Explicit replace_set / "adesso … solo" is the high-confidence mixed path.
    const mixed = await pipeline(
      db,
      'Adesso i miei colori preferiti sono solo rosso e verde.',
    )
    assert.ok(mixed.stats.replaced >= 1 || mixed.stats.created >= 1 || mixed.stats.updated >= 1)
    const event = mapMemoryPipelineToFeedbackEvent(mixed)
    assert.equal(event?.type, 'updated')
  })

  it('H: characters cofavorite removal via singular phrasing', async () => {
    const db = createFakeSupabase()
    await pipeline(db, 'I miei personaggi preferiti sono Itachi e Sasuke.')
    const remove = await pipeline(db, 'Itachi non è più il mio personaggio preferito.')
    assert.ok(remove.stats.revoked >= 1)
    assert.equal(mapMemoryPipelineToFeedbackEvent(remove)?.type, 'removed')
    const active = db.rows
      .filter((r) => r.status === 'active')
      .map((r) => readFactKeyFromTags(r.tags))
    assert.ok(!active.includes('preferences.cofavorite.character.itachi'))
    assert.ok(active.includes('preferences.cofavorite.character.sasuke'))
  })

  it('I: food cofavorite removal', async () => {
    const db = createFakeSupabase()
    await pipeline(db, 'I miei cibi preferiti sono pizza, sushi e pasta.')
    const remove = await pipeline(db, 'Pizza non è più uno dei miei cibi preferiti.')
    assert.ok(remove.stats.revoked >= 1)
    assert.equal(mapMemoryPipelineToFeedbackEvent(remove)?.type, 'removed')
    const active = db.rows
      .filter((r) => r.status === 'active')
      .map((r) => readFactKeyFromTags(r.tags))
    assert.ok(!active.includes('preferences.cofavorite.food.pizza'))
    assert.ok(active.includes('preferences.cofavorite.food.sushi'))
  })

  it('J: existing singular favorite removal still works', async () => {
    const db = createFakeSupabase()
    await pipeline(db, 'Il mio colore preferito è rosso.')
    const remove = await pipeline(db, 'Il rosso non è più il mio colore preferito.')
    assert.ok(remove.stats.revoked >= 1)
    assert.equal(mapMemoryPipelineToFeedbackEvent(remove)?.type, 'removed')
    assert.equal(db.rows.filter((r) => r.status === 'active').length, 0)
  })

  it('EN: Purple is no longer my favorite color → cofavorite revoked', async () => {
    const db = createFakeSupabase()
    await pipeline(db, 'My favorite colors are red, blue and purple.')
    const remove = await pipeline(db, 'Purple is no longer my favorite color.')
    assert.ok(remove.stats.revoked >= 1)
    assert.equal(mapMemoryPipelineToFeedbackEvent(remove)?.type, 'removed')
    const active = db.rows
      .filter((r) => r.status === 'active')
      .map((r) => readFactKeyFromTags(r.tags))
    assert.ok(!active.includes('preferences.cofavorite.color.purple'))
    assert.ok(active.includes('preferences.cofavorite.color.red'))
    assert.ok(active.includes('preferences.cofavorite.color.blue'))
  })

  it('K/L: message-bound indicator wiring still present; null when no mutation', () => {
