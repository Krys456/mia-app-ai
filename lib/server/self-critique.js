...
const GENERIC_OPENERS =
  /\b(in\s+(today'?s|questo)\s+world|it'?s\s+important\s+to\s+note|è\s+importante\s+(notare|sapere)|as\s+an\s+ai|come\s+intelligenza\s+artificiale|there\s+are\s+(many|several)\s+(ways|factors)|ci\s+sono\s+(molti|diversi)\s+(modi|fattori)|in\s+conclusione,?\s+possiamo\s+dire)\b/i

const TEMPLATE_FILLER =
  /\b(i'?m\s+here\s+to\s+help|sono\s+qui\s+per\s+aiutarti|non\s+esitare\s+a|feel\s+free\s+to|let\s+me\s+know\s+if|fammi\s+sapere\s+se|posso\s+aiutarti\s+con\s+altro|hope\s+this\s+helps|spero\s+ti\s+sia\s+utile)\b/i
...
export function mergePreSendRefineBudget(opts) {
  ...
  const instructions = [
    'Sei il Writer di LAIfe. Esegui UNA sola rifinitura pre-invio (budget unico: Self-Critique + Satisfaction).',
    'Mai un secondo loop. Non inventare. Non allungare a vuoto. Non citare motori/gate.',
    'Priorità: togli generico/ripetizioni/filler; alza chiarezza; un dettaglio vivo se manca; mantieni le idee forti.',
    ...briefs,
    'Restituisci solo il testo finale migliorato.',
    'Bozza:',
    '---',
