...
/**
 * POSIX Etc/GMT±N zones use inverted signs and are a common OS misconfig
 * (e.g. browser reports Etc/GMT+12 while the user expects Europe/Rome).
 * Treat them as unspecified so Calendar can fall back to Google primary TZ.
 * @param {string} tz
 */
...
export function sanitizeTimeZone(value) {
  ...
  // #375R — do not let Etc/GMT* override Google Calendar primary timezone.
  if (isUnreliableCalendarTimeZone(tz)) return null
  try {
