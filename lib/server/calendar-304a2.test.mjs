...
function baseEnv(extra = {}) {
  return {
    ...
    CALENDAR_TOKEN_ENCRYPTION_KEY: testKeyHex(),
...
describe('#304A2 file surface + readonly guarantees', () => {
  ...
  it('read service remains importable; chat wiring is #304A3 concern', () => {
    // #304A2 itself does not add Vercel calendar routes. Chat may import calendar-read via #304A3.
    assert.equal(fs.existsSync(path.join(root, 'api/calendar.ts')), false)
    assert.match(readSrc, /listCalendars|listEvents|freeBusy/)
  })
...
describe('#304A2 regressions (#304A1 / #303 / Core / Memory / Vision / Privacy)', () => {
  ...
  it('does not add Vision API route; Privacy still covers Calendar', () => {
