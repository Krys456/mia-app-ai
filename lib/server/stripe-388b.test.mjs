...
// —— Architecture / Hobby safety ——
{
  ...
  // #388B.1: Web Standard fetch export (not Node helper / not bodyParser:false).
  assert.doesNotMatch(read('api/subscription.ts'), /export const config = \{[\s\S]*bodyParser:\s*false/)
  assert.match(read('api/subscription.ts'), /export default\s*\{[\s\S]*async fetch\s*\(/)
  assert.doesNotMatch(read('api/subscription.ts'), /^export default async function/m)
