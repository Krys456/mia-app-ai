...
export function refineToolSelection(candidates, input) {
  ...
  if (tools.includes('calculator') && tools.length > 1) {
    const pureCalc =
      /(?:^|[^\w])\d+(?:[.,]\d+)?\s*[\+\-\*\/×÷%^]\s*\d/.test(text) ||
      /\b(calcola|calcolo|quanto\s+fa|compute|calculate)\b/i.test(text)
    if (pureCalc && !freshness && !personal) {
      tools = /** @type {ToolId[]} */ (['calculator'])
    }
  }
...
export function buildOrchestratorContext(results) {
  ...
  const deduped = dedupeToolResults(ordered)
