...
// —— Plans identity gate ——
{
  ...
  // #388B: Checkout is server-gated; identity gate remains required for anonymous.
  assert.match(plans, /pagamenti saranno disponibili|Pagamenti.*breve|Checkout sicuro|procedere con l’upgrade/i)
  assert.match(plans, /startPlanCheckout/)
  assert.doesNotMatch(plans, /purchase\(|@stripe\/stripe-js|CardElement|STRIPE_SECRET/i)
  // #332E2A — stable callback + equality (no render loop)
  assert.match(plans, /useCallback/)
  assert.match(plans, /identityStatusEquals/)
  assert.match(plans, /autoFocus/)
}

// —— #332E2A panel: mount-only effect + change-gated notify ——
{
  const panel = read('src/components/IdentityAccountPanel.tsx')
  assert.match(panel, /onIdentityChangeRef/)
  assert.match(panel, /reportIfChanged|identityStatusEquals/)
  assert.match(panel, /intentional mount-only|mount-only/)
  assert.match(panel, /scrollIntoView/)
  assert.match(panel, /plans-identity-gate/)
  assert.doesNotMatch(panel, /useEffect\(\(\) => \{\s*void refresh\(\)\s*\}, \[refresh\]\)/)
}

// —— identityStatusEquals ——
{
  const a = resolveIdentityStatus({
    id: '11111111-1111-1111-1111-111111111111',
    is_anonymous: true,
  })
  const b = resolveIdentityStatus({
    id: '11111111-1111-1111-1111-111111111111',
    is_anonymous: true,
  })
  assert.equal(identityStatusEquals(a, b), true)
  assert.equal(identityStatusEquals(a, { ...b, durable: true }), false)
}
...
// #388B adds Stripe server SDK; durable identity remains payment-agnostic.
