...
// --- /api/health handler contracts (Hobby-safe rewrite into subscription) ---
{
  ...
  // #388B.1 uses webJson (Web Fetch Response); legacy sendJson also acceptable.
  assert.match(api, /webJson|sendJson/)
  assert.match(api, /export default\s*\{[\s\S]*async fetch\s*\(/)
