...
// --- /api/health handler contracts ---
{
  const api = read('api/health.ts')
...
// --- vercel.json registers health ---
{
  const vercel = JSON.parse(read('vercel.json'))
  assert.ok(vercel.functions['api/health.ts'])
  assert.equal(Object.keys(vercel.functions).length, 13)
}

assert.equal(existsSync(join(root, 'api/health.ts')), true)
