 ...
 * }} ReminderCreateData
 *
 * @typedef {{ ok: true, data: ReminderCreateData } | { ok: false, errors: Record<string, string> }} ReminderCreateValidation
 */
 ...
 * @param {Date} [now]
 * @returns {ReminderCreateValidation}
 */
...
/**
 * @typedef {{ ok: true, data: Record<string, unknown>, idempotent?: boolean } | { ok: false, errors: Record<string, string> }} ReminderUpdateValidation
 */
 ...
 * @param {Date} [now]
 * @returns {ReminderUpdateValidation}
 */
export function validateReminderUpdateInput(input, current, now = new Date()) {
  ...
  if ('status' in input) {
    ...
    if (!REMINDER_STATUSES.includes(/** @type {any} */ (status))) {
      ...
      isIdempotentReminderStatusAck(status)
    ) {
      // #380E — delivered → delivered: idempotent ack. Do not patch status or
      // delivered_at (preserve existing timestamp; no delivery_attempts bump).
  ...
  if (Object.keys(patch).length === 0) {
    // Status-only duplicate delivered ack → success with empty patch (API returns current row).
    const requested =
      typeof input.status === 'string' ? input.status.trim() : ''
    if (
      'status' in input &&
      requested === current.status &&
      isIdempotentReminderStatusAck(requested)
    ) {
 ...
 *
 * #303B: claim/lease columns are intentionally ignored here. A background
 * worker lease must NEVER hide a due reminder from GET ?due=1 / next-open.
...
export async function listDueReminders(scope, opts = {}) {
  ...
  // Do not filter on claim_owner / claim_expires_at / next_attempt_at.
...
export async function updateReminder(id, patch, scope) {
  ...
  // Increment delivery attempts when marking delivered (first transition only).
  // #380E: duplicate delivered ack never reaches here with status in patch.
  ...
  // #303B: terminal / acknowledged rows must not stay actionable via stale leases.
  // Cancel / complete / delivered clears worker metadata; eligibility also excludes them.
  if (
    patch.status === 'delivered' ||
    patch.status === 'completed' ||
    patch.status === 'cancelled'
  ) {
    updateRow.claim_owner = null
    updateRow.claimed_at = null
    updateRow.claim_expires_at = null
  }

  // #303C: reschedule / snooze starts a new due cycle — allow future push again.
  if (patch.fire_at != null || patch.status === 'snoozed' || patch.snooze_until != null) {
    updateRow.push_sent_at = null
    updateRow.claim_owner = null
    updateRow.claimed_at = null
    updateRow.claim_expires_at = null
    updateRow.next_attempt_at = null
  }
