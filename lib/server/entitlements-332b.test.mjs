...
// —— Runtime: client claim ignored; verifiedPlanId honored only from trusted server ——
assert.equal(resolveRuntimePlanId({}), 'free')
assert.equal(resolveRuntimePlanId({ claimedPlanId: 'pro' }), 'free')
assert.equal(resolveRuntimePlanId({ claimedPlanId: 'base', verifiedPlanId: 'pro' }), 'pro')
{
...
// —— No billing-provider schema (subscriptions_332d persistence is #332D) ——
{
  const migrations = fs.readdirSync(path.join(root, 'supabase/migrations'))
  assert.equal(
    migrations.filter((n) => /billing_provider|stripe_customers|entitlement_enforcement|plan_quotas/i.test(n))
      .length,
    0,
  )
  const entitlementsSrc = read('lib/server/entitlements.js')
  assert.doesNotMatch(entitlementsSrc, /RevenueCat|StoreKit|Play Billing/i)
}
