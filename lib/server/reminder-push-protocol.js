 ...
 * @param {unknown} raw
 * @returns {{ ok: true, data: { reminderId: string, title: string, body: string, url: string, tag: string } } | { ok: false, code: string }}
 */
export function validateServiceWorkerPushPayload(raw) {
  ...
  return {
    ok: true,
    data: { reminderId, title: title || 'Promemoria', body, url, tag },
  }
}
