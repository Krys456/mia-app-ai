 ...
 * }} options
 * @returns {Promise<{ userId: string, isAnonymous: boolean | null, durable?: boolean, email?: string | null } | null>}
 */
export async function requirePaidApiAccess(req, res, options) {
  ...
  // #332C/#332D/#388C — auth → entitlement (optional; DB if enforcement OR shadow) → rate-limit.
  if (options.entitlement) {
    const decision = await decideEntitlement({
      ...
      env: options.env,
      requestId: obs.requestId,
      route: typeof req?.url === 'string' ? String(req.url).split('?')[0] : undefined,
    })
    if (!decision.allowed) {
      console.warn(
