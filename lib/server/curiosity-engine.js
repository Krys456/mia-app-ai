...
function generateCandidates(args) {
  ...
  for (const q of (openQuestions || []).slice(0, 3)) {
    ...
    raw.push({
      ...
      kind: 'open_thread',
      idea: `Chiudi il filo aperto: ${idea}`,
      usefulness: 4.4,
      surprise: 2.6,
  ...
  raw.push({
    ...
    idea: `Un angolo controintuitivo su ${label} che la maggior parte delle persone non si aspetta`,
    usefulness: 3.2,
    surprise: 4.7,
  ...
  raw.push({
    ...
    idea: `L’errore pratico più comune quando si applica ${label} (e come evitarlo)`,
    usefulness: 4.6,
    surprise: 3.4,
    educationalValue: 4.0,
    continuity: 3.8,
  ...
  raw.push({
    id: 'deeper',
    kind: 'deeper_why',
    idea: `Il “perché funziona così” un livello più sotto su ${label}`,
    usefulness: 3.8,
    surprise: 3.5,
  ...
  raw.push({
    id: 'compare',
    kind: 'comparison',
    idea: `Un confronto breve tra ${label} e l’alternativa più vicina (quando scegliere cosa)`,
    usefulness: 4.0,
    surprise: 3.2,
  ...
  raw.push({
    id: 'next_skill',
    kind: 'next_skill',
    idea: `Il passo successivo naturale dopo aver capito ${label}`,
    usefulness: 4.3,
    surprise: 2.8,
  ...
  raw.push({
    id: 'adjacent',
    kind: 'adjacent_topic',
    idea: `Un tema adiacente che rende ${label} più chiaro e potente`,
    usefulness: 3.5,
    surprise: 3.8,
  ...
  if (domain === 'science' || interestHints.includes('science')) {
    raw.push({
      id: 'scientific',
      kind: 'scientific',
      idea: `Un insight scientifico/meccanico su ${label} che cambia come lo immagini`,
      usefulness: 3.6,
      surprise: 4.3,
  ...
  if (domain === 'history' || interestHints.includes('history')) {
    raw.push({
      id: 'historical',
      kind: 'historical',
      idea: `Un contesto storico breve su ${label} che spiega perché è fatto così oggi`,
      usefulness: 3.2,
      surprise: 4.0,
      educationalValue: 4.0,
      continuity: 3.4,
  ...
  if (domain === 'tech') {
    ...
    raw.push({
      id: 'tech_practice',
      kind: 'practical_pitfall',
      idea: `Una best practice concreta su ${label} che evita ore di debug`,
      usefulness: 4.7,
      surprise: 3.0,
      educationalValue: 3.8,
      continuity: 3.9,
...
function buildExtendBrief(chosen) {
  return [
    ...
    'Integra in modo fluido (intreccio breve o coda di 1–3 frasi).',
    'VIETATO: “Anything else?”, “What would you like to know?”, “Posso aiutarti con altro?”, domande generiche di chiusura.',
    'Question Economy: estendi con affermazione/idea — non con una domanda all’utente.',
