...
assert.equal(buildImageGenerationTools('gpt-5.6-sol')[0].output_format, 'jpeg')
...
// --- assistant generated attachment requires HMAC proof; spoof rejected ---
{
  const secret = 'img-gen-test-secret'
  process.env.LAIFE_IMAGE_ARTIFACT_SECRET = secret
  const id = 'ig_ok'
  const proof = signImageArtifact({ id, source: 'generated', dataUrl: tinyJpeg }, secret)
...
// --- api/chat invariants ---
{
  ...
  assert.match(chatSrc, /sealChatApiImages/)
