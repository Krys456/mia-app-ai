 ...
 *
 * Wired into /api/memories* via lib/server/memory-api-auth.js (not /api/chat yet).
 */

import { getServiceSupabase } from './supabase.js'
import { resolveIdentityStatus } from './durable-identity.js'
 ...
 *   providers: string[]
 *   user: { id: string, email?: string | null, is_anonymous?: boolean, identities?: unknown }
 * }>}
 * @throws {AuthError}
 */
export async function verifySupabaseAccessToken(accessToken, options = {}) {
  ...
  const identity = resolveIdentityStatus(user)
  const email =
    typeof user?.email === 'string' && user.email.trim() && !user.email.includes('@laife.local')
 ...
 * @param {{ getSupabase?: () => Promise<{ auth: { getUser: (jwt: string) => Promise<any> } }> }} [options]
 * @returns {Promise<{ userId: string, isAnonymous: boolean | null, durable: boolean, email: string | null, emailMasked: string | null, providers: string[], user: { id: string }, accessToken: string }>}
 * @throws {AuthError}
 */
