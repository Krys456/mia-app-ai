...
/**
 * @typedef {{
 *   clientAuthHint: 'unconfigured' | 'absent' | 'present' | 'unknown'
 *   bearerPresent: boolean
 *   jwtVerified: boolean
 *   usersRowEnsured: boolean
...
/**
 * Read Preview-safe client auth hint header (never a token).
 * @param {{ headers?: Record<string, unknown> } | null | undefined} req
 * @returns {'unconfigured' | 'absent' | 'present' | 'unknown'}
 */
function readClientAuthHint(req) {
  const headers = req?.headers
  if (!headers || typeof headers !== 'object') return 'unknown'
  for (const [key, raw] of Object.entries(headers)) {
    if (String(key).toLowerCase() !== 'x-laife-client-auth') continue
...
export async function resolveChatMemoryOwnerUserId(req, deps = {}) {
  ...
  const diag = {
    clientAuthHint: readClientAuthHint(req),
    bearerPresent: false,
    jwtVerified: false,
    usersRowEnsured: false,
    ownerPresent: false,
    authCode: null,
    authError: null,
  ...
  try {
    ...
    // JWT may already be verified; bridge/DB failed after that.
    diag.authCode = diag.jwtVerified ? 'owner_bridge_failed' : 'owner_resolve_failed'
    diag.authError = sanitizeMemoryDiagError(error)
