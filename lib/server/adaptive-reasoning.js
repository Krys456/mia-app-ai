...
export function classifyRequest(text, understanding = null) {
  ...
  if (
    intent === 'advice' ||
    intent === 'comparison' ||
    /\b(decid|scelg|trade[\s-]?off|pro\s+e\s+contro|should\s+i|conviene|opzioni)\b/i.test(t)
  ) {
    return 'decision'
  }
  if (
...
export function selectAdaptiveStrategy(input) {
  ...
  const mode = selectReasoningMode(classification, complexity, understanding)
