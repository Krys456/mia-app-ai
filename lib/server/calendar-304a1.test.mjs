...
describe('#304A1 OAuth state + PKCE + scopes', () => {
  ...
  it('#358A — never emits comma-joined hostnames in final redirect', async () => {
    const mod = await import(pathToFileURL(path.join(root, 'lib/server/calendar-oauth.js')).href)
    const assertClean = (result) => {
      assert.equal(result.ok, true, JSON.stringify(result))
      assert.doesNotMatch(result.url, /,https?/i)
      assert.doesNotMatch(result.url, /,http/i)
      assert.doesNotMatch(result.url, /,https/i)
    ...
    // valid Production
    {
      const r = mod.resolveSafeReturnUrl(null, 'https://shinkaido.vercel.app')
      assertClean(r)
      assert.equal(r.url, 'https://shinkaido.vercel.app/?calendar=connected')
    }

    // valid Preview
    {
      const r = mod.resolveSafeReturnUrl(
        null,
        'https://mia-app-ai-git-cursor-ca-b86c6c-cristiansolinas9-3530s-projects.vercel.app',
      )
      assertClean(r)
      assert.match(r.url, /^https:\/\/mia-app-ai-git-cursor-ca-b86c6c/)
    }

    // trailing slash
    {
      const r = mod.resolveSafeReturnUrl(null, 'https://app.example.com/')
      assertClean(r)
      assert.equal(r.url, 'https://app.example.com/?calendar=connected')
    }

    // URL with query in env — origin only for canonical redirect
    {
      const r = mod.resolveSafeReturnUrl(null, 'https://app.example.com/settings?x=1')
      assertClean(r)
      assert.equal(r.url, 'https://app.example.com/?calendar=connected')
    }

    // comma-separated Preview + Production — first wins, never concatenate
    {
      const raw =
        'https://mia-app-ai-git-cursor-ca-b86c6c-cristiansolinas9-3530s-projects.vercel.app,https://shinkaido.vercel.app'
      const parsed = mod.parseCalendarReturnAllowlist(raw)
      assert.deepEqual(parsed, [
        'https://mia-app-ai-git-cursor-ca-b86c6c-cristiansolinas9-3530s-projects.vercel.app',
        'https://shinkaido.vercel.app',
      ])
      const r = mod.resolveSafeReturnUrl(null, raw)
      assertClean(r)
      assert.equal(
        r.url,
        'https://mia-app-ai-git-cursor-ca-b86c6c-cristiansolinas9-3530s-projects.vercel.app/?calendar=connected',
      )
    }

    // "production,https://..." junk prefix
    {
      const r = mod.resolveSafeReturnUrl(null, 'production,https://shinkaido.vercel.app')
      assertClean(r)
      assert.equal(r.url, 'https://shinkaido.vercel.app/?calendar=connected')
    }

    // two full URLs pasted together without comma — reject, no concat hostname
    {
      const glued = 'https://preview.vercel.apphttps://shinkaido.vercel.app'
      assert.deepEqual(mod.parseCalendarReturnAllowlist(glued), [])
      const r = mod.resolveSafeReturnUrl(null, glued)
      assert.equal(r.ok, false)
      assert.equal(r.code, 'return_url_not_configured')
    }

    // empty
    {
      const r = mod.resolveSafeReturnUrl(null, '')
      assert.equal(r.ok, false)
      assert.equal(r.code, 'return_url_not_configured')
    }
    ...
    // invalid protocol
    {
      const r = mod.resolveSafeReturnUrl(null, 'ftp://files.example.com')
      assert.equal(r.ok, false)
      assert.equal(r.code, 'return_url_insecure')
    }

    // same-origin candidate with query preserved when allowlisted
    {
      const r = mod.resolveSafeReturnUrl(
        'https://app.example.com/settings?tab=calendar',
        'https://app.example.com',
      )
      assertClean(r)
      assert.equal(r.url, 'https://app.example.com/settings?tab=calendar')
    }

    // candidate on second allowlisted origin is accepted
    {
      const r = mod.resolveSafeReturnUrl(
        'https://shinkaido.vercel.app/',
        'https://preview.vercel.app,https://shinkaido.vercel.app',
      )
      assertClean(r)
      assert.equal(r.url, 'https://shinkaido.vercel.app/')
    }

    // Edge + JS stay in sync on allowlist helper
    assert.match(sharedOauth, /export function parseCalendarReturnAllowlist/)
    assert.match(sharedOauth, /split\(\/\[[\\]n,\]\+\/\)|split\(\/\[\\n,\]\+\/\)/)
    assert.match(sharedEdge, /parseCalendarReturnAllowlist/)
    assert.doesNotMatch(sharedEdge, /new URL\(returnBase\)/)

    // #358B diag helper stays secret-safe
    const diag = mod.describeReturnRedirectDiag(
      'https://a.vercel.app,https://b.vercel.app',
      'https://a.vercel.app/?calendar=connected',
    )
    assert.equal(diag.sourceEnvName, 'CALENDAR_RETURN_URL')
    assert.equal(diag.candidateCount, 2)
    assert.equal(diag.selectedOrigin, 'https://a.vercel.app')
    assert.equal(diag.containsComma, false)
    assert.equal(diag.containsCommaHttps, false)
    assert.match(sharedOauth, /describeReturnRedirectDiag/)
    assert.match(oauthCallback, /358b_return_redirect|describeReturnRedirectDiag/)
...
describe('#304A1 feature flags — server gate / UI always visible', () => {
  it('CALENDAR_ENABLED defaults false; UI visibility always on', async () => {
    ...
    // Client Settings section is always visible (#304A1) — not gated by VITE_*
    assert.equal(mod.isCalendarUiEnabled({}), true)
    assert.equal(mod.isCalendarUiEnabled({ VITE_CALENDAR_ENABLED: '0' }), true)
  })
})
...
describe('#304A1 Settings + privacy + env', () => {
  it('wires Integrations ON/OFF toggle always visible; server gate documented', () => {
    assert.match(settings, /CalendarIntegrationsSettings/)
    assert.doesNotMatch(settings, /isCalendarUiEnabled/)
    assert.match(calendarSettings, /Integrazioni/)
    assert.match(calendarSettings, /Google Calendar/)
    assert.match(calendarSettings, /resolveCalendarToggleModel|memory-toggle/)
    assert.match(calendarSettings, />\s*ON\s*</)
    assert.match(calendarSettings, />\s*OFF\s*</)
    assert.match(calendarSettings, /startGoogleCalendarOAuth/)
    assert.match(calendarSettings, /disconnectGoogleCalendar/)
    assert.match(calendarSettings, /Sola lettura|Non disponibile/)
    assert.match(calendarSettings, /non usa ancora il Calendar in chat|non legge ancora/)
    assert.match(calendarSettings, /calendar_disabled|Non disponibile|non è attivato/)
    assert.doesNotMatch(calendarSettings, /isCalendarUiEnabled|return null/)
    assert.doesNotMatch(calendarSettings, /localStorage/)
    assert.match(calendarApi, /calendar-oauth-start/)
    assert.match(calendarApi, /calendar-connection/)
    assert.match(calendarApi, /calendar_disabled/)
    assert.doesNotMatch(calendarApi, /isCalendarUiEnabled|calendar_ui_disabled/)
    ...
    assert.match(readme, /always visible|ALWAYS visible/i)
  })
})
