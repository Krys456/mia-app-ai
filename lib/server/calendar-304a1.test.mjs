...
describe('#304A1 OAuth state + PKCE + scopes', () => {
  ...
  it('signs return origin into state and allowlists Preview hosts only', async () => {
    const mod = await import(pathToFileURL(path.join(root, 'lib/server/calendar-oauth.js')).href)
    const key = testKeyHex()
    const preview =
      'https://mia-app-ai-git-cursor-ca-b86c6c-cristiansolinas9-3530s-projects.vercel.app'
    const evil = 'https://evil-cristiansolinas9-3530s-projects.vercel.app'
    const attacker = 'https://mia-app.vercel.app.attacker.com'

    assert.equal(mod.isAllowedCalendarReturnOrigin(preview, 'https://mia-app-ai.vercel.app'), true)
    assert.equal(mod.isAllowedCalendarReturnOrigin(evil, 'https://mia-app-ai.vercel.app'), false)
    assert.equal(mod.isAllowedCalendarReturnOrigin(attacker, 'https://mia-app-ai.vercel.app'), false)
    ...
    assert.equal(verified.returnOrigin, preview)

    const back = mod.resolveOAuthCallbackReturnUrl({
      signedReturnOrigin: verified.returnOrigin,
      allowedBase: 'https://mia-app-ai.vercel.app',
      pathQuery: 'calendar=connected',
    })
    assert.equal(back.ok, true)
    assert.equal(back.url, `${preview}/?calendar=connected`)

    const rejected = mod.resolveOAuthCallbackReturnUrl({
      signedReturnOrigin: evil,
      allowedBase: 'https://mia-app-ai.vercel.app',
    })
    assert.equal(rejected.ok, false)
    assert.equal(rejected.code, 'return_origin_rejected')
  })
})
...
describe('#304A1 Edge Function contracts', () => {
  it('oauth start binds JWT user, PKCE, and never exposes client secret', () => {
    ...
    assert.match(oauthStart, /returnOrigin|isAllowedCalendarReturnOrigin/)
  ...
  it('callback validates state/nonce, encrypts before persistence, safe redirect', () => {
    ...
    assert.match(oauthCallback, /resolveOAuthCallbackReturnUrl|resolveSafeReturnUrl/)
  ...
  it('status/disconnect are owner-scoped and wipe tokens', () => {
    ...
    assert.match(calendarConnection, /action === 'disconnect'|action !== 'disconnect'|action === 'diag'/)
...
describe('#304A1 Settings + privacy + env', () => {
  it('wires Integrations ON/OFF toggle always visible; server gate documented', () => {
    ...
    assert.match(calendarSettings, /sola lettura|impegni/i)
    assert.doesNotMatch(calendarSettings, /non usa ancora il Calendar in chat|non legge ancora/)
    ...
    assert.match(calendarApi, /returnOrigin/)
