...
assert.match(read('src/components/MorningBriefingDeepLinkHost.tsx'), /readMorningBriefingDurableIntent/)
...
// #384E — Morning Briefing Web Push urgency high; TTL / payload / claim unchanged
{
  const edge = read('supabase/functions/morning-briefing-dispatch/index.ts')
  assert.match(edge, /pushTextMessage\([\s\S]*?urgency:\s*['"]high['"]/)
  assert.match(edge, /ttl:\s*60\s*\*\s*60/)
  assert.doesNotMatch(edge, /urgency:\s*['"]normal['"]/)
  // Payload / click URL unchanged
  assert.match(edge, /type:\s*['"]morning_briefing['"]/)
  assert.match(edge, /url:\s*['"]\/\?briefing=morning['"]/)
  assert.match(edge, /tag:\s*`morning-briefing:\$\{localDate\}`/)
  assert.match(edge, /Il tuo briefing mattutino è pronto/)
  // Claim / finalize / clear semantics unchanged
  assert.match(edge, /claim_due_morning_briefings/)
  assert.match(edge, /finalize_morning_briefing_delivery/)
  assert.match(edge, /clear_morning_briefing_delivery_claim/)
  assert.match(edge, /last_delivered_local_date|p_local_date/)
  // Reminder push domain untouched by this Edge file
  assert.doesNotMatch(edge, /claim_due_reminders|push_sent_at/)
  // Reminder Push remains high in its own dispatcher
  const reminderEdge = read('supabase/functions/reminder-push-dispatch/index.ts')
  assert.match(reminderEdge, /urgency:\s*['"]high['"]/)
  assert.doesNotMatch(reminderEdge, /urgency:\s*['"]normal['"]/)
}
