...
const GENERIC_ASSISTANT =
  /\b(come\s+posso\s+aiutarti|how\s+can\s+i\s+help|i'?m\s+here\s+to\s+help|sono\s+qui\s+per\s+aiutarti|dimmi\s+pure|tell\s+me\.?|any\s+questions|hai\s+domande|fammi\s+sapere|let\s+me\s+know|non\s+esitare|feel\s+free\s+to\s+ask|in\s+cosa\s+posso\s+aiutarti)\b/i
...
function buildBrief(mode, opts) {
  const lines = [
    ...
    'Checklist interna (sì/no, non stamparla): (1) genuinamente impegnato? (2) reagisco al significato, non solo alle parole? (3) continuo un pensiero condiviso invece di ripartire? (4) questa domanda è utile o solo facile? (5) la risposta è più calda / naturale / interessante?',
    `Preferisci: ${PREFER.join(', ')}.`,
    `Evita: ${AVOID.join(', ')}.`,
  ]
