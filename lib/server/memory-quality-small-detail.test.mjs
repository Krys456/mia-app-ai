...
describe('memory quality small-detail (#280)', () => {
  ...
  it('B2 — IT negozio Etsy si chiama → WRITE (exact Preview failure)', () => {
    const text = 'Il mio negozio Etsy si chiama TemplateNestKrys.'
    const facts = extractDurableFacts(text)
    assert.equal(facts.length, 1, 'WRITE_FAIL: IT Etsy shop must extract')
    assert.equal(facts[0].category, 'projects')
    assert.match(facts[0].content, /User's etsy shop:\s*TemplateNestKrys/i)
    assert.doesNotMatch(facts[0].content, /si chiama/i)
    assert.equal(facts[0].factKey, 'projects.shop.etsy.templatenestkrys')
    assert.equal(facts[0].importance, 7)
    assert.equal(facts[0].confidence, 0.8)
  })

  it('B3 — IT shop Etsy / generic negozio variants (WRITE)', () => {
    const a = extractDurableFacts('Il mio shop Etsy si chiama TemplateNestKrys.')[0]
    const b = extractDurableFacts('Il negozio Etsy si chiama TemplateNestKrys.')[0]
    const c = extractDurableFacts('Il mio negozio si chiama TemplateNestKrys.')[0]
    assert.equal(a.factKey, 'projects.shop.etsy.templatenestkrys')
    assert.equal(b.factKey, 'projects.shop.etsy.templatenestkrys')
    assert.equal(c.factKey, 'projects.shop.templatenestkrys')
    assert.match(c.content, /User's shop:\s*TemplateNestKrys/i)
  })

  it('B4 — IT New Chat recall retrieves Etsy shop (RECALL)', () => {
    const fact = extractDurableFacts('Il mio negozio Etsy si chiama TemplateNestKrys.')[0]
    const row = rowFromFact(fact)
    assert.equal(isRecallEligibleMemory(row), true)
    for (const q of [
      'Come si chiama il mio negozio Etsy?',
      'Qual è il nome del mio negozio Etsy?',
      'Come si chiamava il mio shop?',
      ...
      const scored = scoreMemoryRelevance(row, q, topic)
      assert.equal(scored.matched, true, `RECALL_FAIL for: ${q}`)
      assert.ok(scored.score >= 6)
      const ranked = rerankMemoriesForRecall([row], q, {
        limit: RECALL_MAX_MEMORIES,
        intent,
      })
      const pack = formatCoreMemoryPack(ranked, q)
      assert.match(pack, /TemplateNestKrys/i, `pack miss for: ${q}`)
    }
  })
