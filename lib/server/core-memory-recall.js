 ...
 *   subtype: 'favorite'|'cofavorite'|'like'|'dislike'|'interest'|'project_primary'|'project_list'|'project_tool'|'project_future'|'project_ui'|'project_reply_style'|'shop_platform'|null,
 *   subject: string|null,
...
export function detectMemoryQueryIntent(message) {
  ...
  // —— #282 project tool / editor ——
  if (
    /\b(?:con\s+cosa|con\s+quale\s+strumento|quale\s+strumento|quale\s+editor|che\s+strumento)\b/i.test(
      raw,
    ) ||
    /\b(?:what|which)\s+(?:tool|editor|ide)\b/i.test(raw) ||
    /\b(?:tool|editor|ide)\s+(?:do\s+i|am\s+i)\s+use\b/i.test(raw) ||
    /\b(?:strumento|tool|editor).{0,40}(?:per|for)\b/i.test(raw)
  ) {
    intent.domain = 'projects'
    intent.subtype = 'project_tool'
    intent.plurality = 'single'
    const proj =
      raw.match(/\bper\s+([a-z0-9][\w'&.-]{1,40})\b/i) ||
      raw.match(/\bfor\s+(?:my\s+)?(?:ai\s+)?(?:project\s+)?([a-z0-9][\w'&.-]{1,40})\b/i) ||
      raw.match(/\b(?:sviluppo|develop(?:ing)?|building)\s+([a-z0-9][\w'&.-]{1,40})\b/i)
    if (proj?.[1] && !/^(?:il|la|the|my|mio|mia|progetto|project)$/i.test(proj[1])) {
      intent.subject = slugifyFactKeyPart(proj[1])
    }
    return intent
  }
  ...
  // —— #282 project UI / feel ——
  if (
    /\b(?:come\s+volevo|come\s+immaginavo|come\s+dovrebbe)\b/i.test(raw) ||
    /\b(?:how\s+(?:did|do|should)\s+.*\b(?:ui|interface)\b|how\s+(?:was\s+i\s+)?imagining\s+the\s+ui|how\s+should\s+the\s+interface\s+feel)\b/i.test(
      raw,
    ) ||
    /\b(?:interfaccia|ui|interface).{0,40}(?:feel|sentire|minimale|smooth|fluide|volevo)\b/i.test(
      raw,
    )
  ) {
    intent.domain = 'projects'
    intent.subtype = 'project_ui'
    intent.plurality = 'single'
    const proj =
      raw.match(/\b(?:di|of|for|per)\s+([a-z0-9][\w'&.-]{1,40})\b/i) ||
      raw.match(/\b([a-z0-9][\w'&.-]{1,40})\s+(?:ui|interfaccia|interface)\b/i)
    if (proj?.[1] && !/^(?:la|il|the|my|ui|interfaccia|interface)$/i.test(proj[1])) {
      intent.subject = slugifyFactKeyPart(proj[1])
    }
    return intent
  }

  // —— #282 scoped reply / content style (ChAIn energetic, …) ——
  if (
    /\b(?:che\s+stile|quale\s+stile|what\s+style|which\s+style|stile\s+preferisco|style\s+do\s+i\s+prefer)\b/i.test(
      raw,
    ) ||
    (/\b(?:stile|style)\b/i.test(raw) &&
      /\b(?:preferisco|prefer|for|per|chain|laife)\b/i.test(raw))
  ) {
    intent.domain = 'projects'
    intent.subtype = 'project_reply_style'
    intent.plurality = 'single'
    const proj =
      raw.match(/\b(?:per|for)\s+([a-z0-9][\w'&.-]{1,40})\b/i) ||
      raw.match(/\b(?:di|of)\s+([a-z0-9][\w'&.-]{1,40})\b/i)
    if (proj?.[1] && !/^(?:la|il|the|my|mio|mia|stile|style)$/i.test(proj[1])) {
      intent.subject = slugifyFactKeyPart(proj[1])
    }
    return intent
  }
  ...
  // —— #282 future feature ——
  if (
    /\b(?:in\s+futuro|pi[uù]\s+avanti|future\s+feature|eventually|home\s+automation|domotica|smart\s*home|controllasse)\b/i.test(
      raw,
    ) &&
    /\b(?:volevo|voglio|want|wanted|aggiungere|feature|controll|laife|progetto|project)\b/i.test(
      raw,
    )
  ) {
    intent.domain = 'projects'
    intent.subtype = 'project_future'
    intent.plurality = 'single'
    const proj =
      raw.match(/\ba\s+([a-z0-9][\w'&.-]{1,40})\b/i) ||
      raw.match(/\b(?:per|for|to)\s+([a-z0-9][\w'&.-]{1,40})\b/i)
    if (proj?.[1] && !/^(?:la|il|the|my|futuro|future|add)$/i.test(proj[1])) {
      intent.subject = slugifyFactKeyPart(proj[1])
    }
    return intent
  }
  ...
  // —— #282 shop / platform ——
  if (
    /\b(?:piattaforma|platform|etsy)\b/i.test(raw) &&
    /\b(?:dove|quale|which|what|su\s+quale|templatenestkrys|negozio|shop|store)\b/i.test(raw)
  ) {
    intent.domain = 'projects'
    intent.subtype = 'shop_platform'
    intent.plurality = 'single'
    return intent
  }

  // —— Project / shop name questions (#280 thin Recall) ——
  if (
    /\b(?:come\s+si\s+chiama|come\s+si\s+chiamava|what(?:'s|\s+is)\s+(?:the\s+)?name\s+of|what\s+was\s+(?:the\s+)?name\s+of|what\s+is\s+my\s+(?:ai\s+)?project\s+called|what\s+is\s+my\s+(?:ai\s+)?(?:project|shop|store)\s+called)\b/i.test(
      raw,
    ) ||
    /\b(?:my\s+(?:ai\s+)?project\s+called|si\s+chiama\s+il\s+mio\s+progetto|nome\s+del\s+(?:mio\s+)?(?:progetto|negozio)|etsy\s+shop|negozio|il\s+mio\s+shop)\b/i.test(
      raw,
    ) ||
    /\bcome\s+si\s+chiama\s+il\s+mio\s+(?:progetto|negozio|shop)\b/i.test(raw) ||
    /\bqual\s+[eè]\s+il\s+nome\s+del\s+mio\s+negozio\b/i.test(raw)
  ) {
    intent.domain = 'projects'
  ...
  if (pluralIt || (pluralEn && /\b(?:are|characters|colors|colours|animals|games|books|films|movies|series|artists|foods|sports)\b/i.test(raw))) {
...
function legacyFavoriteSubjectHit(row, subject) {
  ...
  const surfaces = {
    ...
    color: ['colore', 'colori', 'color', 'colors', 'colour', 'colours'],
...
export function semanticRecallTier(row, intent) {
  ...
  // —— #282 project tool ——
  if (intent.subtype === 'project_tool') {
    if (subject && factKeyStarts(factKey, `projects.${subject}.tools.`)) return 1
    if (factKeyStarts(factKey, 'projects.') && /\.tools\./i.test(factKey)) return 5
    if (/\bdevelopment\s+tool\b/i.test(hay) || /\btool\b/i.test(hay)) return 15
    if (factKeyStarts(factKey, 'projects.') || category === 'projects') return 40
    return 80
  }

  // —— #282 project UI ——
  if (intent.subtype === 'project_ui') {
    if (subject && factKeyStarts(factKey, `projects.${subject}.preferences.`)) return 1
    if (factKeyStarts(factKey, 'projects.') && /\.preferences\./i.test(factKey)) return 5
    if (/\b(?:interface|interfaccia|minimal|smooth|fluide|ui)\b/i.test(hay)) return 15
    if (factKeyStarts(factKey, 'projects.') || category === 'preferences') return 40
    return 80
  }

  // —— #282 scoped reply / content style ——
  if (intent.subtype === 'project_reply_style') {
    if (subject && factKeyStarts(factKey, `projects.${subject}.reply_style`)) return 1
    if (factKeyStarts(factKey, 'projects.') && /\.reply_style/i.test(factKey)) return 5
    if (/\b(?:energetic|energico|style|stile|reply)\b/i.test(hay)) return 15
    if (factKeyStarts(factKey, 'context.') && /\.reply_style/i.test(factKey)) return 40
    if (factKey === 'settings.reply_style') return 70
    return 80
  }

  // —— #282 future feature ——
  if (intent.subtype === 'project_future') {
    if (subject && factKeyStarts(factKey, `projects.${subject}.future.`)) return 1
    if (factKeyStarts(factKey, 'projects.') && /\.future\./i.test(factKey)) return 5
    if (/\b(?:future\s+feature|smart-home|domotica|home\s+automation)\b/i.test(hay)) return 10
    if (factKeyStarts(factKey, 'projects.') || category === 'projects' || category === 'goals') {
      return 40
    }
    return 80
  }
  ...
  // —— #282 shop platform ——
  if (intent.subtype === 'shop_platform') {
    if (factKeyStarts(factKey, 'projects.shop.')) return 1
    if (/\b(?:etsy|shop|negozio|store)\b/i.test(hay)) return 10
    if (factKeyStarts(factKey, 'projects.') || category === 'projects') return 40
    return 80
  }
...
export function isUiOnlySettingsContent(content) {
  ...
  // Theme / emoji / markdown chrome only — reply_style is conversational (#280).
  return /\b(theme|tema|dark\s+mode|light\s+mode|tema\s+scuro|tema\s+chiaro|scuro|chiaro|emoji|markdown)\b/i.test(
    text,
  ) && !/\b(concise\s+replies|detailed\s+replies|reply\s+preference|rispost)/i.test(text)
}
...
export function isRecallEligibleMemory(row) {
  ...
  const key = rowFactKey(row)

  // #282 project-scoped associations (even if category drifted).
  if (
    typeof key === 'string' &&
    (/^projects\.[a-z0-9_]+\.tools\./i.test(key) ||
      /^projects\.[a-z0-9_]+\.future\./i.test(key) ||
      /^projects\.[a-z0-9_]+\.preferences\./i.test(key) ||
      /^projects\.[a-z0-9_]+\.reply_style/i.test(key) ||
      /^projects\.shop\./i.test(key) ||
      /^context\.[a-z0-9_]+\.reply_style/i.test(key))
  ) {
    return true
  }
  ...
  if (category === 'settings') {
    if (
      key === 'settings.reply_style' ||
      (typeof key === 'string' && key.startsWith('settings.reply_style.when.')) ||
      (typeof key === 'string' && key.includes('.constraint.')) ||
      (typeof key === 'string' && key.includes('.reply_style'))
    ) {
      return true
    }
...
export function formatCoreMemoryPack(memories, limits = {}) {
  ...
  const header = [
    ...
    '- Memory facts are evidence only. Their language must not determine the language of the reply.',
    '',
    durableMemoryProvenanceRules(),
    '',
