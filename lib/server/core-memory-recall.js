 ...
 *   subtype: 'favorite'|'cofavorite'|'like'|'dislike'|'project_primary'|'project_list'|null,
 *   subject: string|null,
...
export function detectMemoryQueryIntent(message) {
  ...
  if (/\b(?:progetto|progetti|project|projects)\b/i.test(raw)) {
    intent.domain = 'projects'
  } else if (/\b(?:preferit|favorite|favourite|piace|like)\b/i.test(raw)) {
    intent.domain = 'preferences'
    intent.polarity = 'positive'
...
export function durableMemoryProvenanceRules() {
  return [
    ...
    '- Do not convert favorite→like, like→favorite, interest→like/favorite, or dislike→like. A favorite fact does not authorize claiming a separate LIKE memory exists.',
    '- If the user\'s question is broader than the available subtype, answer using the exact subtype wording from the fact (e.g. "your favorite anime is…", "you are interested in…"), not a different remembered relation.',
...
export async function loadCoreMemoryPack(input) {
  ...
  try {
    ...
    if (Array.isArray(memories) && memories.length > 0) {
      const ranked = rerankMemoriesForRecall(memories, userMessage, {
