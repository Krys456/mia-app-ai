...
export function detectMemoryQueryIntent(message) {
  ...
  if (
    /\b(?:come\s+si\s+chiama|what(?:'s|\s+is)\s+(?:the\s+)?name\s+of|what\s+was\s+(?:the\s+)?name\s+of|what\s+is\s+my\s+(?:ai\s+)?project\s+called|what\s+is\s+my\s+(?:ai\s+)?(?:project|shop|store)\s+called)\b/i.test(
      raw,
    ) ||
    /\b(?:my\s+(?:ai\s+)?project\s+called|si\s+chiama\s+il\s+mio\s+progetto|nome\s+del\s+(?:mio\s+)?progetto|etsy\s+shop|negozio)\b/i.test(
      raw,
    ) ||
    /\bcome\s+si\s+chiama\s+il\s+mio\s+progetto\b/i.test(raw)
  ) {
    intent.domain = 'projects'
