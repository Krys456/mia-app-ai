...
const EXPLICIT_TOPIC_RE =
  /^(parliamo\s+(di|dei|delle|del|della|degli)\s+|let'?s\s+talk\s+about\s+|vorrei\s+parlare\s+(di|dei|delle|del)\s+)/i
...
function inferLookingFor(args) {
  ...
  if (REJECTION_RE.test(msg)) {
    signals.push('reject_direction')
    return { lookingFor: 'companionship', signals }
  }
  if (GREETING_RE.test(msg) || HOW_ARE_YOU_RE.test(msg)) {
    signals.push(GREETING_RE.test(msg) ? 'greeting' : 'how_are_you')
    return { lookingFor: 'companionship', signals }
  }
  if (EXPLICIT_TOPIC_RE.test(msg)) {
    signals.push('explicit_user_topic')
...
function pickDesiredFeeling(lookingFor, msg) {
  ...
  if (REJECTION_RE.test(msg)) return 'relaxed'
  if (GREETING_RE.test(msg) || HOW_ARE_YOU_RE.test(msg)) return 'relaxed'
...
function pickStrategy(args) {
  ...
  if (REJECTION_RE.test(msg)) {
    // Acknowledge drop of prior direction — do not deepen or spark
...
function pickDepth(args) {
  ...
  if (GREETING_RE.test(msg) || HOW_ARE_YOU_RE.test(msg) || REJECTION_RE.test(msg) || PLAY_RE.test(msg))
    return 1
...
function pickTopicAction(args) {
  const { lookingFor, msg, opportunity, intent } = args
  if (GREETING_RE.test(msg) || HOW_ARE_YOU_RE.test(msg)) {
    // Presence check-ins: wait / don't invent a topic to hold
    return 'wait'
  }
  if (REJECTION_RE.test(msg)) {
    // Drop assistant-proposed direction — do not expand it
    return 'wait'
  }
  if (EXPLICIT_TOPIC_RE.test(msg)) {
    return 'stay'
...
function buildGoal(plan, lang, msg) {
  ...
  if (REJECTION_RE.test(msg)) {
    return lang === 'en'
      ? 'Acknowledge the rejection of the prior direction — do not deepen it or force a new curiosity topic.'
      : 'Accetta il rifiuto della direzione precedente — non approfondirla né forzare una nuova curiosità.'
  }
  if (EXPLICIT_TOPIC_RE.test(msg)) {
    return lang === 'en'
      ? 'Engage the user’s chosen topic — they own the direction.'
      : 'Entra nel tema scelto dall’utente — la direzione è loro.'
  }
  if (BORED_RE.test(msg)) {
