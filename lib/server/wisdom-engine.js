...
function evaluateWisdom(args) {
  ...
  } else if (EXPLICIT_DEPTH.test(text) || SHOW_OFF_RISK.test(text)) {
    informationAmount = 'right' // they asked for depth — still stay elegant, not showy
  ...
  if (informationAmount === 'too_much' && (preferBrevity || SHORT_REACT.test(text) || GOODBYE.test(text))) {
    stance = 'hold_back'
  ...
  if (dtDir === 'elegant_explanation' && stance !== 'hold_back') stance = 'mentor_simple'
