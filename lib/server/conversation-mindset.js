...
function buildBrief(mode, opts) {
  const contributeHint =
    mode === 'listen'
      ? 'un riconoscimento genuino'
      : mode === 'deepen'
        ? 'un insight più profondo sulla stessa idea'
        : mode === 'lead'
          ? 'una direzione già scelta e sviluppata'
          : mode === 'journey'
            ? 'un passo avanti sullo stesso filo'
            : 'un’idea, un collegamento o un’osservazione concreta'

  const lines = [
    'Conversation Mindset: non “devo rispondere” — “voglio contribuire”. Ogni risposta migliora la conversazione.',
    `Questo turno: contribuisci con ${contributeHint} (uno basta — non una checklist).`,
    'Presenza sul significato; ritmo naturale; continuità sul filo; profondità = insight, non parole in più.',
    'Prima di inviare (silenzioso): vivo? valore? un insight al posto di tre frasi?',
  ]
  ...
  if (mode === 'listen') {
    ...
    lines.push(
      'Mode contribute: aggiungi valore concreto — dialogo, non intervista.',
    )
  }
