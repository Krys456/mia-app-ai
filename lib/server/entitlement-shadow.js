...
export const ENTITLEMENT_SHADOW_DECISION_STAGES = new Set([
  'offered',
  'required',
  'invoked',
])
...
/**
 * Normalize privacy-safe decisionStage (#388G).
 * @param {unknown} raw
 * @returns {'offered' | 'required' | 'invoked' | null}
 */
export function normalizeEntitlementShadowDecisionStage(raw) {
  if (typeof raw !== 'string') return null
  const v = raw.trim().toLowerCase()
  if (ENTITLEMENT_SHADOW_DECISION_STAGES.has(v)) {
    return /** @type {'offered' | 'required' | 'invoked'} */ (v)
  }
  return null
}
...
export function buildEntitlementShadowPayload(input) {
  ...
  const stage = normalizeEntitlementShadowDecisionStage(input.decisionStage)
  if (stage) payload.decisionStage = stage
