 ...
 *
 * Build Ideas, Don't Reset — when the user shows enthusiasm
 * ("Interesting.", "Cool.", "Wow.", "That's awesome.", "I like this."):
 *   - Continue developing the SAME idea
 *   - Do not restart with a new explanation
 *   - Do not immediately ask another question
 *   - Treat enthusiasm as permission to go one layer deeper
 *   - Feel like building a shared train of thought
 *
...
const EXPLICIT_COMPLIMENT =
  /\b(ottim[oa]|brav[oa]|eccezionale|fantastico|fantastic|brilliant|brilliantly|insightful|illuminat\w*|chiarissim\w*|super\s+chiaro|molto\s+utile|really\s+helpful|so\s+helpful|very\s+helpful|great\s+(answer|explanation|response|point)|amazing|impressive|love\s+(this|that|it)|i\s+like\s+(this|that|it)|mi\s+piace(\s+(molto\s+)?(questo|quello|cos[iì]|la\s+spiegazione))?|che\s+(bello|figo|forte)|ben\s+detto|spot\s+on|nailed\s+it|perfect\s+explanation|splendid[oa]?|meraviglios\w*|interessantissim\w*|this\s+is\s+(great|excellent|awesome|brilliant|gold|cool|interesting)|that'?s\s+(great|excellent|awesome|brilliant|helpful|clear|perfect|cool|interesting|amazing)|that\s+(was|is)\s+(great|excellent|awesome|brilliant|helpful|clear|perfect)|wow|bellissima\s+spiegazione|spiegazione\s+(ottima|chiara|perfetta)|risposta\s+(ottima|perfetta|utilissima))\b/i

/** Short praise / enthusiasm tokens that alone count as compliments when substance preceded. */
const SHORT_PRAISE =
  /^(nice|cool|great|awesome|fantastic|brilliant|interesting|wow|love\s+it|i\s+like\s+(this|that|it)|interessante|bell[oa]|figo|forte|top|fire|legend|goat|💯|🔥)[\s!.]*$/i

/** Pure ack without clear enthusiasm (still may continue learning, but not "go deeper" reward). */
const PURE_ACK =
  /^(ok|okay|k|va\s+bene|bene|perfetto|capito|capisco|ho\s+capito|i\s+understand|understood|yes|yep|yeah|yup|si|sì|alright|i\s+see|vedo|ah|oh|mm+|uhm+|got\s+it|makes\s+sense|chiaro|esatto|giusto|fair|sure|right)[\s!.]*$/i

/** Soft enthusiasm that may appear mid-phrase. */
const ENTHUSIASM_SIGNAL =
  /\b(interesting|cool|wow|awesome|amazing|nice|i\s+like\s+(this|that|it)|love\s+(this|that|it)|interessante|figo|forte|bell[oa]|that'?s\s+(awesome|cool|amazing|great|interesting))\b/i
...
/**
 * Detect an explicit compliment / enthusiasm about the prior response.
 * @param {string} text
 */
export function isExplicitCompliment(text) {
  ...
  // Short enthusiasm mid-phrase without a new ask
  if (t.length <= 48 && ENTHUSIASM_SIGNAL.test(t) && !/[?]/.test(t)) return true
...
/**
 * Enthusiasm ≈ permission to go one layer deeper on the same idea.
 * @param {string} text
 */
export function isEnthusiasmSignal(text) {
  const t = normalize(text)
  if (!t || t.length > COMPLIMENT_MAX_LEN) return false
  if (SHORT_PRAISE.test(t)) return true
  if (ENTHUSIASM_SIGNAL.test(t) && t.length <= 64 && !/[?]/.test(t)) return true
  return isExplicitCompliment(t)
}
...
function buildComplimentBrief(topic, style, alreadyExplained) {
  ...
  return [
    `BUILD IDEAS, DON'T RESET — l’utente ha mostrato entusiasmo/complimento sul filo “${topic}”.`,
    'Entusiasmo = permesso di scendere UNO strato più a fondo sulla STESSA idea.',
    'Continua a costruire lo stesso treno di pensiero — non ripartire da zero con una nuova spiegazione.',
    `Sviluppa: ${describeStyle(style)}.`,
    'Niente ack rituale (“bello!”, “grazie!”) — entra subito nello strato successivo, oppure una mezza frase di reazione genuina max.',
    'NON fare subito un’altra domanda. NON resettare. NON solo cortesia.',
    avoid,
    'Vietato filler, ricapitolazioni, menu di scelte, e chiudere solo con cortesia.',
  ].join(' ')
}
...
function describeStyle(style) {
  switch (style) {
    case 'practical':
      return 'un next step concreto e applicabile (senza annunciare “consiglio pratico”)'
    case 'advanced':
      return 'uno strato appena più profondo, ancora chiaro (senza annunciare “spiegazione avanzata”)'
    case 'example':
      return 'uno scenario reale / mini-caso (raccontato, non etichettato)'
    case 'comparison':
      return 'un confronto breve con un’alternativa correlata'
    case 'misconception':
      return 'un malinteso frequente corretto in una frase netta'
    case 'historical':
      return 'un contesto storico breve e pertinente (dettaglio, non lezione)'
    case 'scientific':
      return 'un insight meccanico/scientifico utile, detto semplice'
    case 'best_practice':
      return 'una best practice concreta (non una lista lunga)'
    case 'next_topic':
      return 'il passo successivo naturale sul filo (una sola porta)'
    default:
...
export function analyzeContinuation(input) {
  ...
  if (complimentTurn) {
    ...
    // Prefer depth / misconception / scientific / example — same idea, one layer deeper.
    // Avoid next_topic (feels like a reset) and practical-first restarts.
    ...
    return {
      ...
      reasons: [
        ...reasons,
        'build_ideas_dont_reset',
        'Entusiasmo/complimento = permesso di andare uno strato più a fondo sulla stessa idea.',
        `style=${style}`,
...
export function formatContinuationForWriter(plan) {
  ...
  const action = plan.shouldContinue
    ? plan.intent === 'compliment_go_deeper'
      ? `AZIONE — BUILD IDEAS, DON'T RESET: sviluppa la STESSA idea uno strato più a fondo (${plan.continuationStyle || 'advanced'}); non ripartire; non fare subito una domanda.`
      : `AZIONE: una sola continuazione significativa (${plan.continuationStyle || 'valore'}).`
    : 'AZIONE: risposta breve — non forzare, non filler.'

  return `══════════════════════════════════════
CONVERSATION CONTINUATION · BUILD IDEAS (INVISIBILE)
══════════════════════════════════════
...
- Entusiasmo/complimento ("Interesting.", "Cool.", "Wow.", "That's awesome.", "I like this."): BUILD IDEAS, DON'T RESET — stessa idea, uno strato più a fondo; non ripartire; non chiedere subito
- Se c’è un complimento: NON solo “grazie”
