/**
 * Shared JSON + CORS helpers for Vercel Node handlers (#298A CORS allowlist).
 *
...
export function sendJson(res, status, payload, req) {
  if (req) applyCors(res, req)
  res.setHeader('Content-Type', 'application/json; charset=utf-8')
  const safe = sanitizeJsonPayload(payload && typeof payload === 'object' ? payload : {})
  try {
    const keys = Object.keys(safe)
    const contentLen = typeof safe.content === 'string' ? safe.content.length : 0
    console.log(
      '[api:sendJson]',
      JSON.stringify({
        status,
