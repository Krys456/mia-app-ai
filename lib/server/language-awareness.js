...
const EXPLICIT_IT =
  /\b(parla\s+italiano|rispondi(?:mi)?\s+in\s+italiano|in\s+italiano\s+per\s+favore|passiamo\s+all['’]?italiano|torniamo\s+(?:all['’])?italiano|continua\s+in\s+italiano|puoi\s+(?:parlare|rispondere)\s+in\s+italiano|italiano\s+per\s+favore|let'?s\s+(?:speak|continue)\s+in\s+italian|speak\s+(?:to\s+me\s+)?in\s+italian|answer(?:\s+me)?\s+in\s+italian|r[eé]ponds(?:[- ]moi)?\s+en\s+italien)\b/i
