...
export function detectLanguageSignal(text) {
  ...
  if (
    /\b(come\s+stai|cosa\s+significa|qual\s+['’]?è|qual\s+e|mi\s+piace|a\s+me\s+piace|mi\s+piacciono|i\s+miei|le\s+mie|voglio\s+imparare|progetto\s+principale|parliamo\s+di|preferit[oi]|preferit[ae]|personaggi\s+preferit)/i.test(
      t,
    )
  ) {
    scores.it += 6
