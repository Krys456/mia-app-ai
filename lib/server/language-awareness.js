...
const EXPLICIT_EN =
  /\b(speak\s+(?:to\s+me\s+)?in\s+english|answer\s+in\s+english|reply\s+in\s+english|in\s+english\s+please|switch\s+to\s+english|can\s+you\s+(?:speak|answer|reply)\s+in\s+english|english\s+please|let'?s\s+(?:speak|continue|talk)\s+in\s+english|now\s+let'?s\s+continue\s+in\s+english)\b/i
...
const IT_MARKERS =
  /\b(che|come|sono|perché|perche|qual|quale|quali|voglio|vorrei|mio|mia|miei|mie|non|con|una|degli|delle|questo|questa|questi|queste|quello|quella|anche|molto|più|piu|meno|dove|quando|dopo|prima|sempre|mai|oggi|ieri|domani|prego|ciao|salve|buongiorno|buonasera|perfetto|volentieri|allora|quindi|però|pero|così|cosi|cosa|chi|ecco|boh|già|gia|magari|davvero|forse|adesso|ancora|bene|male|qui|lì|li|suo|sua|loro|noi|voi|tu|io|mi|ti|ci|vi|gli|del|della|dei|nel|nella|sul|sulla|dai|dalla|tra|sono|sei|siamo|avete|hanno|vorrei|posso|puoi|dobbiamo|dimmi|parliamo|significa|errore|progetto|preferito|preferita|ricordi|dimentica|dimenticare)\b/gi

const EN_MARKERS =
  /\b(the|what|how|why|should|would|could|my|is|are|was|were|with|this|that|these|those|please|hello|hey|hi|not|and|but|or|for|from|have|has|had|do|does|did|can|will|just|really|maybe|today|tomorrow|yesterday|because|about|into|over|under|again|also|very|more|most|some|any|your|you|me|we|they|them|our|his|her|its|a|an|of|to|in|on|at|by|as|if|so|then|than|when|where|who|which|there|here|been|being|be|remember|project|favorite|favourite|main|continue|let'?s|mean|error|forget|overview)\b/gi
...
const FR_MARKERS =
  /\b(que|qui|quoi|comment|pourquoi|suis|es|est|sommes|êtes|etes|sont|veux|voulez|parce|aussi|beaucoup|plus|moins|où|ou|quand|après|apres|avant|toujours|jamais|aujourd'?hui|hier|demain|bonjour|bonsoir|salut|parfait|alors|donc|mais|ainsi|ici|là|la|mon|ma|mes|ton|ta|tes|son|sa|ses|notre|votre|leur|nous|vous|ils|elles|les|des|du|au|aux|une|je|tu|il|elle|me|te|se|lui|y|en|avec|dans|pour|sur|pas|c'?est|ça|ca|projet|préféré|prefere|souvenir|oublie|oublier|signifie|erreur)\b/gi
...
export function detectLanguageSignal(text) {
  ...
  // Strong Italian phrases
  if (/\b(come\s+stai|cosa\s+significa|qual\s+['’]?è|qual\s+e|mi\s+piace|progetto\s+principale|parliamo\s+di|preferito)\b/i.test(t)) {
    scores.it += 6
  }
  // Strong English
  ...
  // Strong French
  if (/\b(comment\s+ça\s+va|comment\s+ca\s+va|qu['’]?est[- ]ce|mon\s+projet|je\s+suis)\b/i.test(t)) {
    scores.fr += 6
  }
  // Strong German
