...
const EXPLICIT_EN =
  /\b(speak\s+(?:to\s+me\s+)?in\s+english|answer(?:\s+me)?\s+in\s+english|now\s+answer(?:\s+me)?\s+in\s+english|reply\s+in\s+english|in\s+english\s+please|switch\s+to\s+english|can\s+you\s+(?:speak|answer|reply)\s+in\s+english|english\s+please|let'?s\s+(?:speak|continue|talk)\s+in\s+english|now\s+let'?s\s+continue\s+in\s+english|rispondi(?:mi)?\s+in\s+inglese|r[eé]ponds(?:[- ]moi)?\s+en\s+anglais)\b/i

const EXPLICIT_IT =
  /\b(parla\s+italiano|rispondi(?:mi)?\s+in\s+italiano|in\s+italiano\s+per\s+favore|passiamo\s+all['’]?italiano|torniamo\s+(?:(?:all['’])?italiano|a\s+(?:parlare\s+)?in\s+italiano)|parliamo\s+(?:di\s+nuovo\s+)?in\s+italiano|continua\s+in\s+italiano|puoi\s+(?:parlare|rispondere)\s+in\s+italiano|italiano\s+per\s+favore|let'?s\s+(?:speak|continue)\s+in\s+italian|speak\s+(?:to\s+me\s+)?in\s+italian|answer(?:\s+me)?\s+in\s+italian|r[eé]ponds(?:[- ]moi)?\s+en\s+italien)\b/i

const EXPLICIT_ES =
  /\b(habla\s+espa[nñ]ol|responde\s+en\s+espa[nñ]ol|en\s+espa[nñ]ol\s+por\s+favor|switch\s+to\s+spanish|speak\s+(?:to\s+me\s+)?in\s+spanish|answer\s+in\s+spanish|espa[nñ]ol\s+por\s+favor|rispondi(?:mi)?\s+in\s+spagnolo)\b/i

const EXPLICIT_FR =
  /\b(parle\s+fran[cç]ais|r[eé]ponds(?:[- ]moi)?\s+en\s+fran[cç]ais|en\s+fran[cç]ais\s+s['’]?il\s+te\s+pla[iî]t|speak\s+(?:to\s+me\s+)?in\s+french|answer(?:\s+me)?\s+in\s+french|switch\s+to\s+french|rispondi(?:mi)?\s+in\s+francese)\b/i

const EXPLICIT_DE =
  /\b(sprich\s+deutsch|antworte\s+auf\s+deutsch|auf\s+deutsch\s+bitte|speak\s+(?:to\s+me\s+)?in\s+german|answer\s+in\s+german|switch\s+to\s+german|rispondi(?:mi)?\s+in\s+tedesco)\b/i
...
// Avoid Romance-shared tokens that collide hard with Italian (il/la/me/sa/tu/lui/ma).
// `lui` and `ma` are contextual-only (see scoreRomanceSharedTokens).
const FR_MARKERS =
  /\b(que|qui|quoi|comment|pourquoi|suis|es|est|sommes|êtes|etes|sont|veux|voulez|voudrais|parce|aussi|beaucoup|très|tres|plus|moins|où|ou|quand|après|apres|avant|toujours|jamais|aujourd'?hui|hier|demain|bonjour|bonsoir|salut|parfait|merci|alors|donc|mais|ainsi|ici|là|mon|mes|ton|ta|tes|son|ses|notre|votre|leur|nous|vous|ils|elles|les|des|du|au|aux|une|quel|quelle|quels|quelles|je|elle|y|en|avec|dans|pour|sur|pas|c'?est|ça|ca|projet|préféré|prefere|préférés|preferes|souvenir|oublie|oublier|signifie|erreur|aime|aimé|orage|faire|probl[eè]me|solution|fonctionne|outil|magasin|interface|voulais|ajouter)\b/gi
...
/** Italian-distinct accents (exclude ù — French où). */
const IT_DISTINCT_CHARS = /[òì]/i
/** French-distinct accents / letters (exclude shared à/è/é/ù). */
const FR_DISTINCT_CHARS = /[âæçêëïîôûÿœ]/i
/** Spanish cues — omit bare é (collides with IT perché / FR participles). */
const ES_CHARS = /[áíóúñ¿¡]/i
...
/** Strong Italian elision / clitic units (#287).
 * Avoid trailing \\b after accented letters (JS \\b is ASCII-word only).
 */
const IT_ELISION_UNITS =
  /(?:^|[^\p{L}\p{N}_])(c['’]è|cos['’]è|com['’]è|dov['’]è|l['’]ho|l['’]hai|l['’]ha|l['’]abbiamo|l['’]avete|l['’]hanno|anch['’]io|quest['’]anno|un['’]\p{L}+)(?![\p{L}\p{N}_])/giu

/** Strong French elision units (#287). */
const FR_ELISION_UNITS =
  /(?:^|[^\p{L}\p{N}_])(c['’]est|j['’]ai|j['’]aime|j['’]étais|j['’]etais|qu['’]est[- ]?ce|aujourd['’]hui|d['’]accord|n['’]est|s['’]il|l['’]homme|l['’]eau|l['’]air)(?![\p{L}\p{N}_])/giu

/** Italian right-hand sides after l'/c'/d' — never credit as French elision. */
const IT_ELISION_RHS =
  /^(è|e|ho|hai|ha|abbiamo|avete|hanno|io|anno|altra|altro|ora|oggi)$/i
...
/**
 * Right-hand-aware elision scoring (#287).
 * Italian c'è / l'ho must not receive French c'/l' credit.
 * @param {string} t
 * @param {Record<string, number>} scores
 */
function scoreApostropheElisions(t, scores) {
  const itUnits = countMatches(t, IT_ELISION_UNITS)
  if (itUnits) scores.it += itUnits * 4

  const frUnits = countMatches(t, FR_ELISION_UNITS)
  if (frUnits) scores.fr += frUnits * 4

  // Additional French: "je l'ai …"
  if (/\bje\s+l['’]ai\b/i.test(t) || /\bl['’]ai\s+(?:déjà|deja|fait|vu|dit|pris)\b/i.test(t)) {
    scores.fr += 3
  }

  // Residual Italian article elisions (dell'/nell'/…)
  if (/\b(dell|nell|all|dall|sull|quest|quell)['’]/i.test(t)) {
    scores.it += 3
  }

  // Generic French-style letter+' — skip Italian right-hand sides already covered.
  const flags = 'gi'
  const re = new RegExp(`\\b(l|d|c|j|n|m|t|s|qu)['’]([A-Za-zÀ-ÖØ-öø-ÿ]+)`, flags)
  let m
  while ((m = re.exec(t)) !== null) {
    const full = m[0]
    const rhs = m[2] || ''
    // Already counted as a strong IT/FR lexical unit — do not double-count.
    if (IT_ELISION_UNITS.test(full) || FR_ELISION_UNITS.test(full)) {
      IT_ELISION_UNITS.lastIndex = 0
      FR_ELISION_UNITS.lastIndex = 0
      continue
    }
    IT_ELISION_UNITS.lastIndex = 0
    FR_ELISION_UNITS.lastIndex = 0
    if (IT_ELISION_RHS.test(rhs)) continue
    scores.fr += 2
  }
}

/**
 * Shared Romance tokens that collide IT↔FR (#287).
 * Only credit French when surrounding FR evidence is present.
 * @param {string} t
 * @param {Record<string, number>} scores
 */
function scoreRomanceSharedTokens(t, scores) {
  const hasFrContext =
    scores.fr >= 4 ||
    /\b(je|est|suis|avec|dans|pour|pas|une|des|les|mon|mes|très|tres|oui|bonjour|pourquoi|comment)\b/i.test(
      t,
    ) ||
    FR_DISTINCT_CHARS.test(t) ||
    FR_ELISION_UNITS.test(t)
  FR_ELISION_UNITS.lastIndex = 0

  if (hasFrContext) {
    if (/\b(je\s+)?lui\b|\blui\s+(?:ai|a|as|avez|ont|dire|parl|donn|fait)\b/i.test(t)) {
      scores.fr += 3
    }
    if (
      /\bma\s+(?:voiture|m[eè]re|s[oœ]ur|soeur|maison|chambre|vie|femme|fille|copine|journ[eé]e|chance)\b/i.test(
        t,
      )
    ) {
      scores.fr += 3
...
/**
 * Orthographic cues without equal-boosting shared à/è/é (#287).
 * @param {string} t
 * @param {Record<string, number>} scores
 */
function scoreOrthography(t, scores) {
  if (IT_DISTINCT_CHARS.test(t)) scores.it += 4
  if (FR_DISTINCT_CHARS.test(t)) scores.fr += 4
  if (ES_CHARS.test(t)) scores.es += 5
  if (DE_CHARS.test(t)) scores.de += 5
  if (/[¿¡]/.test(t)) scores.es += 6

  // French où (ù) — do not treat as Italian-distinct.
  if (/(?:^|[^\p{L}])o[uù](?![\p{L}])/iu.test(t) && /\b(est|sont|le|la|les|probl)/i.test(t)) {
    scores.fr += 4
  }

  // Shared è: Italian default, but suppress when French evidence is clear.
  const frEvidence =
    FR_DISTINCT_CHARS.test(t) ||
    countMatches(t, FR_ELISION_UNITS) > 0 ||
    /\b(est|sont|avec|dans|pour|pas|une|des|les|je|pourquoi|comment|probl[eè]me|très|tres|où|ou)\b/i.test(
      t,
    )
  if (/[è]/i.test(t)) {
    if (frEvidence) scores.fr += 1
    else scores.it += 3
  }

  if (/[é]/i.test(t) && (FR_DISTINCT_CHARS.test(t) || /\b(c['’]est|idée|idee|déjà|deja|été|ete|parlé|parle)\b/i.test(t))) {
    scores.fr += 2
  } else if (/[é]/i.test(t) && /\b(perché|perche|affinché|affinche|funziona|perché)\b/i.test(t)) {
    scores.it += 2
  }
}
...
export function detectLanguageSignal(text) {
  ...
  scoreApostropheElisions(t, scores)
  scoreRomanceSharedTokens(t, scores)
  ...
  if (
    /\b(come\s+stai|come\s+va|che\s+ne\s+pensi|cosa\s+ne\s+pensi|che\s+fai|che\s+faccio|cosa\s+facciamo|tutto\s+bene|cosa\s+significa|qual\s+['’]?è|qual\s+e|mi\s+piace|a\s+me\s+piace|mi\s+piacciono|i\s+miei|le\s+mie|voglio\s+imparare|progetto\s+principale|parliamo\s+di|preferit[oi]|preferit[ae]|personaggi\s+preferit|ma\s+dai|ce\s+l['’]ho|te\s+l['’]ho)\b/i.test(
      t,
    )
  ) {
    scores.it += 6
  ...
  if (
    /\b(comment\s+ça\s+va|comment\s+ca\s+va|qu['’]?est[- ]ce|mon\s+projet|je\s+suis|je\s+voudrais|j['’]aime|je\s+t['’]?aime|quels?\s+sont|très\s+content|il\s+faut|ce\s+soir|o[uù]\s+est|je\s+lui)\b/i.test(
      t,
    )
  ) {
    scores.fr += 6
...
export function deriveStickyConversationLanguage(messages, excludeLatest) {
  ...
  // Explicit language requests establish sticky even when content scoring is weak
  // (e.g. "Rispondimi in italiano." / "Ora torniamo a parlare in italiano.").
  for (let i = recentUsers.length - 1; i >= 0; i -= 1) {
    const content = recentUsers[i]
    if (exclude && content === exclude) continue
    const intent = detectLanguageIntent(content)
    if (intent.explicit && SUPPORTED.has(intent.explicit)) {
      return /** @type {ReplyLanguage} */ (intent.explicit)
    }
