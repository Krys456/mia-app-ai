...
function defineCategoryPlugin(def) {
  ...
  return {
    ...
    needsConfirmation(capability) {
      ...
      return /^(create|update|delete|send|set|start|stop|play|purchase|share|move|write)/i.test(
        capability,
      )
    },
    /**
...
export function registerBuiltinPlugins() {
  const builtins = [
    defineCategoryPlugin({
      id: 'smart_home',
      category: 'Smart Home',
      capabilities: ['set', 'query', 'scene'],
      requiredPermissions: ['smarthome.control'],
      pattern:
        /\b(accendi|spegni|dimmer|termostato|luce|luci|lampad[ae]|presa|smart\s*home|homekit|hue)\b/i,
      resolve(text) {
      ...
      confirmIf: (cap) => cap === 'set' || cap === 'scene',
    }),
    ...
    defineCategoryPlugin({
      ...
      pattern:
        /\b(calendario|appuntament[oi]|riunion[ei]|meeting|agenda|schedule|prenota|book\s+a\s+meeting|crea\s+evento)\b/i,
      resolve(text) {
        ...
        const title =
          ...
          text.slice(0, 80)
        return {
          capability: create ? 'create' : 'list',
          params: { title, raw: text },
          actionSummary: create ? `Creare evento: ${title}` : 'Consultare calendario',
        }
      },
    }),
    ...
    defineCategoryPlugin({
      ...
      // Action-oriented weather (alerts / location weather fetch via action path)
      pattern:
        /\b(allerta\s+meteo|weather\s+alert|avvisami\s+se\s+piove|notify\s+me\s+(?:if|when)\s+.*\b(rain|snow|storm))\b/i,
      resolve(text) {
        return {
          capability: 'alert',
          params: { raw: text },
          actionSummary: 'Impostare avviso meteo',
        }
      },
    }),
