...
export async function analyzeAction(input) {
  ...
  // Multi-step plans: user already asked for the overall workflow → auto low-risk reads
  const batchLowOk =
    input?.batchAuthorizeLow === true &&
    trust.trustLevel === 'low' &&
    trust.policy === 'confirm_once'

  const confirmationRequired =
    trust.confirmationRequired && !explicitForce && !batchLowOk
  ...
  if (batchLowOk) {
    ...
    reasons.push('Low-risk authorized by multi-step plan batch.')
  }
