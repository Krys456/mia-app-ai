...
describe('MemoryToast + ChatContext wiring (#281)', () => {
  it('W: role=status / aria-live preserved; book icon; no brain', () => {
    const toast = read('src/components/MemoryToast.tsx')
    const css = read('src/components/MemoryToast.css')
    assert.match(toast, /role="status"/)
    assert.match(toast, /aria-live="polite"/)
    assert.match(toast, /📖/)
    assert.doesNotMatch(toast, /🧠/)
    assert.doesNotMatch(toast, /Ho salvato una nuova memoria/)
    assert.match(css, /prefers-reduced-motion/)
    assert.match(css, /safe-area-inset-bottom/)
    assert.match(css, /composer-h/)
    assert.match(css, /max-width:\s*min\(92vw/)
  })
  ...
  it('O/P: New Chat + ASSISTANT_FAIL + TRIM_TO clear notice', () => {
    const ctx = read('src/context/ChatContext.tsx')
    assert.match(ctx, /memoryNotice:\s*MemoryFeedbackEvent\s*\|\s*null/)
    assert.match(ctx, /case 'NEW_CHAT':[\s\S]*?memoryNotice:\s*null/)
    assert.match(ctx, /case 'ASSISTANT_FAIL':[\s\S]*?memoryNotice:\s*null/)
    assert.match(ctx, /case 'TRIM_TO':[\s\S]*?memoryNotice:\s*null/)
    assert.match(ctx, /CLEAR_MEMORY_NOTICE/)
    // Ephemeral: ChatMessage type has no memoryEvent/memoryNotice field
    const types = read('src/types.ts')
    assert.doesNotMatch(types, /interface ChatMessage \{[\s\S]*?memory(Event|Notice)/)
  })
  ...
  it('F/G/H/I/J gates remain: OFF / unsafe / image / document / caption', () => {
    ...
    const toast = read('src/components/MemoryToast.tsx')
    assert.match(toast, /memoryFeedbackLabel/)
  })
})
...
describe('regressions: Core invariants + #277–#280 untouched by toast', () => {
  ...
  it('N: regenerate clears notice before re-run (TRIM_TO)', () => {
    const ctx = read('src/context/ChatContext.tsx')
    assert.match(ctx, /const regenerateAssistant = useCallback\([\s\S]{0,900}TRIM_TO/)
    assert.match(ctx, /case 'TRIM_TO':[\s\S]{0,250}memoryNotice:\s*null/)
  })
})
