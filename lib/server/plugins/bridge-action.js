...
import { resolveTrustLevel, setPluginTrustConfig } from '../trust/index.js'
...
export function bridgeActionPlugin(actionPlugin) {
  ...
  const actionTrust = {}
  for (const cap of actionPlugin.capabilities || []) {
    actionTrust[cap] = resolveTrustLevel({
      pluginId: actionPlugin.id,
      category: actionPlugin.category,
      actionId: cap,
      mutating: actionPlugin.needsConfirmation?.(cap, {}),
    })
  }

  // Permissions configurable per plugin (defaults from action plugin scopes)
  setPluginTrustConfig(actionPlugin.id, {
    actionTrust,
    permissions: [...(actionPlugin.requiredPermissions || [])],
    autoLowIfAuthorized: true,
    mediumConfirm: 'always',
  })
