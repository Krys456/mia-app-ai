...
// —— #272 image-only / empty caption: NO NEW LANGUAGE SIGNAL ——
{
  const itHist = [
    { role: 'user', content: 'Ciao, parliamo in italiano.' },
    { role: 'assistant', content: 'Certo! Di cosa vuoi parlare?' },
    { role: 'user', content: 'Dimmi qualcosa sulla fotografia.' },
    { role: 'assistant', content: 'La luce naturale rende i ritratti più caldi.' },
  ]
  const enHist = [
    { role: 'user', content: 'Hi, let’s talk in English.' },
    { role: 'assistant', content: 'Sure — what’s on your mind?' },
    { role: 'user', content: 'Tell me something interesting about photography.' },
    { role: 'assistant', content: 'Natural light at sunset warms portraits.' },
  ]
  const frHist = [
    { role: 'user', content: 'Bonjour, parlons en français.' },
    { role: 'assistant', content: 'Bien sûr ! De quoi veux-tu parler ?' },
    { role: 'user', content: 'Dis-moi quelque chose sur la photographie.' },
    { role: 'assistant', content: 'La lumière naturelle rend les portraits plus chauds.' },
  ]
  const esHist = [
    { role: 'user', content: 'Hola, hablemos en español.' },
    { role: 'assistant', content: '¡Claro! ¿De qué quieres hablar?' },
    { role: 'user', content: 'Dime algo interesante sobre la fotografía.' },
    { role: 'assistant', content: 'La luz natural hace los retratos más cálidos.' },
  ]
  const deHist = [
    { role: 'user', content: 'Hallo, lass uns auf Deutsch sprechen.' },
    { role: 'assistant', content: 'Klar! Worüber möchtest du sprechen?' },
    { role: 'user', content: 'Erzähl mir etwas Interessantes über Fotografie.' },
    { role: 'assistant', content: 'Natürliches Licht macht Porträts wärmer.' },
  ]

  // A IT history + image-only
  {
    const plan = buildLanguageAwarenessPlan({ userMessage: '', messages: [...itHist, { role: 'user', content: '' }] })
    assert.equal(plan.active, true, '#272 A active')
    assert.equal(plan.replyLanguage, 'it', '#272 A replyLanguage it')
    assert.equal(plan.conversationLanguage, 'it', '#272 A sticky it')
    assert.equal(plan.switched, false, '#272 A no switch')
    assert.equal(plan.noLinguisticSignal, true, '#272 A noLinguisticSignal')
    const appendix = buildCoreLanguageAppendix({
      userMessage: '',
      messages: [...itHist, { role: 'user', content: '' }],
    })
    assert.ok(appendix.includes('response language: it'), '#272 A brief response language it')
    assert.ok(appendix.includes('Current user turn contains no linguistic signal.'), '#272 A no-signal line')
    assert.ok(
      appendix.includes('Respond in the established conversation language: it.'),
      '#272 A established language line',
    )
  }

  // B EN
  {
    const plan = buildLanguageAwarenessPlan({ userMessage: '', messages: [...enHist, { role: 'user', content: '' }] })
    assert.equal(plan.replyLanguage, 'en', '#272 B en')
    assert.ok(
      buildCoreLanguageAppendix({ userMessage: '', messages: [...enHist, { role: 'user', content: '' }] }).includes(
  ...
  // C FR
  {
    const plan = buildLanguageAwarenessPlan({ userMessage: '', messages: [...frHist, { role: 'user', content: '' }] })
    assert.equal(plan.replyLanguage, 'fr', '#272 C fr')
  }

  // D ES
  {
    const plan = buildLanguageAwarenessPlan({ userMessage: '', messages: [...esHist, { role: 'user', content: '' }] })
    assert.equal(plan.replyLanguage, 'es', '#272 D es')
  }

  // E DE
  {
    const plan = buildLanguageAwarenessPlan({ userMessage: '', messages: [...deHist, { role: 'user', content: '' }] })
    assert.equal(plan.replyLanguage, 'de', '#272 E de')
  }

  // F no established history + image-only → safe inactive/auto fallback
  {
    const plan = buildLanguageAwarenessPlan({ userMessage: '', messages: [{ role: 'user', content: '' }] })
    assert.equal(plan.active, false, '#272 F inactive')
    assert.equal(plan.replyLanguage, 'auto', '#272 F auto')
    const appendix = buildCoreLanguageAppendix({ userMessage: '', messages: [{ role: 'user', content: '' }] })
    assert.ok(appendix.includes('Respond in the language of the user'), '#272 F contract only')
    assert.ok(!appendix.includes('response language:'), '#272 F no forced response language')
  }

  // G IT history + English caption → EN switch
  {
    const plan = buildLanguageAwarenessPlan({
      userMessage: 'What is in this photo?',
      messages: [...itHist, { role: 'user', content: 'What is in this photo?' }],
    })
    assert.equal(plan.replyLanguage, 'en', '#272 G EN caption switches')
    assert.equal(plan.switched, true, '#272 G switched')
  }

  // H FR history + Italian caption → IT switch
  {
    const plan = buildLanguageAwarenessPlan({
      userMessage: 'Che cos’è questa foto?',
      messages: [...frHist, { role: 'user', content: 'Che cos’è questa foto?' }],
    })
    assert.equal(plan.replyLanguage, 'it', '#272 H IT caption switches')
    assert.equal(plan.switched, true, '#272 H switched')
  }
}

console.log('ok: language-awareness #262+#267+#272 tests passed')
