...
export const WRITER_VERSION = '3.2.1-writer'
...
/** Bare minimal user turns (surface form only — meaning depends on prior assistant turn). */
...
/** Allowed one-line replies for passive acks with no pending proposal. */
...
/**
 * Short-reply intents — meaning depends on the previous assistant turn.
 * @typedef {'accept_proposal'|'continue'|'passive_acknowledgement'|'stop'|'change_topic'|'uncertain'} ShortReplyIntent
 */

/** User forms that can mean accept / continue / passive / stop depending on context. */
const SHORT_REPLY_CANDIDATE_RE =
  /^(ok|okay|okey|okk|va\s*bene|d['’]?accordo|alright|all\s*right|sì|si|yes|yep|yeah|yup|certo|esatto|perfetto|vai|vai\s*pure|continua|continuiamo|dai|procedi|go\s*on|go\s*ahead|keep\s*going|tell\s*me\s*more|dimmi|dimmi\s*di\s*più|👍|👌|mh+|mhm+|mm+|uhm+)[.!…]*$/i
...
/**
 * Prior assistant turn looks like an unresolved conversational proposal / offer / next step.
 * @type {RegExp[]}
 */
const UNRESOLVED_PROPOSAL_RES = [
  /\b(possiamo|potremmo|proviamo|andiamo|facciamo)\b/i,
  /\b(posso|vorrei|voglio)\s+(spieg|raccont|mostrar|parlar|propor|iniziar|partir)/i,
  ...
  /\b(magari|ad\s+esempio|for\s+example|adesso\s+ti|allora\s+(scelgo|ti\s+dico|parto))\b/i,
  /\b(curiosit[aà]|esplor|scienz|stor(ia|ie)|spazio|corpo\s+umano|buchi\s+neri)\b/i,
  /\?\s*$/,
]
...
export function hasUnresolvedConversationalProposal(assistantText) {
  ...
  // Bare assistant acks are not proposals.
  if (ALLOWED_MINIMAL_REPLIES.has(t) || /^(capisco|ok|okay)\.?$/i.test(t)) return false
  return UNRESOLVED_PROPOSAL_RES.some((re) => re.test(t))
}

/**
 * Classify short user replies using the previous assistant turn + plan hints.
 * Words like "ok" / "vai" / "sì" have no fixed meaning.
 *
 * @param {string|any} requestOrUser
 * @param {string} [previousAssistant]
 * @returns {{
 *   intent: ShortReplyIntent | null,
...
export function classifyShortReply(requestOrUser, previousAssistant = '') {
  const userText =
    typeof requestOrUser === 'string'
      ? asString(requestOrUser).replace(/\s+/g, ' ').trim()
      : latestUserMessage(requestOrUser)
  const request = typeof requestOrUser === 'string' ? null : requestOrUser
  const prev =
    asString(previousAssistant).trim() ||
    (request ? latestAssistantMessage(request) : '')
  const continueTopic = Boolean(request?.plan?.writerBrief?.continueTopic)
  const strategy = asString(request?.plan?.writerBrief?.strategy || request?.decision?.strategy)
  const hasPendingProposal =
    hasUnresolvedConversationalProposal(prev) ||
    (continueTopic && Boolean(prev) && !ALLOWED_MINIMAL_REPLIES.has(prev))
  ...
  // "ok grazie" / "grazie" after a completed beat → brief close, not execute proposal.
  if (THANKS_DONE_RE.test(userText) && !hasUnresolvedConversationalProposal(prev)) {
    return {
      intent: 'stop',
      isShortReply: true,
  ...
  if (CHANGE_TOPIC_SHORT_RE.test(userText)) {
    return {
      ...
      userText,
      previousAssistant: prev,
      reason: 'change_topic_cue',
    }
  }

  const shortForm =
    SHORT_REPLY_CANDIDATE_RE.test(userText) || isMinimalUserTurn(userText)
  if (!shortForm) {
    return {
      intent: null,
      isShortReply: false,
      ...
      userText,
      previousAssistant: prev,
      reason: 'not_short_reply',
    }
  }

  if (/^(mh+|mhm+|mm+|uhm+)[.!…]*$/i.test(userText)) {
    return {
      intent: 'uncertain',
      isShortReply: true,
      hasPendingProposal,
      userText,
      previousAssistant: prev,
      reason: hasPendingProposal ? 'uncertain_with_proposal' : 'uncertain_bare',
    }
  }

  // Open proposal / continue strategy → short reply accepts and continues.
  if (hasPendingProposal) {
    const goOn =
      /^(vai|continua|continuiamo|dai|procedi|go\s*on|keep\s*going|tell\s*me\s*more|dimmi|dimmi\s*di\s*più)[.!…]*$/i.test(
        userText,
      )
    return {
      ...
      userText,
      previousAssistant: prev,
      reason: goOn ? 'explicit_continue' : 'accept_open_proposal',
    }
  }

  if (
    (strategy === 'continue' || strategy === 'explore' || continueTopic) &&
    prev &&
    !ALLOWED_MINIMAL_REPLIES.has(prev)
  ) {
    return {
      intent: 'continue',
      isShortReply: true,
      ...
      userText,
      previousAssistant: prev,
      reason: 'plan_continue_without_explicit_proposal',
    }
  }

  return {
    intent: 'passive_acknowledgement',
    isShortReply: true,
    hasPendingProposal: false,
    userText,
    previousAssistant: prev,
...
/**
 * Only passive acks (no open proposal) may collapse to "Va bene." / peers.
 * @param {any} request
 * @returns {boolean}
 */
export function shouldForceMinimalAck(request) {
  if (!request || !isMinimalUserTurn(request)) return false
  const classified = classifyShortReply(request)
  return (
    classified.intent === 'passive_acknowledgement' ||
    (classified.intent === 'stop' && THANKS_DONE_RE.test(classified.userText))
  )
}
 ...
 * @returns {string}
 */
export function buildIdentityEvaluatorRewriteBrief(evaluation) {
...
export function assembleInstructions(request, options = {}) {
  ...
  const shortReply = classifyShortReply(req)
  ...
  const voiceRules = [
    ...
    'The Planner already decided strategy, tone, depth, coda, and constraints.',
  ...
  if (forceMinimal) {
    voiceRules.push(
      '',
      'MINIMAL TURN (hard)',
      'The latest user message is only a short ack with no open conversational proposal.',
    ...
    (shortReply.intent === 'uncertain' && shortReply.hasPendingProposal)
  ) {
    voiceRules.push(
      '',
      'ACCEPT / CONTINUE (hard)',
      'The latest user message is a short acknowledgement (ok / sì / vai / continua / …).',
      'In context it ACCEPTS the previous assistant proposal or asks to continue.',
      'Do NOT reply with dead-end acknowledgements like "Va bene.", "Ok.", "Capisco.", "Perfetto.", "D\'accordo."',
      'Immediately execute the proposed conversational action: introduce the topic, explain, tell the story, or take the next step.',
      'Carry conversational momentum yourself — do not ask the user to choose from a menu.',
      shortReply.previousAssistant
        ? `Previous assistant proposal to execute:\n"""${shortReply.previousAssistant.slice(0, 500)}"""`
        : 'Use the living topic from the conversation history.',
    )
  } else if (shortReply.intent === 'stop') {
    voiceRules.push(
      '',
      'STOP / CLOSE (hard)',
      'The user wants to stop or has thanked you after a completed beat.',
      'Reply briefly and warmly. Do not start a new topic or continue explaining.',
    )
  }
  ...
  async function write(request) {
    ...
    if (shouldForceMinimalAck(request) && !isAllowedMinimalReply(response.text)) {
      ...
      const rewriteRequest = {
        ...request,
        mode: 'rewrite',
        rewriteBrief: buildIdentityEvaluatorRewriteBrief(draftIdentity),
      ...
      const finalIdentity = evaluateWriterIdentity(rewritten.text, request)
      identityTrace = {
        draftText,
        draft: draftIdentity,
        final: finalIdentity,
