 ...
 */
/**
 * Soft-dedupe nearly identical must/mustNot lines for Writer salience.
 * Does not remove safety/grounding constraints.
 * @param {string[]} lines
 * @returns {string[]}
 */
function dedupeDirectiveLines(lines) {
...
/**
 * When Planner already requires one concrete direction, drop experience/momentum
 * lines that push multi-option menus — they bury the primary objective.
 * @param {string} line
 * @returns {boolean}
 */
function contradictsOneDirectionDelivery(line) {
  return /\b(varie\s+direzioni|molte\s+idee|expanding\s+options|topic\s+menu|menu\s+of\s+topics|proponi\s+varie|privilegia\s+variet)\b/i.test(
    asString(line),
  )
}
 ...
 * @returns {boolean}
 */
function planRequiresConcreteInitiative(plan) {
  ...
  const topic = asString(brief.activeTopic || '').trim()
  if (topic) return false
  if (strategy !== 'explore' && asString(brief.coda) !== 'direction') return false
  const objective = asString(plan.objective || '')
  const constraints = Array.isArray(plan.constraints) ? plan.constraints : []
  return (
    /Introduce one concrete|concrete interesting subject/i.test(objective) ||
    asString(brief.coda) === 'direction' ||
    constraints.some((c) => /initiative\s*:\s*one_direction/i.test(String(c))) ||
    (Array.isArray(brief.must) &&
      brief.must.some((m) => /Topic-establishing initiative|ONE concrete/i.test(String(m))))
  )
}
...
export function formatPlanForWriter(plan) {
  ...
  const oneDirection = planRequiresConcreteInitiative(plan)
  ...
  if (brief) {
    ...
    if (Array.isArray(brief.must) && brief.must.length) {
      lines.push('MUST:')
      let mustLines = dedupeDirectiveLines(brief.must)
      if (oneDirection) {
        mustLines = mustLines.filter((m) => !contradictsOneDirectionDelivery(m))
      }
      for (const m of mustLines) lines.push(`- ${m}`)
    }
    if (Array.isArray(brief.mustNot) && brief.mustNot.length) {
      lines.push('MUST NOT:')
      for (const m of dedupeDirectiveLines(brief.mustNot)) lines.push(`- ${m}`)
    }
  }
  if (cp) {
    ...
    if (Array.isArray(cp.development)) {
      for (const beat of cp.development) {
        const purpose = asString(beat.purpose)
        if (oneDirection && contradictsOneDirectionDelivery(purpose)) continue
        lines.push(`- development[${asString(beat.kind)}]: ${purpose}`)
      }
    }
    if (cp.closing) {
      lines.push(
...
export function assembleInstructions(request, options = {}) {
  ...
  const parts = []
  const concreteInitiative = planRequiresConcreteInitiative(req.plan)
  const planBlock = formatPlanForWriter(req.plan)

  // For one_direction initiative turns, put the delivery contract + plan BEFORE voice
  // corpus/examples so the primary objective is not buried under ~5k chars of tone material.
  if (concreteInitiative) {
    parts.push(
      [
        'PRIMARY DELIVERY CONTRACT (highest priority for this turn)',
        `objective=${asString(req.plan?.objective || 'Introduce one concrete interesting subject and start it.')}`,
        'Do not describe the intention to explore or introduce a topic.',
        'Actually choose ONE concrete subject, name it explicitly, and immediately give one substantive detail about it.',
        'No topic menu. No follow-up question. One direction only.',
        'Voice/style rules below are HOW only — they must not override this WHAT.',
      ].join('\n'),
    )
    if (planBlock) parts.push(planBlock)
  }

  // Voice corpus / examples — tone before lower-priority rules (after delivery contract when set).
  if (useVoiceCorpus) {
  ...
  if (!concreteInitiative && planBlock) parts.push(planBlock)
