  ...
  if (refStrip.flagged) {
    for (const sentence of refStrip.removed) {
      removed.push({
        ...
        reason: 'unsupported_referential_continuation',
      })
    }
    text = refStrip.text
    if (!text) {
      ...
      stripUnsupportedReferentialContinuation(text, {
        activeTopic: activeTopic || null,
        messages,
        isOpeningSocial: true,
      }).flagged
    ) {
      text = /^(ciao|hey|hi|hello)/i.test(original) ? 'Ciao!' : text.split(/(?<=[.!?])\s+/)[0] || 'Ciao!'
    }
  }
