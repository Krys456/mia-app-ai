...
export const REVIEWER_VERSION = '2.0.0-reviewer'
...
export const DEFAULT_THRESHOLDS = {
  ...
  emotionalCalibration: 0.5,
  practicalValue: 0.45,
  redundancy: 0.55,
  clicheDetection: 0.55,
  respectOfPlanner: 0.7,
  responseCompleteness: 0.55,
}
...
export const DEFAULT_WEIGHTS = {
  ...
  emotionalCalibration: 1,
  practicalValue: 1,
  redundancy: 1,
  clicheDetection: 1.1,
  respectOfPlanner: 1.4,
  ...
  function review(input) {
    ...
    const writerResponse =
      safe.writerResponse && typeof safe.writerResponse === 'object'
        ? safe.writerResponse
        : {}
    // Prefer explicit plan; fall back to request.plan (still read-only)
    const plan =
    ...
    const overall = Number(
      aggregateOverall(metricResults, weights).toFixed(4),
    )
    ...
    for (const key of METRIC_KEYS) {
      ...
      metrics[key] = Number(result.score.toFixed(4))
      const metricThreshold = thresholds[/** @type {keyof ReviewerThresholds} */ (key)]
      if (result.score < metricThreshold) {
        reasons.push(
          `${key}_below_threshold:${result.score.toFixed(2)}<${metricThreshold.toFixed(2)}`,
        )
        for (const note of result.notes) reasons.push(`${key}:${note}`)
      } else if (result.notes.length) {
        // keep high-signal notes only when overall may fail
        for (const note of result.notes) {
          if (
            /robotic|cliche|forbidden|empty|interview|minimizing|overlap|missing_/.test(
              note,
            )
          ) {
            reasons.push(`${key}:${note}`)
          }
        }
      }
    ...
    if (overall < thresholds.overallPass) {
      reasons.unshift(
        `overall_below_threshold:${overall.toFixed(2)}<${thresholds.overallPass.toFixed(2)}`,
      )
    }

    const decision =
      overall < thresholds.overallPass ? 'REWRITE' : 'PASS'

    // On PASS, keep suggestions empty unless soft tips desired — spec asks suggestions for rewrite guidance
    const finalSuggestions =
      decision === 'REWRITE' ? uniqueSuggestions(suggestions).slice(0, 12) : []

    // Still expose soft reasons on PASS for diagnostics, but capped
    const finalReasons =
      decision === 'REWRITE'
        ? uniqueSuggestions(reasons).slice(0, 24)
        : uniqueSuggestions(reasons.filter((r) => !r.startsWith('overall_'))).slice(0, 8)
