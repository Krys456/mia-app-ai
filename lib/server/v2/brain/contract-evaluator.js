...
export const CONTRACT_EVALUATOR_VERSION = '2.1.3-contract-evaluator'
 ...
 * @returns {boolean}
 */
function decisionInitiativeIsDirection(brief, plan) {
  ...
  return (
    coda === 'direction' ||
    /Introduce one concrete|one_direction|initiative/i.test(objective) ||
    constraints.some((c) => /one_direction|direction/i.test(String(c)))
  )
}

/**
 * Explore + one_direction + null topic + introduce-subject objective.
 * Delivery must name a concrete subject and develop it — not describe exploring.
 *
 * @param {object|null|undefined} brief
 * @param {object|null|undefined} plan
 ...
 * @returns {boolean}
 */
export function requiresConcreteInitiativeDelivery(brief, plan, state) {
  const strategy = asString(brief?.strategy || plan?.strategy || '')
  const topic = asString(brief?.activeTopic || state?.activeTopic || '').trim()
  if (topic) return false
  if (strategy !== 'explore') return false

  const objective = asString(plan?.objective || '')
  const constraints = Array.isArray(plan?.constraints) ? plan.constraints : []
  const initiativeConstraint = constraints.some((c) =>
    /initiative\s*:\s*one_direction|^one_direction$/i.test(String(c)),
  )
  const oneDirection =
    asString(brief?.coda) === 'direction' ||
    initiativeConstraint ||
    /one_direction/i.test(objective) ||
    decisionInitiativeIsDirection(brief, plan)
  const introducesSubject =
    /Introduce one concrete|concrete interesting subject|name\/?establish|Topic-establishing/i.test(
      objective,
    ) ||
    (Array.isArray(brief?.must) &&
      brief.must.some((m) =>
        /Topic-establishing initiative|Introduce one concrete|ONE concrete interesting subject/i.test(
          String(m),
        ),
      ))

  return oneDirection && introducesSubject
}
...
function buildRewriteBrief(args) {
  ...
  if (codes.includes('collapsed_execute_continue') || codes.includes('collapsed_initiative')) {
    targeted.push('Deliver the pending content now — never reply with only "Va bene." / "Ok." / "Capisco."')
  }
  ...
  if (codes.includes('unsupported_referential_continuation')) {
    targeted.push(
      'Remove unsupported pronouns/referents and in-medias-res openings with no antecedent.',
    )
    targeted.push(
      'If the plan requires introducing a new subject (activeTopic is null): explicitly NAME/establish that concrete subject in the first substantive sentence, then explain it. Do not start with "È un fenomeno…", "Questa cosa…", "This shows…".',
    )
    targeted.push(
      'Preserve Planner WHAT: introduce ONE concrete subject, begin explaining it, no topic menu, no follow-up question, no invented prior context.',
    )
  }
  if (
    codes.includes('missing_concrete_initiative_delivery') ||
    codes.includes('missing_subject_development')
  ) {
    targeted.push(
      'Do not describe the intention to introduce a topic. Actually choose one concrete subject, name it explicitly, and immediately provide one substantive detail about it.',
    )
    targeted.push(
      'Preserve: no topic menu, no follow-up question, one direction only, responseProfile, language, conversationalMove.',
    )
  }
...
export function evaluateContractFidelity(input = {}) {
  ...
  // Boredom / explore initiative must not collapse to Capisco./Ok./Va bene.
  const strategy = asString(brief.strategy || plan.strategy || '')
  const initiativeHint =
    /initiative|concrete interesting subject|Introduce one concrete/i.test(objective) ||
    strategy === 'explore' ||
    asString(state?.activeGoal) === 'casual_exploration' ||
    (conversationSignals && conversationSignals.affect?.boredom >= 0.55)
  if (initiativeHint && text && DEAD_ACK_RE.test(text)) {
    violations.push({
      code: 'collapsed_initiative',
      message: 'Explore/boredom initiative collapsed to acknowledgement only',
      severity: 'hard',
    })
  }

  // Unsupported referential continuation / in-medias-res opening (no antecedent).
  if (text && !asString(state?.activeTopic || brief.activeTopic || '')) {
    const refCheck = stripUnsupportedReferentialContinuation(textRaw || text, {
      activeTopic: null,
      messages: Array.isArray(input.messages) ? input.messages : [],
      isOpeningSocial:
        asString(state?.conversationPhase) === 'opening' ||
        strategy === 'connect' ||
        asString(state?.activeGoal) === 'social_connection',
      requireTopicEstablishment: initiativeHint || decisionInitiativeIsDirection(brief, plan),
    })
    const inMediasRes = hasUnsupportedInMediasResOpening(textRaw || text, {
      activeTopic: null,
      messages: Array.isArray(input.messages) ? input.messages : [],
    })
    if (
      refCheck.flagged ||
      inMediasRes ||
      hasUnresolvedCentralReferent(textRaw || text, { activeTopic: null })
    ) {
      violations.push({
        code: 'unsupported_referential_continuation',
        message: inMediasRes
          ? 'Reply opens in medias res without establishing the concrete subject'
          : 'Reply uses an unresolved pronoun/referent with no grounded antecedent',
        severity: 'hard',
      })
    }
  }

  // one_direction / boredom initiative: must deliver a concrete subject + development
  // (generic "let's explore something" language is under-delivery).
  if (text && requiresConcreteInitiativeDelivery(brief, plan, state)) {
    const hasSubject = responseEstablishesConcreteSubject(textRaw || text)
    const hasDevelopment = hasSubstantiveSubjectDevelopment(textRaw || text)

    if (!hasSubject) {
      violations.push({
        code: 'missing_concrete_initiative_delivery',
        message:
          'Planner requires one concrete direction but reply never establishes an identifiable subject',
        severity: 'hard',
      })
    } else if (!hasDevelopment) {
      violations.push({
        code: 'missing_subject_development',
        message:
          'Subject is named but no meaningful fact/idea/claim about it is provided',
        severity: 'hard',
      })
    }
  }
