...
export const CONTRACT_EVALUATOR_VERSION = '2.1.1-contract-evaluator'
...
function buildRewriteBrief(args) {
  ...
  if (codes.includes('unsupported_referential_continuation')) {
    targeted.push(
      'Remove unsupported pronouns/referents with no antecedent. Do not invent a subject to attach them to.',
    )
  }
...
export function evaluateContractFidelity(input = {}) {
  ...
  // Unsupported referential continuation (no antecedent in state/history).
  if (text && !asString(state?.activeTopic || brief.activeTopic || '')) {
    ...
    if (refCheck.flagged || hasUnresolvedCentralReferent(textRaw || text, { activeTopic: null })) {
      violations.push({
        code: 'unsupported_referential_continuation',
        message: 'Reply uses an unresolved pronoun/referent with no grounded antecedent',
        severity: 'hard',
      })
    }
  }
