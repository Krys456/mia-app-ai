/**
 * LAIfe V2 — Contract Evaluator (Phase 5)
 *
 * Post-Writer fidelity gate:
 *   A. Hard contract fidelity (Planner WHAT)
...
export const CONTRACT_EVALUATOR_VERSION = '2.0.0-contract-evaluator'
...
export function checkStockOpener(text, ctx = {}) {
  ...
  // Factual/definitional ask + stock padding.
  const factualAsk =
...
export function evaluateContractFidelity(input = {}) {
  ...
  if (profile && text) {
    ...
    const emojiHit = checkEmojiPolicy(profile.emojiPolicy, textRaw)
