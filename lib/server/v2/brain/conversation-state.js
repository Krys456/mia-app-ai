...
export const CONVERSATION_STATE_VERSION = '3.0.0-conversation-state'
...
const CORRECTION_RE =
  /\b(no,?\s+parlavo\s+(dell['’]?altro|di\s+altro)|intendevo\s+(dire\s+)?|non\s+quello|l['’]?altro|i\s+meant|actually\s+i\s+meant)\b/i
...
function resolveActiveTopic(args) {
  const { messages, focus, resume, previousState, shortReply, latestUser, perception } = args
  const priorTopic =
  ...
  // Correction: mark prior topic uncertain — prefer focus/latest over blind continue.
  if (CORRECTION_RE.test(latestUser)) {
    return focus?.topic || inferCurrentTopic(messages) || priorTopic
  }

  // Explicit topic change wins.
  if (
    (focus?.status === 'changed' || TOPIC_CHANGE_EXPLICIT_RE.test(latestUser)) &&
    TOPIC_CHANGE_EXPLICIT_RE.test(latestUser)
  ) {
    const next =
      focus.topic ||
      inferCurrentTopic([
...
export function buildConversationState(input = {}) {
  ...
  let activeGoal = inferActiveGoal(latestUser, perception, mode, shortReplyFull)
