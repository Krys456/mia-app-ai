...
export const CONVERSATION_STATE_VERSION = '3.3.0-conversation-state'
...
/**
 * @typedef {'learning'|'debugging'|'planning'|'casual_conversation'|'social_connection'|'exploration'|'decision_support'|'task_execution'|'casual_exploration'|null} ActiveGoal
 */
 ...
 * }} references
 * @property {import('./reference-resolution.js').RepairState} [repair]
 * @property {string[]} [recentAlternatives] short-lived contrasted options (not Memory)
 * @property {object|null} [referenceResolution] last resolution snapshot (diagnostics-friendly)
...
function topicFromProposalText(assistantText) {
  const t = asString(assistantText).replace(/\s+/g, ' ').trim()
  if (!t) return null
  if (hasUnresolvedCentralReferent(t, {})) return null
  const m =
    t.match(
      /\b(?:su|sul|sulla|sui|sugli|sulle|dei|degli|delle|di|about|regarding)\s+([A-ZÁÉÍÓÚÀÈÌÒÙa-záéíóúàèìòù][\wÁÉÍÓÚÀÈÌÒÙáéíóúàèìòù'’ -]{2,60})/i,
    ) ||
    t.match(
      /\b(?:curiosit[aà]|storia|spiegazione)\s+(?:su|sul|sulla|sui|di|dei)\s+([^.!?]{3,60})/i,
    )
  if (m && m[1]) {
    const topic = m[1].replace(/[.!?].*$/, '').trim().slice(0, 80) || null
    if (
      topic &&
      isValidActiveTopic(topic, { source: 'proposal', allowAssistantDerived: true })
    ) {
      return topic
    }
  }
...
function resolveActiveTopic(args) {
  ...
  const social = isSocialSmallTalkTurn({
  ...
  // Focus / resume topics are diagnostic candidates — must pass validation.
  const focusTopic =
    focus?.topic &&
    isValidActiveTopic(focus.topic, {
      userMessage: latestUser,
      perception,
      conversationSignals,
      messages,
  ...
  const resumeTopic =
    resume?.currentTopic &&
    isValidActiveTopic(resume.currentTopic, {
      userMessage: latestUser,
      perception,
      conversationSignals,
      messages,
  ...
  const inferred =
    ...
    isValidActiveTopic(inferCurrentTopic(messages), {
      userMessage: latestUser,
      perception,
      conversationSignals,
      messages,
      source: social ? 'assistant' : 'user',
      allowAssistantDerived: !social,
    })
      ? inferCurrentTopic(messages)
      : null

  // Small talk / opening social: topic is optional — do not manufacture from assistant text.
  if (social) {
    // Keep only a previously validated living topic when user clearly continues it.
    if (
      priorTopic &&
      (shortReply?.intent === 'accept_proposal' ||
  ...
  // Boredom / no-topic-to-talk-about: do not promote the utterance into activeTopic.
  const bored =
    asString(perception?.intent) === 'boredom' ||
    (conversationSignals && conversationSignals.affect?.boredom >= 0.55) ||
    BOREDOM_RE.test(latestUser)
  if (bored) {
    // Keep a previously valid living topic only on explicit continuation; otherwise null.
    if (
      priorTopic &&
      (shortReply?.intent === 'accept_proposal' ||
        shortReply?.intent === 'continue' ||
  ...
  if (TASK_EXECUTION_RE.test(latestUser)) {
    const next =
      inferCurrentTopic([{ role: 'user', content: latestUser }]) || focusTopic || null
    return next && isValidActiveTopic(next, { userMessage: latestUser, source: 'user' })
      ? next
      : null
  }

  // Correction: surface cue only — Phase 7 resolution updates later in build.
  if (correction) {
    candidate = focusTopic || inferred || priorTopic
  }
  ...
  if ((focus?.status === 'changed' || topicChange) && topicChange) {
    const next =
      ...
      latestUser
        ...
        .trim()
    if (next && isValidActiveTopic(next, { userMessage: latestUser, source: 'user' })) {
      return asString(next).slice(0, 80)
    }
    return null
  }

  if (focus?.status === 'changed' && focusTopic) {
    return focusTopic
  }

  // Short continuation / pronouns / "ok" → keep living prior topic when valid.
  if (
    shortReply?.intent === 'accept_proposal' ||
    shortReply?.intent === 'continue' ||
    ...
    return priorTopic || focusTopic || resumeTopic || null
  }

  // Incremental: when prior state is valid and Focus says active, keep prior topic.
  if (priorTopic && focus?.status === 'active') {
    return priorTopic
  }

  if (focus?.status === 'active' && focusTopic) {
    if (
      priorTopic &&
      asString(focusTopic)
        .toLowerCase()
        .includes(asString(priorTopic).toLowerCase().split(/\s+/)[0] || '___')
    ) {
      return priorTopic
    }
    return focusTopic
  ...
  return candidate && isValidActiveTopic(candidate, {
    userMessage: latestUser,
    perception,
    conversationSignals,
    messages,
...
function resolvePendingProposal(args) {
  ...
  const validateProposal = (proposal, assistantText = '') => {
    if (!proposal) return null
    const ok = isValidPendingProposal(
      {
        type: proposal.type,
        topic: proposal.topic,
        assistantText: assistantText || asString(shortReply?.previousAssistant || ''),
      },
      {
        userMessage: latestUser,
        perception,
    ...
    const topicOk =
      !proposal.topic ||
      isValidActiveTopic(proposal.topic, {
        userMessage: latestUser,
        perception,
        messages,
        source: 'proposal',
    ...
    if (!ok || !topicOk) return null
    // Unresolved pronoun in assistant offer text → never keep.
    if (
      assistantText &&
      hasUnresolvedCentralReferent(assistantText, { activeTopic, messages })
    ) {
      return null
    }
    return proposal
  ...
  if (isSocialSmallTalkTurn({ userMessage: latestUser, perception }) && !shortReply?.hasPendingProposal) {
    // Social check-in must not keep a phantom explore_topic from a vague greeting.
    if (
      prior &&
      (prior.type === 'explore_topic' ||
        !isValidActiveTopic(prior.topic, { source: 'proposal', allowAssistantDerived: true }))
    ) {
      return null
    }
  }
  ...
  if (prior && (priorStatus === 'open' || priorStatus === 'accepted')) {
    ...
    const nextIdle =
      shortReply?.intent === 'accept_proposal' || shortReply?.intent === 'continue'
        ? 0
        : Math.max(0, Number(prior.idleTurns) || 0) + 1
    if (nextIdle > PENDING_PROPOSAL_MAX_IDLE_TURNS) return null
    return validateProposal(
      {
        ...prior,
        status: priorStatus === 'accepted' ? 'open' : priorStatus,
        idleTurns: nextIdle,
      },
      shortReply?.previousAssistant || '',
    )
...
export function freezeConversationState(state) {
  ...
  const frozen = {
    ...
    references: Object.freeze({
      ...
      resolved: Object.freeze([...(state.references?.resolved || [])]),
      ambiguous: Object.freeze([...(state.references?.ambiguous || [])]),
    }),
    repair: state.repair ? Object.freeze({ ...state.repair }) : Object.freeze(emptyRepairState()),
    recentAlternatives: Object.freeze([...(state.recentAlternatives || [])]),
    referenceResolution: state.referenceResolution
      ? Object.freeze({ ...state.referenceResolution })
      : null,
...
export function buildConversationState(input = {}) {
  ...
  // Sanitize echoed previous State so phantom topics/proposals cannot cascade.
  const previousState = sanitizeEchoedConversationState(
  ...
  let activeTopic = resolveActiveTopic({
    messages,
    focus,
    resume,
    previousState,
  ...
  // Phase 7: Reference Resolution (short-range grounding — not a decision engine).
  ...
  // Apply strong / explicit corrections to activeTopic; never guess on unresolved.
  if (
    (referenceResolution.status === 'explicit' || referenceResolution.status === 'resolved') &&
    referenceResolution.referent &&
    (referenceResolution.confidenceBand === 'high' ||
      referenceResolution.confidenceBand === 'medium')
  ) {
    // Do not snap back to a rejected interpretation.
    const rejected = asString(referenceResolution.repair?.rejectedInterpretation).toLowerCase()
    if (
      !rejected ||
      referenceResolution.referent.toLowerCase() !== rejected
    ) {
      activeTopic = referenceResolution.referent
    }
  }
  ...
  // Topic correction / repair: clear conflicting pending proposal (State does not invent a new one).
  if (
    referenceResolution.repair?.active &&
    (referenceResolution.repair.type === 'explicit_correction' ||
      referenceResolution.repair.type === 'topic_correction' ||
      referenceResolution.repair.type === 'alternate_referent')
  ) {
    const proposalTopic = asString(pendingProposal?.topic || previousState?.pendingProposal?.topic)
    const corrected = asString(referenceResolution.repair.correctedReferent || referenceResolution.referent)
    if (
      pendingProposal &&
      proposalTopic &&
      corrected &&
      proposalTopic.toLowerCase() !== corrected.toLowerCase()
    ) {
      pendingProposal = null
    } else if (
      previousState?.pendingProposal &&
      referenceResolution.repair.rejectedInterpretation &&
      asString(previousState.pendingProposal.topic).toLowerCase() ===
        asString(referenceResolution.repair.rejectedInterpretation).toLowerCase()
    ) {
      pendingProposal = null
    }
  }
  ...
  // Repair turn: recovering phase while correction is active.
  if (referenceResolution.repair?.active && conversationPhase !== 'closing') {
  ...
  const unresolvedRefs = [
    ...unresolvedQuestions,
    ...(Array.isArray(referenceResolution.references?.unresolved)
      ? referenceResolution.references.unresolved
      : []),
  ].filter(Boolean)

  // Recent alternatives: refresh from assistant turn; expire on unrelated topic change.
  let recentAlternatives = Array.isArray(referenceResolution.recentAlternatives)
    ? referenceResolution.recentAlternatives.slice(0, 6)
    : []
  if (
    topicChangeCue &&
    previousState?.activeTopic &&
    activeTopic &&
    previousState.activeTopic.toLowerCase() !== activeTopic.toLowerCase() &&
    !(referenceResolution.repair?.active)
  ) {
    recentAlternatives = []
  }
  if (
    previousState?.conversationPhase === 'closing' &&
    conversationPhase !== 'recovering'
  ) {
    // Closed context should not leak old alternatives into a fresh thread.
    if (topicChangeCue || TASK_EXECUTION_RE.test(latestUser)) {
      recentAlternatives = []
    }
  }

  // Repair state: carry rejected interpretation briefly; clear when topic stabilizes.
  let repair = referenceResolution.repair
    ? { ...referenceResolution.repair }
    : emptyRepairState()
  if (
    !repair.active &&
    previousState?.repair?.rejectedInterpretation &&
    activeTopic &&
    previousState.repair.rejectedInterpretation.toLowerCase() !== activeTopic.toLowerCase() &&
    conversationPhase !== 'recovering'
  ) {
    // Keep rejectedInterpretation one more turn unless topic already differs (already cleared).
    repair = {
      ...emptyRepairState(),
    ...
    repair = {
      ...emptyRepairState(),
      rejectedInterpretation: previousState.repair.rejectedInterpretation,
    }
  }
  ...
  const state = {
    ...
    continuity,
    references: {
      unresolved: unresolvedRefs.slice(0, 8),
      resolved: Array.isArray(referenceResolution.references?.resolved)
        ? referenceResolution.references.resolved.slice(0, 4)
        : [],
      ambiguous: Array.isArray(referenceResolution.references?.ambiguous)
        ? referenceResolution.references.ambiguous.slice(0, 4)
        : [],
    },
    repair,
    recentAlternatives,
    referenceResolution: {
      ...
      referentType: referenceResolution.referentType,
      referent: referenceResolution.referent,
      confidence: referenceResolution.confidence,
      confidenceBand: referenceResolution.confidenceBand,
      expression: referenceResolution.expression,
    },
    turnCount,
    responseProfile:
    ...
    diagnostics: {
      ...
      signalsSummary: turnSignals
        ...
        : null,
      referenceStatus: referenceResolution.status,
      repairActive: Boolean(repair.active),
  ...
  return {
    ...
    references: {
      unresolved: Array.isArray(state.references?.unresolved)
        ...
        : [],
      resolved: Array.isArray(state.references?.resolved)
        ? state.references.resolved.slice(0, 4)
        : [],
      ambiguous: Array.isArray(state.references?.ambiguous)
        ? state.references.ambiguous.slice(0, 4)
        : [],
    },
    repair: state.repair && typeof state.repair === 'object'
      ? {
          active: Boolean(state.repair.active),
          type: state.repair.type ?? null,
          rejectedInterpretation: state.repair.rejectedInterpretation ?? null,
          correctedReferent: state.repair.correctedReferent ?? null,
          confidence:
            typeof state.repair.confidence === 'number' ? state.repair.confidence : null,
          requiresClarification: Boolean(state.repair.requiresClarification),
        }
      : emptyRepairState(),
    recentAlternatives: Array.isArray(state.recentAlternatives)
      ? state.recentAlternatives.map((x) => String(x || '').trim()).filter(Boolean).slice(0, 6)
      : [],
    referenceResolution:
      state.referenceResolution && typeof state.referenceResolution === 'object'
        ? {
            status: state.referenceResolution.status ?? null,
            referentType: state.referenceResolution.referentType ?? null,
            referent: state.referenceResolution.referent ?? null,
            confidence: state.referenceResolution.confidence ?? null,
            confidenceBand: state.referenceResolution.confidenceBand ?? null,
          }
        : null,
    responseProfile:
...
export function hydrateConversationState(raw) {
  ...
  const state = {
    ...
    references: {
      ...
      resolved: Array.isArray(v.references?.resolved) ? v.references.resolved : [],
      ambiguous: Array.isArray(v.references?.ambiguous) ? v.references.ambiguous : [],
    },
    repair:
      v.repair && typeof v.repair === 'object'
        ? {
            active: Boolean(v.repair.active),
            type: v.repair.type ?? null,
            rejectedInterpretation: v.repair.rejectedInterpretation ?? null,
            correctedReferent: v.repair.correctedReferent ?? null,
            confidence: typeof v.repair.confidence === 'number' ? v.repair.confidence : null,
            requiresClarification: Boolean(v.repair.requiresClarification),
          }
        : emptyRepairState(),
    recentAlternatives: Array.isArray(v.recentAlternatives)
      ? v.recentAlternatives.map((x) => String(x || '').trim()).filter(Boolean).slice(0, 6)
      : [],
    referenceResolution:
      v.referenceResolution && typeof v.referenceResolution === 'object'
...
export function transitionConversationState(input = /** @type {any} */ ({})) {
  ...
  let topic = brief.activeTopic || pre.activeTopic || null
  const pendingType =
  ...
  if (
    ...
    !hasUnresolvedCentralReferent(responseText, {
      activeTopic: topic,
      messages: [],
    })
  ) {
    const proposalTopic = topicFromProposalText(responseText) || topic
    const candidate = {
    ...
    if (
      isValidPendingProposal(
        { ...candidate, assistantText: responseText },
        { activeTopic: topic },
      )
    ) {
      pendingProposal = candidate
    }
  }

  // Never publish an invalid topic from Writer brief / prior state.
  if (
    topic &&
  ...
  if (move === 'stop') conversationPhase = 'closing'
    ...
    conversationPhase = 'deepening'
  } else if (pre.conversationPhase === 'recovering') {
    // Successful delivery after repair → return to deepening.
    conversationPhase = 'deepening'
  }
  ...
  // Refresh alternatives from delivered assistant text; keep prior if none found.
  const fromResponse = extractRecentAlternatives(responseText, pre.recentAlternatives || [])
  const recentAlternatives =
    conversationPhase === 'closing'
      ? []
      : fromResponse.length >= 2
        ? fromResponse.slice(0, 6)
        : Array.isArray(pre.recentAlternatives)
          ? pre.recentAlternatives.slice(0, 6)
          : []

  // Clear active repair after successful delivery; keep rejectedInterpretation briefly.
  const repair = {
    active: false,
    type: null,
    rejectedInterpretation: pre.repair?.rejectedInterpretation ?? null,
    correctedReferent: null,
    confidence: null,
    requiresClarification: false,
  }
  // Clear rejectedInterpretation once topic has stabilized away from it.
  if (
    repair.rejectedInterpretation &&
    topic &&
    repair.rejectedInterpretation.toLowerCase() !== asString(topic).toLowerCase()
  ) {
    // Keep for one more turn only if still recovering context; else clear.
    if (pre.conversationPhase !== 'recovering') {
      repair.rejectedInterpretation = null
    }
  }
  ...
  const next = {
    ...
    references: {
      unresolved: Array.isArray(pre.references?.unresolved)
        ...
        : [],
      resolved: Array.isArray(pre.references?.resolved) ? [...pre.references.resolved] : [],
      ambiguous: Array.isArray(pre.references?.ambiguous) ? [...pre.references.ambiguous] : [],
    },
    repair,
    recentAlternatives,
    referenceResolution: pre.referenceResolution || null,
