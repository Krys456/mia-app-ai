...
export const TOPIC_VALIDATION_VERSION = '1.1.0-topic-validation'
...
/** Vague nouns that never count as a concrete initiative subject. */
const VAGUE_SUBJECT_NOUN_RE =
  /^(cosa|cose|fenomeno|fatto|argomento|topic|subject|something|qualcosa|momento|occasione|direzione|idea|idee)$/i

/** Generic exploration / intention-to-explore placeholders (not banned globally). */
const GENERIC_INITIATIVE_PLACEHOLDER_RE =
  /\b(qualcosa\s+di\s+(interessante|sorprendente|nuovo|bello|curioso)|una\s+cosa\s+(interessante|sorprendente)|esplorare\s+qualcosa|trovare\s+un\s+argomento|parlare\s+di\s+qualcosa|scoprire\s+qualcosa|something\s+(interesting|surprising)|let'?s\s+explore\s+something|here'?s\s+something\s+interesting|explore\s+something(\s+interesting)?|buon\s+momento\s+per\s+(esplorare|parlare)|difficile\s+trovare\s+un\s+argomento|ti\s+dico\s+qualcosa\s+di\s+interessante)\b/i

/**
 * True when the reply uses generic "let's explore something" language that substitutes
 * for an actual concrete subject. Only meaningful when the plan requires initiative.
 *
 * @param {string} text
 * @returns {boolean}
 */
export function hasGenericInitiativePlaceholder(text) {
  const raw = asString(text).replace(/\s+/g, ' ').trim()
  if (!raw) return false
  return GENERIC_INITIATIVE_PLACEHOLDER_RE.test(raw)
}

/**
 * First substantive sentence after skipping a short social ack.
 * @param {string} raw
 * @returns {string}
 */
function firstSubstantiveSentence(raw) {
  const sentences = asString(raw)
    .split(/(?<=[.!?…])\s+/)
    .map((s) => s.trim())
    .filter(Boolean)
  let first = sentences[0] || ''
...
/**
 * Rough check that a concrete named noun (not "qualcosa" / "something") appears.
 * Not NER — structural only.
 *
 * @param {string} text
 * @returns {boolean}
 */
export function hasIdentifiableConcreteSubject(text) {
  const raw = asString(text).replace(/\s+/g, ' ').trim()
  if (!raw) return false
  const first = firstSubstantiveSentence(raw)

  // Explicit framing + following noun that is not a placeholder.
  const framed = first.match(
    /\b(?:ti\s+(?:lancio|propongo|racconto)(?:\s+una\s+curiosit[aà])?|parliamo\s+(?:di|dei|delle|del|della|degli)|partiamo\s+(?:da|con)|una\s+curiosit[aà]\s+(?:su|sul|sulla|di|dei|del)|let'?s\s+talk\s+about|here'?s\s+(?:a|one)|i'?ll\s+(?:share|tell|start\s+with)|one\s+(?:concrete|interesting)\s+(?:subject|topic|fact))\s*[:\-]?\s*(.+)$/i,
  )
  if (framed) {
    const rest = asString(framed[1])
    if (/\b(qualcosa|something|un\s+argomento|an?\s+topic)\b/i.test(rest) && !/\b(i|gli|le|il|la|lo|the)\s+[a-zà-ÿ]{3,}/i.test(rest)) {
      return false
    }
    // Named entity / determiner phrase after framing.
    if (
      /\b([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ-]{2,})(?:\s+[A-Za-zÀ-ÿ-]{2,}){0,4}\b/.test(rest) &&
      !GENERIC_INITIATIVE_PLACEHOLDER_RE.test(rest)
    ) {
      const nounGuess = rest.match(/\b(?:dei|delle|degli|di|del|della|about|the|i|gli|le|il|la|lo)?\s*([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ-]{2,})\b/i)
      const noun = nounGuess ? nounGuess[1] : ''
      if (noun && !VAGUE_SUBJECT_NOUN_RE.test(noun) && !/^(una|uno|un|a|an|curiosit)/i.test(noun)) {
        return true
      }
      // "curiosità: i polpi…" — colon introduces the subject.
      if (/:\s*\S{3,}/.test(rest) || /:\s*\S{3,}/.test(first)) return true
    }
  }

  // Determiner + concrete noun + assertive / existential verb.
  const det = first.match(
    /\b(gli|i|le|il|la|lo|the)\s+([A-Za-zÀ-ÿ]{3,}(?:\s+[A-Za-zÀ-ÿ-]{2,}){0,3})\s+(possono|sono|è|e['’]?|hanno|ha|può|can|are|is|have|has|was|were)\b/i,
  )
  if (det) {
    const head = det[2].split(/\s+/)[0] || ''
    if (head && !VAGUE_SUBJECT_NOUN_RE.test(head)) return true
  }

  // "…curiosità: SUBJECT …" anywhere in the reply.
  if (
    /curiosit[aà]\s*:\s*(?:i|gli|le|il|la|lo|the)?\s*([A-Za-zÀ-ÿ]{3,})/i.test(raw) ||
    /:\s*(?:i|gli|le|il|la|lo|the)\s+([A-Za-zÀ-ÿ]{3,})\s+\w+/i.test(raw)
  ) {
    return true
  }
...
export function responseEstablishesConcreteSubject(text) {
  ...
  // Placeholder exploration talk is not a concrete subject, even if long.
  if (hasGenericInitiativePlaceholder(raw) && !hasIdentifiableConcreteSubject(raw)) {
    return false
  }

  return hasIdentifiableConcreteSubject(raw)
}

/**
 * True when an established subject is actually developed with a substantive fact/claim.
 * Invitation-only ("Parliamo dei polpi!") is under-delivery.
 *
 * @param {string} text
 * @returns {boolean}
 */
export function hasSubstantiveSubjectDevelopment(text) {
  const raw = asString(text).replace(/\s+/g, ' ').trim()
  if (!raw) return false
  if (!responseEstablishesConcreteSubject(raw)) return false

  // Pure invitation / naming without a claim.
  if (
    /^(parliamo|partiamo|let'?s\s+talk|ti\s+propongo)\b[^:?.!]{0,80}[!?]?$/i.test(raw) &&
    !/:\s*\S.{8,}/.test(raw)
  ) {
    return false
  }
  ...
  const hasClaimVerb =
    /\b(hanno|sono|è|e['’]|ha|possono|può|puo|smettono|contiene|contengono|misura|vive|vivono|can|are|is|have|has|was|were|do|does|allows?|lets?|makes?)\b/i.test(
      raw,
    )
  // Need enough substance beyond naming the subject.
  if (!hasClaimVerb || words < 12) return false

  // After a colon / em dash, require a non-trivial clause.
  if (/[:—–]\s*\S/.test(raw)) {
    const after = raw.split(/[:—–]/).slice(1).join(':').trim()
    if (after.split(/\s+/).filter(Boolean).length >= 6 && hasClaimVerb) return true
  }

  // Multi-sentence development, or a long single sentence with a claim.
  const sentences = raw.split(/(?<=[.!?…])\s+/).filter(Boolean)
  if (sentences.length >= 2 && hasClaimVerb) return true
  if (words >= 16 && hasClaimVerb) return true
