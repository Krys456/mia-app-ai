...
queue.push(
  test('2. runConversation returns perception, conversationState, decision, plan, response', async () => {
...
queue.push(
  test('3. modules run in order Perception → Conversation State → Mind → Planner → Writer', async () => {
    /** @type {string[]} */
    const order = []
    ...
    assert(
      order.join('>').startsWith('perception>resume>conversationState>mind>planner>writer'),
