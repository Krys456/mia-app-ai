...
function createFakeWriterProvider(options = {}) {
  const {
    id = 'fake',
    mode = 'normal',
    text = 'Risposta sintetica del FakeProvider.',
...
queue.push(
  test('2. runConversation returns perception, decision, plan, response', async () => {
...
queue.push(
  test('3. modules run in order Perception → Mind → Planner → Writer', async () => {
    /** @type {string[]} */
    const order = []
    ...
    const pipeline = createPipeline({
      ...
      planFn: (input) => {
        ...
        return {
          ...
          constraints: ['strategy:connect', 'ask_question:no'],
          confidence: 0.9,
        }
      },
    })
    await pipeline.runConversation({ userMessage: 'Ciao!' })
    assertEqual(order.join('>'), 'perception>mind>planner>writer', 'order')
  }),
)
...
queue.push(
  test('5. Writer receives plan strategy in instructions', async () => {
    const { pipeline, fake } = makePipeline({ text: 'OK' })
    await pipeline.runConversation({ userMessage: 'Ciao!' })
    assertEqual(fake.__calls.length, 1, 'one writer call')
...
queue.push(
  test('6. Writer provider interchange via providerId', async () => {
    ...
    assert(/from-b/.test(res.response.text), 'text b')
    assertEqual(a.__calls.length, 0, 'a unused')
    assertEqual(b.__calls.length, 1, 'b used')
  }),
)
...
queue.push(
  test('26. plan objective matches decision goal family', async () => {
    const { pipeline } = makePipeline()
    const result = await pipeline.runConversation({ userMessage: 'Ciao!' })
    assertEqual(result.plan.objective, result.decision.goal, 'objective=goal')
  }),
)
...
queue.push(
  test('29. two pipelines with different providers do not share state', async () => {
    const { pipeline: p1, fake: f1 } = makePipeline({ id: 'p1', text: 'ONE' })
    const { pipeline: p2, fake: f2 } = makePipeline({ id: 'p2', text: 'TWO' })
    const r1 = await p1.runConversation({ userMessage: 'Ciao!' })
    const r2 = await p2.runConversation({ userMessage: 'Ciao!' })
    assert(/ONE/.test(r1.response.text), 'one')
    assert(/TWO/.test(r2.response.text), 'two')
    assertEqual(f1.__calls.length, 1, 'f1')
    assertEqual(f2.__calls.length, 1, 'f2')
  }),
)
