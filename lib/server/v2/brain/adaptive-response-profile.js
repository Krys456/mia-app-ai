/**
 * LAIfe V2 — Adaptive Response Profile (Phase 4)
 *
 * Derives HOW LAIfe should communicate this turn from Perception,
 * Conversation State, user message, and soft preference modes.
 *
...
export const ADAPTIVE_RESPONSE_PROFILE_VERSION = '1.0.0-adaptive-response'
...
const SHORT_CUE_RE =
  /\b(risposta\s+breve|breve\.?|in\s+breve|solo\s+la\s+risposta|concis[oa]|short\s+answer|be\s+brief|tl;?dr|in\s+una\s+riga)\b/i

const DETAILED_CUE_RE =
  /\b(approfondisci(\s+molto)?|in\s+dettaglio|dettagliat[oa]|spiegami\s+(bene|tutto|a\s+fondo)|detailed|go\s+deep|elaborate|più\s+dettagli)\b/i

const SIMPLE_CUE_RE =
  /\b(spiegamelo\s+facile|in\s+parole\s+semplici|come\s+se\s+fossi|eli5|simple\s+terms|senza\s+tecnichismi|non\s+troppo\s+tecnico)\b/i

const EXPERT_CUE_RE =
  /\b(tecnicamente|a\s+livello\s+esperto|in\s+profondità\s+tecnica|pwm|spwm|dead-?time|switching\s+loss|trifase|ponte\s+h|algoritmo|implementazione|architettura|derivata|integrale)\b/i

const EXCITEMENT_RE =
  /\b(ahah|haha|lol|lmao|assurdo|pazzesco|😂|🤣|🔥|wtf|incredibile)\b/i

const CALM_CUE_RE =
  /\b(con\s+calma|piano|tranquill[oa]|calmly|take\s+your\s+time)\b/i

const PROFESSIONAL_CUE_RE =
  /\b(professionale|formale|per\s+il\s+lavoro|business|corporate|email\s+formale|tono\s+serioso)\b/i

const BOREDOM_RE =
  /\b(mi\s+annoio|annoiato|bored|non\s+so\s+(di\s+)?cosa\s+(fare|parlare))\b/i
...
export function buildAdaptiveResponseProfile(input = {}) {
  ...
  if (goal === 'casual_exploration' || intent === 'boredom' || BOREDOM_RE.test(userText)) {
    warmth = 0.75
    formality = 0.15
    energy = 'high'
    depth = 'short'
  ...
  if (engagement === 'high') {
    ...
    // boredom already handled; otherwise calm
    if (!BOREDOM_RE.test(userText) && intent !== 'boredom') {
      energy = 'low'
  ...
  if (knowledge === 'beginner' || SIMPLE_CUE_RE.test(userText)) {
  ...
  // What-is questions → normal depth
  ...
  // Expert technical wording
  if (EXPERT_CUE_RE.test(userText) || (userText.length > 120 && /\b(e|and|,)\b/.test(userText) && EXPERT_CUE_RE.test(userText))) {
  ...
  if (SHORT_CUE_RE.test(userText)) {
    verbosity = 'minimal'
    if (DETAILED_CUE_RE.test(userText) === false) {
      // keep depth unless only brevity requested on a what-is
      if (WHAT_IS_RE.test(userText)) depth = 'normal'
    }
    signals.push('cue:explicit_short')
  }
  if (DETAILED_CUE_RE.test(userText)) {
  ...
  // Excitement / calm
  if (EXCITEMENT_RE.test(userText) || emotional === 'playful' || emotional === 'excited') {
  ...
  if (PROFESSIONAL_CUE_RE.test(userText)) {
  ...
  if (intent === 'curiosity' || (/\?\s*$/.test(userText) && !EXCITEMENT_RE.test(userText))) {
  ...
  if (baselines.depthBias > 0.15 && depth === 'normal' && !SHORT_CUE_RE.test(userText)) {
    depth = 'detailed'
  }
  ...
  const hardShift =
    SHORT_CUE_RE.test(userText) ||
    DETAILED_CUE_RE.test(userText) ||
    EXPERT_CUE_RE.test(userText) ||
    PROFESSIONAL_CUE_RE.test(userText) ||
    SIMPLE_CUE_RE.test(userText) ||
    EXCITEMENT_RE.test(userText)
  ...
  if (!useEmojis) {
    emojiPolicy = 'none'
    signals.push('emoji:disabled_by_pref')
  } else if (PROFESSIONAL_CUE_RE.test(userText) || formality >= 0.65 || technicality >= 0.75) {
    emojiPolicy = 'none'
  } else if (
    emotional === 'sad' ||
    emotional === 'anxious' ||
    ...
    emojiPolicy = 'none'
  } else if (energy === 'high' && formality < 0.35 && (mode === 'social' || EXCITEMENT_RE.test(userText))) {
