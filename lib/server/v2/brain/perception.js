...
export const PERCEPTION_VERSION = '2.0.0-perception'
...
export function detectLanguage(text) {
  ...
  const lex = [
    [
      'it',
      /\b(ciao|salve|buongiorno|buonasera|grazie|perch[eé]|cos['']è|come|non|sono|voglio|posso|aiuto|spiegami|continua|davvero|anche|però|oggi|domani)\b/gi,
    ],
    [
      'en',
      /\b(hello|hi|hey|thanks|please|what|why|how|the|and|you|i'm|i\s+am|help|explain|continue|really|today|tomorrow|because)\b/gi,
    ],
    [
      'es',
  ...
  if (bestScore <= 0) {
    ...
    if (/^[a-z0-9\s?',.!_-]+$/i.test(t) && /[a-z]{3,}/i.test(t)) return 'en'
    return 'unknown'
  }
...
function scoreIntents(text, ctx, signals) {
  ...
  if (
    /\b(noios[oa]|bored|annoiato|non\s+so\s+cosa\s+fare|nothing\s+to\s+do)\b/i.test(
      t,
    )
  ) {
    bump('boredom', 0.75, 'intent:boredom')
  }
