...
/**
 * @typedef {'accept_proposal'|'continue'|'passive_acknowledgement'|'stop'|'uncertain'|'change_topic'|'not_short_reply'} ShortReplyIntent
 */

/**
 * Writer-facing conversational move chosen by Planner from short-reply intent.
 * @typedef {'execute_pending_proposal'|'continue_topic'|'passive_acknowledgement'|'stop'|'clarify_uncertain'|'change_topic'|'default'} ConversationalMove
 */
...
const UNRESOLVED_PROPOSAL_RES = [
  /\b(possiamo|potremmo|proviamo|andiamo|facciamo|parliamo)\b/i,
  ...
  /\b(parliamo\s+di|let'?s\s+talk|curiosit[aà]\s+scientific)/i,
  /\?\s*$/,
]
...
export function inferPendingProposalType(assistantText) {
  ...
  if (/\b(esplor|possiamo|parliamo|scienz|spazio)\b/i.test(t)) return 'explore_topic'
