...
queue.push(
  test('63. Writer 3.2 version marker', async () => {
    assertEqual(WRITER_VERSION, '3.2.1-writer', 'version')
  }),
)
 ...
 */
function proposalFollowUpRequest(latestUser, previousAssistant, planOverrides = {}) {
  return sampleRequest({
    ...
    plan: samplePlan({
      objective: 'continue the proposal',
      writerBrief: {
        ...samplePlan().writerBrief,
        strategy: 'continue',
        need: 'continuation',
...
queue.push(
  test('68. ok after open proposal → ACCEPT_PROPOSAL, never force Va bene.', async () => {
    const proposal =
    ...
    const classified = classifyShortReply(req)
    assertEqual(classified.intent, 'accept_proposal', 'accept')
    assertEqual(shouldForceMinimalAck(req), false, 'no force')
    const instr = assembleInstructions(req)
    assert(/ACCEPT \/ CONTINUE/.test(instr), 'accept block')
...
queue.push(
  test('69. vai after explanation offer starts the explanation (accept_proposal)', async () => {
    const proposal = 'Posso spiegarti come funzionano i buchi neri.'
    const req = proposalFollowUpRequest('vai', proposal)
    assertEqual(classifyShortReply(req).intent, 'accept_proposal', 'accept')
    assertEqual(shouldForceMinimalAck(req), false, 'no force')
    assert(/ACCEPT \/ CONTINUE/.test(assembleInstructions(req)), 'instructions')
  }),
)

queue.push(
  test('70. sì after story offer accepts the proposal', async () => {
...
queue.push(
  test('71. Grazie, basta così → stop, do not continue', async () => {
...
queue.push(
  test('72. ok grazie with no pending proposal → brief ack allowed', async () => {
    const req = sampleRequest({
      messages: [
      ...
      plan: samplePlan({
        writerBrief: { ...samplePlan().writerBrief, continueTopic: false, strategy: 'close' },
      }),
    })
    const classified = classifyShortReply(req)
    assertEqual(classified.intent, 'stop', 'thanks done')
    // "ok grazie" is not MINIMAL_USER_RES alone — shouldForce may be false; brief close instr OK
    const instr = assembleInstructions(req)
    assert(/STOP \/ CLOSE/.test(instr) || classified.intent === 'stop', 'stop path')
  }),
)

queue.push(
  test('73. ok without history still forces short ack (passive)', async () => {
    const req = sampleRequest({ messages: [{ role: 'user', content: 'ok' }] })
    assertEqual(classifyShortReply(req).intent, 'passive_acknowledgement', 'passive')
    assertEqual(shouldForceMinimalAck(req), true, 'force')
    assert(/MINIMAL TURN/.test(assembleInstructions(req)), 'minimal block')
    assertEqual(cleanDraft('Troppe riflessioni inutili qui.', req), 'Va bene.', 'collapse')
  }),
)

queue.push(
  test('74. accept_proposal write does not collapse model output to Va bene.', async () => {
    const fake = createFakeWriterProvider({
