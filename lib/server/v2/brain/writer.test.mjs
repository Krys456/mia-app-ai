...
function sampleRewriteHints(overrides = {}) {
  return {
    mustFix: ['remove closing question'],
    improve: ['add a concrete observation'],
    keep: ['preserve factual content and planner objective'],
    targetScore: 0.8,
    ...overrides,
  }
}
...
queue.push(
  test('26. rewrite mode requires rewriteHints', async () => {
    const fake = createFakeWriterProvider()
    const writer = createWriter({ providers: { fake }, defaultProviderId: 'fake' })
    try {
      ...
      assertEqual(e.code, 'invalid_request', 'code')
      assert(/rewriteHints/.test(e.message), 'mentions rewriteHints')
    }
  }),
)

queue.push(
  test('27. rewrite mode uses rewriteHints exclusively (not rewriteBrief)', async () => {
    const fake = createFakeWriterProvider({ text: 'Refined reply.' })
    const writer = createWriter({ providers: { fake }, defaultProviderId: 'fake' })
    const hints = sampleRewriteHints({
      mustFix: ['Remove the closing question.'],
    })
    await writer.write(
      sampleRequest({
        mode: 'rewrite',
        rewriteHints: hints,
        rewriteBrief: 'THIS_OLD_BRIEF_MUST_NOT_APPEAR',
        previousDraft: 'Hello? What do you think?',
      }),
    )
    const instr = fake.__calls[0].instructions
    assert(/REWRITE MODE/.test(instr), 'rewrite mode')
    assert(/REWRITE CONTRACT/.test(instr), 'contract')
    assert(/Remove the closing question/.test(instr), 'mustFix hint')
    assert(/mustFix:/.test(instr), 'mustFix section')
    assert(/improve:/.test(instr), 'improve section')
    assert(/keep:/.test(instr), 'keep section')
    assert(/targetScore=0\.8/.test(instr), 'targetScore')
    assert(/Hello\? What do you think\?/.test(instr), 'previous draft')
    assert(!/THIS_OLD_BRIEF_MUST_NOT_APPEAR/.test(instr), 'no rewriteBrief')
  ...
  test('28. rewrite does not mutate plan object', async () => {
    ...
    await writer.write(
      sampleRequest({
        plan,
        mode: 'rewrite',
        rewriteHints: sampleRewriteHints({ mustFix: ['tighten'] }),
        previousDraft: 'draft',
      }),
    )
