...
function samplePlan(overrides = {}) {
  return {
    ...
    writerBrief: {
      ...
      shortReplyIntent: 'not_short_reply',
    },
    constraints: [
      'strategy:connect',
      'need:connection',
    ...
    const instr = assembleInstructions(
      sampleRequest({
        ...
        plan: samplePlan({
          ...
          writerBrief: {
            ...samplePlan().writerBrief,
            conversationalMove: 'passive_acknowledgement',
            forceMinimalAck: true,
            shortReplyIntent: 'passive_acknowledgement',
            coda: 'none',
          },
        }),
      }),
...
queue.push(
  test('49. cleanDraft collapses minimal turns to allowed ack', async () => {
    const req = sampleRequest({
      messages: [{ role: 'user', content: 'perfetto' }],
      plan: samplePlan({
        objective: 'passive_acknowledgement',
        writerBrief: {
          ...samplePlan().writerBrief,
...
queue.push(
  test('63. Writer 3.2 version marker', async () => {
    assertEqual(WRITER_VERSION, '3.2.2-writer', 'version')
  }),
)
 ...
 * @param {object} [briefOverrides]
 */
function proposalFollowUpRequest(latestUser, previousAssistant, briefOverrides = {}) {
  const classified = classifyShortReply({
    messages: [
  ...
  const move =
    classified.intent === 'accept_proposal'
      ? 'execute_pending_proposal'
      : classified.intent === 'continue'
        ? 'continue_topic'
        : classified.intent === 'passive_acknowledgement'
          ? 'passive_acknowledgement'
          : classified.intent === 'stop'
            ? 'stop'
  ...
  return sampleRequest({
    ...
    plan: samplePlan({
      objective: move === 'default' ? 'continue the proposal' : move,
      writerBrief: {
        ...samplePlan().writerBrief,
        strategy: 'continue',
        need: 'continuation',
        continueTopic: move !== 'stop' && move !== 'passive_acknowledgement',
        coda: 'insight',
        conversationalMove: move,
        shouldContinue: move === 'execute_pending_proposal' || move === 'continue_topic',
        shouldAskQuestion: false,
        activeTopic: 'curiosità scientifiche',
        pendingProposalAction: classified.pendingProposalType,
        forceMinimalAck: move === 'passive_acknowledgement',
        shortReplyIntent: classified.intent,
        ...briefOverrides,
      },
    }),
  })
...
queue.push(
  test('68. ok after open proposal → Planner move execute; Writer never forces Va bene.', async () => {
    const proposal =
      'Capisco, può succedere. Possiamo esplorare curiosità nel mondo, magari legate alla scienza o alla storia.'
    assert(hasUnresolvedConversationalProposal(proposal), 'proposal detected')
    const req = proposalFollowUpRequest('ok', proposal)
    assertEqual(req.plan.writerBrief.conversationalMove, 'execute_pending_proposal', 'move')
    assertEqual(shouldForceMinimalAck(req), false, 'no force')
    const instr = assembleInstructions(req)
    assert(/EXECUTE PLANNER MOVE/.test(instr), 'execute block')
    assert(!/MINIMAL TURN \(hard\)/.test(instr), 'no minimal hard')
    assertEqual(
      cleanDraft('I corvi sanno pianificare il futuro.', req),
      'I corvi sanno pianificare il futuro.',
      'keeps substance',
    )
    assert(
      !/^Va bene\.?$/i.test(finalizeWriterText('Va bene.', req).text),
      'dead-end ack rewritten under execute move',
    )
  }),
)

queue.push(
  test('69. vai after explanation offer → continue_topic; no force ack', async () => {
    const proposal = 'Posso spiegarti come funzionano i buchi neri.'
    const req = proposalFollowUpRequest('vai', proposal)
    assertEqual(req.plan.writerBrief.conversationalMove, 'continue_topic', 'continue')
    assertEqual(shouldForceMinimalAck(req), false, 'no force')
    assert(/EXECUTE PLANNER MOVE/.test(assembleInstructions(req)), 'instructions')
  }),
)

queue.push(
  test('70. sì after story offer → execute_pending_proposal', async () => {
    const proposal = 'Se vuoi posso raccontarti qualcosa di assurdo sul corpo umano.'
    const req = proposalFollowUpRequest('sì', proposal)
    assertEqual(req.plan.writerBrief.conversationalMove, 'execute_pending_proposal', 'execute')
    assertEqual(shouldForceMinimalAck(req), false, 'no force')
  }),
)

queue.push(
  test('71. Grazie, basta così → stop', async () => {
    const classified = classifyShortReply('Grazie, basta così.', 'Ecco fatto, ti ho spiegato i buchi neri.')
    assertEqual(classified.intent, 'stop', 'stop')
  }),
)

queue.push(
  test('72. ok grazie with no pending proposal → stop; brief close OK', async () => {
    const req = sampleRequest({
      messages: [
        { role: 'assistant', content: 'Ho sistemato il file.' },
      ...
      plan: samplePlan({
        ...
        writerBrief: {
          ...
          shouldAskQuestion: false,
          forceMinimalAck: false,
          shortReplyIntent: 'stop',
        },
      }),
    })
    assertEqual(classifyShortReply(req).intent, 'stop', 'thanks done')
    const instr = assembleInstructions(req)
    assert(/STOP \/ CLOSE/.test(instr), 'stop path')
  }),
)

queue.push(
  test('73. ok without Planner forceMinimalAck does NOT collapse (authority rule)', async () => {
    const req = sampleRequest({ messages: [{ role: 'user', content: 'ok' }] })
    // Without Planner contract force, Writer must not invent passive ack.
    assertEqual(shouldForceMinimalAck(req), false, 'no force without plan')
    const passive = sampleRequest({
      messages: [{ role: 'user', content: 'ok' }],
      plan: samplePlan({
        objective: 'passive_acknowledgement',
        writerBrief: {
    ...
    assertEqual(shouldForceMinimalAck(passive), true, 'force when Planner says so')
    assert(/MINIMAL TURN/.test(assembleInstructions(passive)), 'minimal block')
    assertEqual(cleanDraft('Troppe riflessioni inutili qui.', passive), 'Va bene.', 'collapse')
  }),
)

queue.push(
  test('74. execute_pending_proposal write does not collapse model output to Va bene.', async () => {
    const fake = createFakeWriterProvider({
      text: 'Allora ti dico una cosa: i corvi nascondono cibo e lo ritrovano mesi dopo.',
    })
    const writer = createWriter({ providers: { fake }, defaultProviderId: 'fake' })
    const res = await writer.write(
      proposalFollowUpRequest(
        'ok',
        'Possiamo parlare di qualche curiosità scientifica.',
      ),
    )
    assert(/corvi/i.test(res.text), `kept substance: ${res.text}`)
    assert(!/^Va bene\.?$/i.test(res.text), 'not dead ack')
  }),
)

queue.push(
  test('75. Writer fidelity: execute move never yields only Va bene.', async () => {
    const fake = createFakeWriterProvider({ text: 'Va bene.' })
    const writer = createWriter({ providers: { fake }, defaultProviderId: 'fake' })
    const res = await writer.write(
      sampleRequest({
        messages: [
        ...
        plan: samplePlan({
          ...
          writerBrief: {
            ...
            forceMinimalAck: false,
            shortReplyIntent: 'accept_proposal',
            coda: 'insight',
          },
        }),
      }),
    )
    assert(!/^Va bene\.?$/i.test(res.text), `not va bene: ${res.text}`)
    assert(!/\?\s*$/.test(res.text), 'no question')
  }),
)

queue.push(
  test('76. Identity rewrite brief preserves Planner contract fields', async () => {
    const req = sampleRequest({
      plan: samplePlan({
        objective: 'execute_pending_proposal',
        writerBrief: {
    ...
    assert(/PLANNER CONTRACT/.test(brief), 'contract block')
    assert(/execute_pending_proposal/.test(brief), 'move')
    assert(/corpo umano/.test(brief), 'topic')
