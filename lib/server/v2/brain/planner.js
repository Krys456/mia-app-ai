...
export const PLANNER_VERSION = '2.5.1-planner'
...
export function plan(input = {}) {
  ...
  // Lock living topic when engagement is medium+ (Director rule).
  if (
  ...
  // Keep Mind ask-flag for focus-based suppression, then let Director veto questions.
  ...
  if (!conversationState.shouldAskQuestion && coda === 'question') {
    let next = /** @type {WriterCoda} */ ('none')
    if (decision.initiative === 'one_insight') next = 'insight'
    else if (decision.initiative === 'one_spark') next = 'spark'
    else if (decision.initiative === 'one_direction') next = 'direction'
  ...
  const decisionForConstraints = {
    ...decision,
    shouldAskQuestion: conversationState.shouldAskQuestion && decision.shouldAskQuestion,
  }
  ...
  const writerBrief = {
    ...
    continueTopic:
      ...
      false,
    // Writer-facing: only the opaque sentence (or null). Full Resume stays out of Writer.
  ...
  const objective = directorObjectiveToPlanObjective(conversationState.objective, conversationState)
