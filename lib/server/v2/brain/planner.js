...
export const PLANNER_VERSION = '2.4.4-planner'
...
const MINIMAL_CONTINUATION_RE =
  /^(ok|okay|okey|okk|esatto|certo|perfetto|va\s*bene|d['’]?accordo|sure|exactly|right|perfect|alright|all\s*right|sì|si|yes|yep|yeah)[.!…]*$/i
...
export function evaluateConversationFocus(messages = [], decision = /** @type {any} */ ({}), perception = /** @type {any} */ ({})) {
  ...
  const topic = labelTopic(priorBlob || priorUser, priorTokens)
  /** @type {string[]} */
  const signals = []
  ...
  const minimal = MINIMAL_CONTINUATION_RE.test(latestUser.replace(/\s+/g, ' ').trim())
  ...
  if (minimal) {
    signals.push('minimal_ack_continuation', `overlap=${overlap.toFixed(2)}`)
    return {
      topic,
      status: 'active',
      confidence: 0.9,
...
export function plan(input = {}) {
  ...
  const { coda, suppressedClarification } = resolveCodaWithFocus(
    baseCoda,
    decision,
    conversationFocus,
  )
  ...
  const writerBrief = {
    ...
    continueTopic: decision.shouldContinueTopic || conversationFocus.status === 'active',
