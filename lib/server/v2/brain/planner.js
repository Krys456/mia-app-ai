...
import { isValidActiveTopic, isSocialSmallTalkTurn } from './topic-validation.js'
 ...
 * @property {import('./conversation-signals.js').ConversationSignals} [conversationSignals] Phase 6 turn cues (observations only)
 * @property {string} [userMessage] latest user text (for Adaptive Profile fallback)
...
function normalize(input) {
  ...
  const conversationSignals =
    raw.conversationSignals && typeof raw.conversationSignals === 'object'
      ? raw.conversationSignals
      : null

  const userMessage = asString(raw.userMessage)
...
function livingTopicFromHistory(messages) {
  ...
  for (let i = list.length - 1; i >= 0; i -= 1) {
    ...
    // Diagnostic only — invalid bag-of-words assistant topics are discarded.
    if (topic && isValidActiveTopic(topic, { source: 'assistant', allowAssistantDerived: false, messages: list })) {
      return { topic, source: 'last_substantive_assistant' }
    }
  }

  const { priorUser, priorBlob } = splitHistoryForFocus(messages)
  const priorTokens = focusTokens(priorUser || '')
  const topic = labelTopic(priorUser || priorBlob, priorTokens)
  if (topic && isValidActiveTopic(topic, { source: 'user', messages: list })) {
    return { topic, source: 'prior_user' }
  }
...
export function chooseDirectorObjective(decision, focus, momentum, continuationType, engagement) {
  ...
  // Social connect / greeting / how-are-you without a living topic → comfort, not teach/surprise facts.
  if (
  ...
  // Opening with no topic: warm presence, not an invented science fact.
  if ((focus?.status === 'none' || continuationType === 'opening') && !focus?.topic) {
    if (decision.strategy === 'connect' || decision.strategy === 'explore') {
      return decision.strategy === 'explore' ? 'brainstorm' : 'comfort'
    }
  }
  if (decision.initiative === 'one_insight' || decision.initiative === 'one_spark') {
    // Spark without a topic still must not invent a teach objective.
    if (!focus?.topic && decision.strategy === 'connect') return 'comfort'
    return 'surprise'
  }
  if (focus?.status === 'none' || continuationType === 'opening') return 'surprise'
  if (decision.strategy === 'continue') return 'deepen'
  // Default: do not invent teach when Mind explicitly said shouldTeach=false.
  if (decision.shouldTeach === false && decision.strategy === 'connect') return 'comfort'
  if (decision.shouldTeach === false) return 'deepen'
  return 'teach'
}
  ...
  switch (objective) {
    ...
    case 'comfort':
      return state?.activeTopic
        ? 'Comfort and stay with the user.'
        : 'Respond naturally to the social check-in.'
    case 'motivate':
      return 'Motivate with one concrete next spark.'
    case 'brainstorm':
      return state?.activeTopic
        ? 'Brainstorm one fresh angle on the active topic.'
        : 'Introduce one concrete interesting subject and start it.'
    case 'summarize':
...
export function directConversation(input = {}) {
  ...
  // Invalid / missing topic can never be locked.
  const situationTopic =
    situation?.activeTopic &&
    isValidActiveTopic(situation.activeTopic, {
  ...
  const focusTopicValid =
    ...
    isValidActiveTopic(focus.topic, {
      ...
      ? focus.topic
      : null
  if (!situationTopic && !focusTopicValid) {
    // No valid topic → never lock; allow social turns to float.
    shouldChangeTopic = false
  }

  const socialTurn = isSocialSmallTalkTurn({
    userMessage: latestUser,
    perception,
  })
  const focusForObjective = {
    ...focus,
    topic: situationTopic || focusTopicValid || null,
  }

  let objective = chooseDirectorObjective(
    decision,
    focusForObjective,
    momentum,
  ...
  // Pure social opening: never spark/surprise from residual initiative.
  if (
    socialTurn &&
    !situationTopic &&
    (decision.strategy === 'connect' || decision.strategy === 'support')
  ) {
    if (objective === 'surprise' || objective === 'teach' || objective === 'deepen') {
      objective = 'comfort'
    }
  }
  ...
  const shouldSurprise =
    !socialTurn &&
    (objective === 'surprise' ||
      (focus.status === 'none' && !decision.shouldComfort && Boolean(situationTopic)) ||
      (decision.initiative === 'one_spark' &&
        engagementInfo.engagement !== 'uncertain' &&
        Boolean(situationTopic)))

  const shouldExplain =
    !socialTurn &&
    decision.shouldTeach === true &&
    (objective === 'teach' ||
      objective === 'deepen' ||
  ...
  // Pure social check-in without a topic: be present, do not "lead" into invented curriculum.
  if (socialTurn && !situationTopic && decision.strategy === 'connect') {
    shouldLeadConversation = false
  }

  // Topic authority: Conversation State when present and valid.
  const authoritativeTopic = situationTopic
    ? situationTopic
    : shouldChangeTopic
      ? focusTokens(latestUser).slice(0, 6).join(' ') || focusTopicValid
      : focusTopicValid
  ...
  const state = {
    ...
    shouldExplain,
    shouldSurprise,
    shouldChangeTopic: Boolean(situationTopic) && shouldChangeTopic,
    signals: [
      ...
      situationTopic
        ? mediumOrHigher
          ? 'engagement_medium_plus_lock_topic'
          : 'engagement_low_flexible'
        : 'no_valid_topic_no_lock',
      situation ? 'topic_from_conversation_state' : 'topic_from_focus_compat',
      socialTurn ? 'social_small_talk' : 'content_turn',
    ],
  }
...
function buildOpening(decision) {
  ...
  if (strategy === 'explore') {
    return phase(
      'opening',
      'commit_direction',
      'Open by explicitly naming ONE concrete subject, then start explaining it — never begin as if the subject were already known.',
    )
  }
...
export function plan(input = {}) {
  ...
  const situation =
    ctx.conversationState && typeof ctx.conversationState === 'object'
      ...
      : buildConversationState({
          ...
          perception,
          decision,
          conversationSignals: ctx.conversationSignals,
        })
  ...
  if (situation.activeTopic && conversationFocus.status !== 'changed') {
    if (
      ...
      conversationFocus.topic = null
      if (conversationFocus.status === 'active') conversationFocus.status = 'none'
    }
  }
  if (situation.activeTopic && conversationFocus.status === 'changed') {
    if (
  ...
  // Topic lock is subordinate to topic validity — invalid topics can never be locked.
  directorState.activeTopic =
    situation.activeTopic &&
    isValidActiveTopic(situation.activeTopic, { source: 'state', allowAssistantDerived: true })
      ? situation.activeTopic
      : null
  ...
  if (
    ...
    conversationFocus.status = 'active'
    conversationFocus.topic = directorState.activeTopic
    conversationFocus.avoidClarification = true
  ...
  } else if (!directorState.activeTopic) {
    conversationFocus.topic = null
    if (conversationFocus.signals.includes('conversation_state_topic_lock')) {
      conversationFocus.signals = conversationFocus.signals.filter(
        (s) => s !== 'conversation_state_topic_lock',
      )
    }
  }

  // Never lock an invalid topic via conversation_state_topic_lock.
  if (!directorState.activeTopic) {
    directorState.shouldChangeTopic = false
    directorState.shouldExplain =
      directorState.shouldExplain && decision.shouldTeach === true
  }
  ...
  const refStatus = asString(situation.referenceResolution?.status || '')
  const refBand = asString(situation.referenceResolution?.confidenceBand || '')
  const repairNeedsClarify = Boolean(situation.repair?.requiresClarification)
  const refResolvedStrong =
    (refStatus === 'resolved' || refStatus === 'explicit') &&
    (refBand === 'high' || refBand === 'medium')

  // Strong resolution → never ask what the user meant.
  // Material ambiguity → allow one concise clarification when Mind/Director already lean ask.
  ...
  if (refResolvedStrong) {
    shouldAskQuestion = false
    suppressedClarification = true
  } else if (
    (refStatus === 'ambiguous' || repairNeedsClarify) &&
    situation.conversationPhase === 'recovering'
  ) {
    // Only clarify when distinction is material and short-reply does not block.
    shouldAskQuestion = !shortReplyBlocksQuestion
    if (shouldAskQuestion) coda = 'question'
  }

  // Greeting / pure social opening: never keep a spark coda that invents content.
  const socialOpeningEarly =
    isSocialSmallTalkTurn({
      userMessage: ctx.userMessage || '',
      perception,
    }) && !situation.activeTopic
  if (socialOpeningEarly && (coda === 'spark' || coda === 'direction' || coda === 'insight')) {
    coda = 'none'
  }
  ...
  const hasValidTopic = Boolean(
    (situation.activeTopic &&
      ...
      (directorState.activeTopic &&
        isValidActiveTopic(directorState.activeTopic, {
          source: 'state',
          allowAssistantDerived: true,
        })),
  )
  ...
  // Invariant helpers: pure social opening = social turn with no valid topic.
  const socialOpening = socialTurn && !hasValidTopic
  ...
  // Social small talk without a topic must not inherit continueTopic from engagement alone.
  const shouldContinueFinal =
    socialTurn && !hasValidTopic && decision.shouldContinueTopic === false
      ? false
      : shouldContinue
  ...
  const pendingProposalAction =
    conversationalMove === 'execute_pending_proposal' || conversationalMove === 'continue_topic'
      ...
      : null
  // Single topic authority — never publish an invalid Director topic.
  const activeTopic = hasValidTopic
    ? situation.activeTopic || directorState.activeTopic || null
    : null
  ...
  // Boredom + casual exploration: Planner may take initiative (Signals inform; Planner decides).
  if (
    ctx.conversationSignals?.affect?.boredom >= 0.55 &&
    (situation.activeGoal === 'casual_exploration' || decision.strategy === 'explore')
  ) {
    must.push(
      'User boredom signal is high: take one concrete conversational initiative; do not ask a menu of topics.',
    )
  }

  // Topic-establishing initiative: when starting a new subject with no living topic,
  // the first substantive sentence must explicitly name that subject (no in-medias-res).
  if (
    !hasValidTopic &&
    (decision.initiative === 'one_direction' ||
      decision.strategy === 'explore' ||
      perception.intent === 'boredom' ||
      situation.activeGoal === 'casual_exploration')
  ) {
    must.push(
      'Topic-establishing initiative: activeTopic is null — the first substantive sentence MUST explicitly name/establish ONE concrete subject before referring to it.',
    )
    must.push(
      'Self-contained opening: do not start in medias res (avoid "È un fenomeno…", "Questa cosa…", "This shows…", "The interesting thing is…") until the subject is named.',
    )
    mustNot.push(
      'Do not open as if a phenomenon/topic were already known to the user; name it first, then explain.',
    )
  }

  // Phase 7: reference / repair directives (HOW guidance; WHAT already in activeTopic).
  if (refResolvedStrong && situation.referenceResolution?.referent) {
    must.push(
      `Resolved referent: continue about "${situation.referenceResolution.referent}" without asking what the user meant.`,
    )
    mustNot.push('Do not ask clarifying questions about the resolved referent.')
  }
  if (situation.repair?.active && situation.repair.correctedReferent) {
    must.push(
      `Conversation repair: user corrected toward "${situation.repair.correctedReferent}". Continue naturally from that referent.`,
    )
    mustNot.push(
      'Do not expose internal terms like "reference resolved", "correction detected", or "updating topic".',
    )
    mustNot.push(
      'Do not over-apologize (avoid forced "Mi scuso" / "Hai ragione" / "Mi dispiace" on every correction).',
    )
    if (situation.repair.rejectedInterpretation) {
      mustNot.push(
        `Do not continue about the rejected interpretation "${situation.repair.rejectedInterpretation}".`,
      )
    }
  } else if (refStatus === 'ambiguous' && shouldAskQuestion) {
    const amb = Array.isArray(situation.references?.ambiguous)
      ? situation.references.ambiguous.map((r) => r?.referent).filter(Boolean).slice(0, 2)
      : []
    if (amb.length === 2) {
      must.push(
        `Reference is ambiguous between "${amb[0]}" and "${amb[1]}". Ask ONE concise clarification naming both.`,
      )
    } else {
      must.push('Reference is ambiguous. Ask ONE concise clarification; do not guess.')
    }
  }

  // Greeting / pure social opening invariants (HOW guidance).
  if (socialOpening) {
    must.push('Respond as a natural social greeting/check-in; no topic is required.')
    mustNot.push(
      'Do not invent a referential continuation (e.g. proteggerla/quello/this/that) with no antecedent.',
    )
    mustNot.push('Do not invent an intellectual topic merely to satisfy initiative or spark.')
    mustNot.push('Do not teach, explain, or deepen a phantom topic on this pure social turn.')
  }
  ...
  const writerBrief = {
    ...
    // Invariant: Mind shouldTeach=false cannot become teaching=true on pure social turns.
    teaching: Boolean(decision.shouldTeach === true && !(socialTurn && !hasValidTopic)),
    comfort: decision.shouldComfort || (socialTurn && !hasValidTopic),
    challenge: decision.shouldComfort ? false : decision.shouldChallenge,
    // Invariant: shouldContinueTopic=false + activeTopic=null ⇒ continueTopic=false
    continueTopic:
      shouldContinueFinal &&
      conversationalMove !== 'stop' &&
      !(decision.shouldContinueTopic === false && !hasValidTopic),
  ...
  // activeTopic=null ⇒ topicLock=false (never emit lock without a valid topic).
  if (activeTopic && !directorState.shouldChangeTopic) {
    constraints.push('director_lock_topic:yes')
  } else {
    constraints.push('director_lock_topic:no')
  }
  ...
  if (socialOpening) constraints.push('pure_social_opening:yes')
  ...
  // Pure social turns must remain social — never teach/curiosity curriculum.
  if (socialOpening) {
