...
export const PLANNER_VERSION = '2.5.2-planner'
 ...
 * @property {number} confidence
 * @property {ConversationFocus} conversationFocus
 * @property {ConversationMomentum} conversationMomentum
 ...
 * @property {ConversationState} conversationState Conversation Director state (before Writer brief)
 * @property {ShortReplyState} [shortReplyState] authoritative short-reply interpretation (Phase 1)
 */
...
/**
 * Internal Conversation Director state — built before Writer brief.
 ...
 * @property {object} [conversationResume] output of Conversation Resume Engine (optional)
...
/**
 * Conversation Director — produces internal conversationState before Writer brief.
...
/**
 * Evaluate the living conversation focus from current chat history only.
 * Pure. No durable memory. No LLM.
 * Conversation Cues are signals that steer status; semantic overlap still decides hard cases.
 *
 * @param {ChatMessage[]} [messages]
 * @param {ReturnType<typeof normalize>['decision']} [decision]
...
/**
 * Infer conversational momentum from current chat history only.
 * Pure. No durable memory. No LLM.
 *
 * @param {ChatMessage[]} [messages]
 * @param {ReturnType<typeof normalize>['decision']} [decision]
...
/**
 * Build a concrete plan from Perception + Mind Decision (+ optional chat history).
 * Pure. No I/O. No new Mind decisions.
 * conversationFocus / conversationMomentum / Conversation Resume use current chat only.
 * Planner decides whether to expose resumeSentence to Writer.
 *
 * @param {PlannerInput} [input]
 * @returns {PlannerPlan}
 */
export function plan(input = {}) {
  ...
  // Phase 1: one authoritative short-reply interpretation (before Writer brief).
  const shortReplyState = interpretShortReply({ messages })
  ...
  // Conversation Director — internal state before any Writer brief construction.
  const conversationState = directConversation({
    messages,
    decision,
    perception,
    conversationFocus,
  ...
  // Short-reply accept/continue locks topic onto the living thread.
  if (
    shortReplyState.intent === 'accept_proposal' ||
    shortReplyState.intent === 'continue' ||
    shortReplyState.intent === 'uncertain'
  ) {
    conversationState.shouldChangeTopic = false
    conversationState.shouldLeadConversation = true
    conversationState.shouldAskQuestion = false
    if (shortReplyState.intent === 'uncertain') {
      conversationState.shouldExplain = true
    } else {
      conversationState.shouldExplain = true
      conversationState.shouldLeadConversation = true
    }
  ...
  if (shortReplyState.intent === 'stop') {
    conversationState.shouldAskQuestion = false
    conversationState.shouldLeadConversation = false
  }
  ...
  // Lock living topic when engagement is medium+ (Director rule) or short-reply continues.
  if (
    !conversationState.shouldChangeTopic &&
    conversationState.activeTopic &&
    conversationFocus.status !== 'none'
  ) {
    conversationFocus.status = 'active'
    conversationFocus.topic = conversationState.activeTopic
    conversationFocus.avoidClarification = true
    if (!conversationFocus.signals.includes('director_topic_lock')) {
      conversationFocus.signals = [...conversationFocus.signals, 'director_topic_lock']
    }
  }
  ...
  const shortReplyBlocksQuestion =
    ...
    shortReplyState.intent === 'uncertain'
  if ((!conversationState.shouldAskQuestion || shortReplyBlocksQuestion) && coda === 'question') {
    let next = /** @type {WriterCoda} */ ('none')
    if (
      shortReplyState.intent === 'accept_proposal' ||
      shortReplyState.intent === 'continue'
  ...
  const shouldAskQuestion =
    Boolean(conversationState.shouldAskQuestion && decision.shouldAskQuestion) &&
  ...
  applyDirectorDirectives(must, mustNot, conversationState)
  applyShortReplyDirectives(
    must,
    mustNot,
    shortReplyState,
    conversationState.activeTopic || conversationFocus.topic,
  )
  ...
  const shouldContinue =
    ...
    (!conversationState.shouldChangeTopic &&
      (decision.shouldContinueTopic ||
        conversationFocus.status === 'active' ||
        isEngagementMediumOrHigher(conversationState.userEngagement)))
  const forceMinimalAck = moveRequiresMinimalAck(conversationalMove)
  const pendingProposalAction =
    conversationalMove === 'execute_pending_proposal' || conversationalMove === 'continue_topic'
      ? shortReplyState.pendingProposalType
      : null
  const activeTopic = conversationState.activeTopic || conversationFocus.topic || null
  ...
  const writerBrief = {
    ...
    depth:
      forceMinimalAck || conversationalMove === 'stop'
        ? 'minimal'
    ...
    teaching: decision.shouldTeach || conversationState.shouldExplain,
  ...
  constraints.push(`director_objective:${conversationState.objective}`)
  constraints.push(`user_engagement:${conversationState.userEngagement}`)
  constraints.push(`conversational_move:${conversationalMove}`)
  constraints.push(`short_reply_intent:${shortReplyState.intent}`)
  if (!conversationState.shouldChangeTopic) constraints.push('director_lock_topic:yes')
  if (conversationState.shouldLeadConversation) constraints.push('director_lead:yes')
  if (forceMinimalAck) constraints.push('force_minimal_ack:yes')
  if (!shouldAskQuestion) constraints.push('hard:no_question')

  let objective = directorObjectiveToPlanObjective(conversationState.objective, conversationState)
...
export function isPlannerPlan(value) {
  ...
  const director = v.conversationState
  return (
