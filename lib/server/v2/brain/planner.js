...
export function directConversation(input = {}) {
  ...
  // Precedence: small talk + Mind negatives + no topic → never invent teach/deepen/lock.
  if (
    socialTurn &&
    decision.shouldTeach === false &&
    decision.shouldContinueTopic === false &&
    !situationTopic
  ) {
    if (objective === 'teach' || objective === 'deepen') {
      objective = 'comfort'
    }
  }
  ...
  const shouldSurprise =
    objective === 'surprise' ||
    (focus.status === 'none' && !decision.shouldComfort && !socialTurn && Boolean(situationTopic)) ||
...
export function plan(input = {}) {
  ...
  const writerBrief = {
    ...
    teaching: Boolean(
      (decision.shouldTeach || directorState.shouldExplain) &&
        !(socialTurn && !hasValidTopic && decision.shouldTeach === false),
    ),
    comfort: decision.shouldComfort || (socialTurn && !hasValidTopic),
    challenge: decision.shouldComfort ? false : decision.shouldChallenge,
    continueTopic: shouldContinueFinal && conversationalMove !== 'stop',
    resumeSentence: conversationResumeUsage.used ? conversationResumeUsage.resumeSentence : null,
    conversationalMove,
    shouldContinue: shouldContinueFinal && conversationalMove !== 'stop',
    shouldAskQuestion,
    activeTopic,
