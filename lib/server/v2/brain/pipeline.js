 ...
 *
 * Orchestrates:
 *   Perception → Conversation Signals → Conversation State → Mind → Planner → Writer
 *   → Contract Evaluator (WHAT + adaptive HOW + referent fidelity) → State Transition
 *
 * Reference Resolution runs as a State helper (short-range grounding), not a Planner.
 *
...
export const PIPELINE_VERSION = '2.7.0-pipeline'
 ...
 * @property {typeof deriveConversationSignals} [conversationSignalsFn]
 ...
 * @property {import('./conversation-signals.js').ConversationSignals} [conversationSignals]
...
export function createPipeline(config = {}) {
  ...
  const conversationSignalsFn = config.conversationSignalsFn || deriveConversationSignals
  ...
  async function runConversation(input = {}) {
    ...
    // 2. Conversation Signals (WHAT SIGNALS ARE PRESENT)
    ...
    const conversationSignals = conversationSignalsFn({
      userMessage,
      messages,
      perception,
    ...
    // 3. Conversation State (WHAT IS CURRENTLY TRUE)
    ...
    // 7. Optional Contract Evaluator — at most one constrained rewrite (HOW only).
    ...
    // 8. State Transition / Persistence — only after successful delivery.
