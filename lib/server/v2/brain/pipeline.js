 ...
 *
 * Orchestrates: Perception → Conversation State → Mind → Planner → Writer
 * (Conversation Resume remains a compat helper feeding State / Planner; not a topic authority.)
...
export const PIPELINE_VERSION = '2.2.0-pipeline'
 ...
 * @property {import('./conversation-state.js').ConversationState} [previousConversationState]
 ...
 * @property {import('./conversation-state.js').ConversationState} conversationState
 * @property {object} decision
 * @property {object} conversationResume compat helper output (not topic authority)
...
export function createPipeline(config = {}) {
  ...
  async function runConversation(input = {}) {
    ...
    // 2. Conversation State (WHAT IS CURRENTLY TRUE) — before Mind
    const conversationResume = resumeFn({ messages })
    const conversationState = conversationStateFn({
      messages,
      perception,
      previousState: input.previousConversationState || null,
    })

    // 3. Mind (strategy only — consumes Conversation State facts, does not override them)
    ...
    // 4. Planner (receives Conversation State as primary conversational context)
    ...
    // 5. Writer — Planner contract remains authoritative; State is linguistic grounding only
    let response
    try {
