...
function assertShape(p) {
  ...
  assert(typeof p.conversationState.objective === 'string', 'director objective')
  assert(typeof p.conversationState.userEngagement === 'string', 'director engagement')
  assert(typeof p.conversationState.expectedNextReaction === 'string', 'director expected reaction')
  assert(typeof p.conversationState.shouldChangeTopic === 'boolean', 'director shouldChangeTopic')
...
test('16. objective is a conversation move (Director), not a Mind goal code when directing', () => {
  ...
  assert(
    DIRECTOR_OBJECTIVES.includes(/** @type {any} */ (p.conversationState.objective)),
    `director objective=${p.conversationState.objective}`,
  )
  assert(
...
test('79f. plan emits conversationState and blocks disconnected continuations', () => {
  ...
  assertEqual(p.conversationState.shouldChangeTopic, false, 'no change')
  assert(p.writerBrief.continueTopic, 'continueTopic')
  assert(
...
test('79g. Director prefers surprise/open with unexpected fact on cold open', () => {
  ...
  assert(
    p.conversationState.shouldSurprise || p.conversationState.objective === 'surprise',
    'surprise path',
  )
  assert(
    /unexpected fact|curiosity/i.test(p.objective) || p.conversationState.objective === 'surprise',
    `objective=${p.objective}`,
  )
})
...
test('80b. ok after scientific proposal keeps topic and forbids dead-end ack', () => {
  ...
  assertEqual(p.conversationFocus.status, 'active', 'active')
  assert(p.writerBrief.continueTopic, 'continueTopic')
  assertEqual(p.conversationState.shouldChangeTopic, false, 'no topic change')
  assert(
