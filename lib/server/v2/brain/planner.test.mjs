...
test('79g. Director prefers surprise/open with unexpected fact on cold open', () => {
  const p = plan({
    perception: { language: 'en', intent: 'greeting', confidence: 0.7 },
    decision: {
      need: 'connection',
      goal: 'connect__need_connection',
      strategy: 'connect',
  ...
  assert(
    p.directorState.shouldSurprise || p.directorState.objective === 'surprise',
    'surprise path',
  )
  assert(
    /unexpected fact|curiosity/i.test(p.objective) || p.directorState.objective === 'surprise',
    `objective=${p.objective}`,
  )
})
