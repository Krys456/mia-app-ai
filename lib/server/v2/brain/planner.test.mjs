...
import {
  ...
  isEngagementMediumOrHigher,
  chooseDirectorObjective,
  directorObjectiveToPlanObjective,
  decideConversationResumeUsage,
  matchConversationCues,
  isEncouragementContinuation,
  CONTINUATION_CUES,
  TOPIC_CHANGE_CUES,
  ENCOURAGEMENT_CONTINUATION_EXAMPLES,
  DIRECTOR_OBJECTIVES,
...
function assertShape(p) {
  ...
  assert(p.conversationState && typeof p.conversationState === 'object', 'conversationState')
  assert(p.directorState && typeof p.directorState === 'object', 'directorState')
  assert(typeof p.directorState.objective === 'string', 'director objective')
  assert(typeof p.directorState.userEngagement === 'string', 'director engagement')
  assert(typeof p.directorState.expectedNextReaction === 'string', 'director expected reaction')
  assert(typeof p.directorState.shouldChangeTopic === 'boolean', 'director shouldChangeTopic')
  assert(p.shortReplyState && typeof p.shortReplyState === 'object', 'shortReplyState')
  assert(typeof p.writerBrief.conversationalMove === 'string', 'conversationalMove')
  assert(typeof p.writerBrief.shouldAskQuestion === 'boolean', 'shouldAskQuestion contract')
  assert(typeof p.writerBrief.forceMinimalAck === 'boolean', 'forceMinimalAck')
  assert(
...
test('2. greeting/connect → warm_presence opening, no spark coda on pure social', () => {
  const p = run(
    {
      need: 'connection',
  ...
  // Pure greeting: no spark/surprise coda (greeting invariants 1–3).
  assertEqual(p.writerBrief.coda, 'none', 'coda none')
  assertEqual(p.writerBrief.teaching, false, 'no teaching')
  assertEqual(p.writerBrief.continueTopic, false, 'no continue')
  assertEqual(p.writerBrief.activeTopic, null, 'no topic')
  assert(p.constraints.includes('director_lock_topic:no'), 'no topic lock')
...
test('16. objective is a conversation move (Director), not a Mind goal code when directing', () => {
  const p = run({
  ...
  // No history → Director still picks a conversation move (surprise/teach/…)
  assert(typeof p.objective === 'string' && p.objective.length > 0, 'objective string')
  assert(p.conversationState, 'conversationState')
  assert(p.directorState, 'directorState')
  assert(
    DIRECTOR_OBJECTIVES.includes(/** @type {any} */ (p.directorState.objective)),
    `director objective=${p.directorState.objective}`,
  )
  assert(
    !/^continue__need_/.test(p.objective),
    'plan.objective should not be a raw Mind goal code under Director',
  )
})
...
test('43i. matchConversationCues covers both cue sets', () => {
  ...
  assertEqual(CONTINUATION_CUES.length, 16, 'continuation set size')
...
/**
 * Shared history: scientific curiosity thread, then a short encouragement.
 * @param {string} latestUser
 */
function scientificCuriosityMessages(latestUser) {
  return [
    { role: 'user', content: 'Parliamo di qualcosa.' },
    {
      role: 'assistant',
      content: 'Parliamo di curiosità scientifiche: i corvi sanno pianificare.',
    },
    { role: 'user', content: 'Ok' },
 ...
 * @param {string} latestUser
 */
function planEncouragementTurn(latestUser) {
  return plan({
    perception: {
      language: 'it',
      intent: 'continuation',
    ...
    messages: scientificCuriosityMessages(latestUser),
  })
}

test('78a. encouragement "ok" keeps previous scientific topic', () => {
  assert(isEncouragementContinuation('ok'), 'ok is encouragement')
  const p = planEncouragementTurn('ok')
  assertEqual(p.conversationFocus.status, 'active', 'active')
  assert(
    /curiosit|scientific|corvi|pianificare/i.test(String(p.conversationFocus.topic || '')),
    `topic kept: ${p.conversationFocus.topic}`,
  )
  assert(p.writerBrief.continueTopic, 'continueTopic')
  assert(
    /Deepen the previous explanation|Connect to the previous assistant|execute_pending_proposal|continue_topic/i.test(
      p.objective,
    ),
    `objective=${p.objective}`,
  )
  assertEqual(p.shortReplyState.intent, 'accept_proposal', 'accept open scientific proposal')
  assert(
    p.writerBrief.mustNot.some((m) => /isolated fact disconnected/i.test(m)),
    'no isolated facts',
  )
})

test('78b. encouragement "mh" keeps previous scientific topic', () => {
  assert(isEncouragementContinuation('mh'), 'mh is encouragement')
  const p = planEncouragementTurn('mh')
  assertEqual(p.conversationFocus.status, 'active', 'active')
  assert(
    /curiosit|scientific|corvi|pianificare/i.test(String(p.conversationFocus.topic || '')),
    `topic kept: ${p.conversationFocus.topic}`,
  ...
  assert(
    p.conversationFocus.signals.some((s) => /encouragement_continuation|minimal_ack/.test(s)),
    'encouragement signal',
  )
  assert(
    /Deepen the previous explanation|Connect to the previous assistant|clarify_uncertain/i.test(
      p.objective,
    ),
    `objective=${p.objective}`,
  )
  assertEqual(p.shortReplyState.intent, 'uncertain', 'uncertain')
})

test('78c. encouragement "continua" keeps previous scientific topic', () => {
  assert(isEncouragementContinuation('continua'), 'continua is encouragement')
  const p = planEncouragementTurn('continua')
  assertEqual(p.conversationFocus.status, 'active', 'active')
  assert(
    /curiosit|scientific|corvi|pianificare/i.test(String(p.conversationFocus.topic || '')),
    `topic kept: ${p.conversationFocus.topic}`,
  ...
  assert(p.writerBrief.continueTopic, 'continueTopic')
  assert(
    p.writerBrief.must.some((m) => /previous assistant sentence|current topic|living conversation focus/i.test(m)),
    'must stay on thread',
  )
})

test('78d. encouragement "vai" keeps previous scientific topic', () => {
  assert(isEncouragementContinuation('vai'), 'vai is encouragement')
  assert(ENCOURAGEMENT_CONTINUATION_EXAMPLES.includes('vai'), 'listed')
  const p = planEncouragementTurn('vai')
  assertEqual(p.conversationFocus.status, 'active', 'active')
  assert(
    /curiosit|scientific|corvi|pianificare/i.test(String(p.conversationFocus.topic || '')),
    `topic kept: ${p.conversationFocus.topic}`,
  )
  assertEqual(p.conversationFocus.avoidClarification, true, 'avoid clarify')
})

test('78e. encouragement "interessante" keeps previous scientific topic', () => {
  assert(isEncouragementContinuation('interessante'), 'interessante is encouragement')
  const p = planEncouragementTurn('interessante')
  assertEqual(p.conversationFocus.status, 'active', 'active')
  assert(
    /curiosit|scientific|corvi|pianificare/i.test(String(p.conversationFocus.topic || '')),
    `topic kept: ${p.conversationFocus.topic}`,
  ...
  assert(
    /Deepen the previous explanation|Connect to the previous assistant|Create curiosity|Introduce an unexpected/i.test(
      p.objective,
    ),
    `conversation objective=${p.objective}`,
  )
  assert(
    p.writerBrief.mustNot.some((m) => /isolated fact disconnected/i.test(m)),
    'block disconnected facts',
  )
})

test('78f. buildConversationObjective prefers conversation moves on active encouragement', () => {
  const focus = evaluateConversationFocus(
    scientificCuriosityMessages('mh'),
    { shouldContinueTopic: true, strategy: 'continue' },
    { intent: 'continuation' },
  )
  const objective = buildConversationObjective(
    {
      need: 'continuation',
      goal: 'continue__need_continuation__one_insight',
      strategy: 'continue',
    ...
    focus,
    { kind: 'learning', confidence: 0.7, signals: [], scores: {} },
    scientificCuriosityMessages('mh'),
  )
  assertEqual(objective, 'Deepen the previous explanation.', 'deepen')
})

test('79a. Conversation Director: ok → engaged, never change topic', () => {
  const messages = scientificCuriosityMessages('ok')
  const state = directConversation({
    messages,
    decision: {
      strategy: 'continue',
      need: 'continuation',
    ...
    perception: { intent: 'continuation', language: 'it' },
  })
  assertEqual(estimateUserEngagement('ok', { status: 'active', topic: 'x', confidence: 1, signals: [], avoidClarification: true }).engagement, 'engaged', 'ok engaged')
  assertEqual(state.userEngagement, 'engaged', 'state engaged')
  assertEqual(state.shouldChangeTopic, false, 'no topic change')
  assert(isEngagementMediumOrHigher(state.userEngagement), 'medium+')
  assert(DIRECTOR_OBJECTIVES.includes(state.objective), 'valid objective')
  assert(state.expectedNextReaction.length > 0, 'expected reaction')
})

test('79b. Conversation Director: mh → uncertain, still lock topic', () => {
  const state = directConversation({
    messages: scientificCuriosityMessages('mh'),
    decision: {
      strategy: 'continue',
      need: 'continuation',
      shouldContinueTopic: true,
  ...
  assertEqual(state.userEngagement, 'uncertain', 'uncertain')
  assertEqual(state.continuationType, 'uncertain_signal', 'uncertain_signal')
  assertEqual(state.shouldChangeTopic, false, 'lock topic')
  assertEqual(state.shouldAskQuestion, false, 'no question while continuing')
  assertEqual(state.shouldLeadConversation, true, 'lead')
  assert(/curiosit|scientific|corvi/i.test(String(state.activeTopic || '')), `topic=${state.activeTopic}`)
})

test('79c. Conversation Director: interesting → engaged', () => {
  const eng = estimateUserEngagement('interesting', {
    status: 'active',
    topic: 'space',
    confidence: 1,
    signals: [],
  ...
  assertEqual(eng.engagement, 'engaged', 'engaged')
  assert(isEngagementMediumOrHigher(eng.engagement), 'medium+')
})

test('79d. Conversation Director: why? → high curiosity', () => {
  const eng = estimateUserEngagement('why?', {
    status: 'active',
    topic: 'space',
    confidence: 1,
    signals: [],
  ...
  assertEqual(eng.engagement, 'high', 'high')
  assertEqual(eng.continuationType, 'curiosity_question', 'curiosity')
  assert(eng.curiosityLevel >= 0.9, 'curiosity level')
})

test('79e. Conversation Director: continue → maximum engagement', () => {
  const eng = estimateUserEngagement('continue', {
    status: 'active',
    topic: 'space',
    confidence: 1,
    signals: [],
  ...
  // "continue" may also match via EXPLICIT_CONTINUE English keep going — ensure maximum
  const eng2 = estimateUserEngagement('continua', {
    status: 'active',
    topic: 'space',
    confidence: 1,
    signals: [],
  ...
  assertEqual(eng2.engagement, 'maximum', 'continua maximum')
  assertEqual(eng2.continuationType, 'explicit_continue', 'explicit continue')
})

test('79f. plan emits conversationState and blocks disconnected continuations', () => {
  const p = planEncouragementTurn('mh')
  assert(p.conversationState, 'has state')
  assertEqual(p.directorState.shouldChangeTopic, false, 'no change')
  assert(p.writerBrief.continueTopic, 'continueTopic')
  assert(
    p.writerBrief.mustNot.some((m) => /disconnected continuation|isolated fact|change topic/i.test(m)),
    'mustNot disconnect',
  )
  assert(
    p.writerBrief.must.some((m) => /Conversation Director objective/i.test(m)),
    'director must',
  )
  assert(
    /Deepen the previous explanation|Open with an unexpected|Create curiosity|Connect to the previous|clarify_uncertain/i.test(
      p.objective,
    ),
    `plan objective=${p.objective}`,
  )
  assert(isPlannerPlan(p), 'isPlannerPlan includes conversationState')
})

test('79g. Director prefers social presence on cold greeting (no invented topic facts)', () => {
  const p = plan({
    perception: { language: 'en', intent: 'greeting', confidence: 0.7, socialIntent: 'greeting' },
    decision: {
      need: 'connection',
      goal: 'connect__need_connection',
      strategy: 'connect',
  ...
  // Cold greeting must NOT invent teach / unexpected science facts (phantom topic contamination).
  assert(p.directorState.objective !== 'teach', `objective=${p.directorState.objective}`)
  assertEqual(p.writerBrief.teaching, false, 'no teaching')
  assertEqual(p.writerBrief.activeTopic, null, 'no phantom topic')
  assert(
    p.directorState.objective === 'comfort' ||
      /social check-in|Connect|presence|warm/i.test(String(p.objective)),
    `objective=${p.objective} dir=${p.directorState.objective}`,
  )
})

test('79h. chooseDirectorObjective + directorObjectiveToPlanObjective cover the set', () => {
  for (const obj of DIRECTOR_OBJECTIVES) {
    const text = directorObjectiveToPlanObjective(obj, {
      activeTopic: 'science',
      objective: obj,
      curiosityLevel: 0.7,
      userEngagement: 'engaged',
      continuationType: 'encouragement',
      expectedNextReaction: 'User becomes curious.',
      shouldLeadConversation: true,
      shouldAskQuestion: false,
      shouldExplain: true,
      shouldSurprise: obj === 'surprise',
      shouldChangeTopic: false,
      signals: [],
    })
    assert(typeof text === 'string' && text.length > 0, obj)
  }
  const chosen = chooseDirectorObjective(
    {
      strategy: 'continue',
      shouldTeach: false,
      shouldComfort: false,
    ...
    { status: 'active', topic: 'x', confidence: 1, signals: [], avoidClarification: true },
    { kind: 'learning', confidence: 0.8, signals: [], scores: {} },
    'encouragement',
    'engaged',
  )
  assertEqual(chosen, 'deepen', 'deepen on encouragement+learning')
})

test('80a. boredom plan takes initiative, bans topic menus', () => {
  const p = plan({
    perception: {
      language: 'it',
      intent: 'boredom',
    ...
    decision: {
      need: 'direction',
      goal: 'explore__need_direction__one_direction',
      strategy: 'explore',
      initiative: 'one_direction',
      emotionalTone: 'warm',
      responseDepth: 'light',
  ...
  assert(
    p.writerBrief.must.some((m) => /initiative|concrete interesting subject/i.test(m)),
    'must initiative',
  )
  assert(
    p.writerBrief.mustNot.some((m) => /topic menu|di cosa vuoi parlare/i.test(m)),
    'ban menu',
  )
  assert(
...
test('80b. ok after scientific proposal keeps topic and forbids dead-end ack', () => {
  const p = planEncouragementTurn('ok')
  assertEqual(p.conversationFocus.status, 'active', 'active')
  assert(p.writerBrief.continueTopic, 'continueTopic')
  assertEqual(p.directorState.shouldChangeTopic, false, 'no topic change')
  assert(
    p.writerBrief.mustNot.some((m) => /Va bene|dead-end|bare acknowledgement/i.test(m)),
    'forbid dead ack',
  )
  assert(
    /curiosit|scientific|corvi|pianificare/i.test(String(p.conversationState.activeTopic || p.conversationFocus.topic || '')),
    `topic coherent: ${p.conversationState.activeTopic}`,
  )
})

test('81a. Phase1: pending proposal + ok → accept_proposal / execute move', () => {
  const p = plan({
    perception: {
      language: 'it',
      intent: 'continuation',
    ...
    decision: {
      need: 'continuation',
      goal: 'continue__need_continuation',
      strategy: 'continue',
      initiative: 'one_insight',
      emotionalTone: 'warm',
      responseDepth: 'light',
    ...
    messages: [
      ...
      {
        role: 'assistant',
        content: 'Posso raccontarti una curiosità sullo spazio.',
      },
      { role: 'user', content: 'ok' },
    ],
  })
  assertEqual(p.shortReplyState.intent, 'accept_proposal', 'intent')
  assertEqual(p.writerBrief.conversationalMove, 'execute_pending_proposal', 'move')
  assertEqual(p.writerBrief.forceMinimalAck, false, 'no force')
  assertEqual(p.writerBrief.shouldAskQuestion, false, 'no ask')
  assertEqual(p.objective, 'execute_pending_proposal', 'objective')
  assert(
    p.writerBrief.mustNot.some((m) => /Va bene/i.test(m)),
    'forbid va bene',
  )
})

test('81b. Phase1: completed answer + ok → passive_acknowledgement', () => {
  const p = plan({
    perception: {
      language: 'it',
      intent: 'continuation',
  ...
  assertEqual(p.shortReplyState.intent, 'passive_acknowledgement', 'passive')
  assertEqual(p.writerBrief.conversationalMove, 'passive_acknowledgement', 'move')
  assertEqual(p.writerBrief.forceMinimalAck, true, 'force allowed')
})

test('81c. Phase1: vai after continue offer → continue_topic', () => {
  const p = plan({
    perception: {
      language: 'it',
      intent: 'continuation',
  ...
  assertEqual(p.shortReplyState.intent, 'continue', 'continue')
  assertEqual(p.writerBrief.conversationalMove, 'continue_topic', 'move')
  assertEqual(p.writerBrief.forceMinimalAck, false, 'no force')
})

test('81d. Phase1: Grazie basta così → stop', () => {
  const p = plan({
    perception: {
      language: 'it',
      intent: 'continuation',
    ...
    messages: [
      { role: 'assistant', content: 'Ecco la spiegazione sui buchi neri.' },
      { role: 'user', content: 'Grazie, basta così.' },
    ],
  })
  assertEqual(p.shortReplyState.intent, 'stop', 'stop')
  assertEqual(p.writerBrief.conversationalMove, 'stop', 'move')
  assertEqual(p.writerBrief.shouldContinue, false, 'no continue')
})

test('81e. Phase1: mh → uncertain / clarify', () => {
  const p = planEncouragementTurn('mh')
  assertEqual(p.shortReplyState.intent, 'uncertain', 'uncertain')
  assertEqual(p.writerBrief.conversationalMove, 'clarify_uncertain', 'move')
  assertEqual(p.writerBrief.forceMinimalAck, false, 'no force')
})

test('81f. Phase1: sì preserves active topic from proposal', () => {
  const p = plan({
    perception: {
      language: 'it',
      intent: 'continuation',
    ...
    messages: [
      {
        role: 'assistant',
        content: 'Se vuoi posso raccontarti qualcosa di assurdo sul corpo umano.',
      },
      { role: 'user', content: 'sì' },
    ],
  })
  assertEqual(p.shortReplyState.intent, 'accept_proposal', 'accept')
  assertEqual(p.writerBrief.shouldAskQuestion, false, 'ask vetoed')
  assert(
    /corpo|umano|assurdo/i.test(String(p.writerBrief.activeTopic || p.conversationFocus.topic || '')),
    `topic=${p.writerBrief.activeTopic}`,
  )
})
