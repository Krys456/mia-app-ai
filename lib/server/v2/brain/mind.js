...
export const MIND_VERSION = '2.2.0-mind'
 ...
 * @property {import('./conversation-signals.js').ConversationSignals} [conversationSignals] Phase 6 turn cues
 * @property {import('./adaptive-response-profile.js').AdaptivePreferences|object} [preferences] soft style modes
 * @property {string} [userMessage] latest user text for adaptive cues
 */
...
function normalize(input) {
  ...
  const preferenceBias = asString(s.preferenceBias) || asString(preferences.replyLength)
  ...
  const closingFromState =
    ...
    Boolean(conversationSignals && conversationSignals.interaction?.stopCue)

  return {
    perception: {
      language:
        asString(conversationSignals?.language?.code) ||
        asString(p.language) ||
        'unknown',
 ...
 * @param {string} socialIntent
 * @param {import('./conversation-signals.js').ConversationSignals|null} [conversationSignals]
 * @returns {MindEmotionalTone}
 */
function decideTone(emotionalState, strategy, socialIntent, conversationSignals = null) {
  ...
  if (conversationSignals?.affect?.seriousness >= 0.55) return 'serious'
  if (conversationSignals?.affect?.frustration >= 0.55) return 'serious'
  if (strategy === 'explore' || strategy === 'continue') {
    if (
      emotionalState === 'playful' ||
      socialIntent === 'laughter' ||
      conversationSignals?.affect?.playfulness >= 0.55
    ) {
      return 'playful'
    }
  ...
  if (strategy === 'connect') {
    if (emotionalState === 'playful' || conversationSignals?.affect?.playfulness >= 0.55) {
      return 'playful'
    }
    return 'warm'
  }
...
function decideChallenge(ctx, strategy, shouldComfort) {
  ...
  const { perception: p, memory, session, conversationSignals } = ctx
  if (p.emotionalState === 'frustrated' || p.emotionalState === 'angry') return false
  if (conversationSignals?.affect?.frustration >= 0.55) return false
  if (conversationSignals?.affect?.seriousness >= 0.7) return false
...
function decideInitiative(ctx, strategy, need, shouldAskQuestion) {
  ...
  if (strategy === 'connect' && p.conversationStage === 'opening') {
    // Pure how-are-you / small talk after greeting: presence only, no spark curriculum.
    if (p.socialIntent === 'how_are_you' || p.intent === 'small_talk') return 'none'
    return 'one_spark'
  }
...
export function think(input = {}) {
  ...
  const emotionalTone = decideTone(
    p.emotionalState,
    strategy,
    p.socialIntent,
    ctx.conversationSignals,
  )
  let responseDepth = decideDepth(ctx, strategy, need)
  const goal = buildGoal(strategy, need, initiative, shouldContinueTopic)

  // Phase 4: adaptive HOW profile (Mind derives; Planner may constrain; Writer consumes).
  // Phase 6: Conversation Signals feed shared cues (no duplicated regex here).
  const responseProfile = buildAdaptiveResponseProfile({
    perception: p,
    conversationState: ctx.conversationState,
    conversationSignals: ctx.conversationSignals,
    userMessage: ctx.userMessage,
    preferences: ctx.preferences,
    previousProfile: ctx.conversationState?.responseProfile || null,
    mindDepth: responseDepth,
    mindTone: emotionalTone,
    strategy,
  })
  // Align legacy depth band with adaptive depth when adaptive is more specific.
  // Voice and explicit preferenceBias concise keep Mind depth authority.
  if (!ctx.session.isVoice && ctx.session.preferenceBias !== 'concise') {
    const mapped = mapAdaptiveDepthToMind(responseProfile.depth)
    if (
      responseProfile.depth === 'expert' ||
      responseProfile.depth === 'detailed' ||
      responseProfile.verbosity === 'minimal' ||
      responseProfile.verbosity === 'short'
    ) {
      responseDepth = mapped
    }
  }
  // Cap adaptive profile for voice to short/light.
  if (ctx.session.isVoice) {
    responseProfile.depth = 'short'
    if (responseProfile.verbosity === 'long' || responseProfile.verbosity === 'medium') {
      responseProfile.verbosity = 'short'
    }
  }
  ...
  // Preserve profile after invariants (enforceInvariants spreads/rebuilds goal only).
  decision.responseProfile = responseProfile
