 ...
 */
function suggestionsToRewriteBrief(suggestions) {
  const lines = Array.isArray(suggestions)
    ? suggestions.map((s) => String(s || '').trim()).filter(Boolean)
    : []
  if (!lines.length) {
    return 'Improve naturalness, specificity, and respect of the planner brief. Keep facts unchanged.'
  }
  return ['Apply these reviewer suggestions once:', ...lines.map((s) => `- ${s}`)].join(
    '\n',
  )
}
...
async function runTurn(ctx, userMessage, history) {
  ...
  if (review.decision === 'REWRITE') {
    const rewriteBrief = suggestionsToRewriteBrief(review.suggestions)
