...
export function detectSparkNeed(input = {}) {
  ...
  // Conversation Opportunity Engine gate: initiative must earn its place.
  const opportunity =
    input.conversationOpportunity?.plan || input.conversationOpportunity || null
  if (opportunity?.active && opportunity.initiativeAllowed === false) {
    return {
      should: false,
      trigger: 'idle',
      confidence: 'high',
      signals: ['opportunity_blocked', ...(opportunity.signals || []).slice(0, 2)],
    }
  }
