...
test('TEST 9 — revoke + successor IT → Dragon Ball', async () => {
  ...
  const correction = resolveSameTurnCorrection(msg)
  assert.equal(correction.mode, 'rewrite')
  assert.equal(correction.kind, 'favorite_successor')
  assert.match(correction.text, /il mio anime preferito è Dragon Ball/i)
  ...
  assert.equal(activeFavoriteAnime(db.rows), 'dragon_ball')
  // Naruto must not remain active; no empty slot.
  const activeFav = db.rows.filter(
    (r) => r.status === 'active' && readFactKeyFromTags(r.tags) === 'preferences.favorite.anime',
  )
  assert.equal(activeFav.length, 1)
  assert.equal(favoriteValueSlugFromContent(activeFav[0].content), 'dragon_ball')
  assert.ok(
    !db.rows.some(
      (r) =>
        r.status === 'active' &&
        readFactKeyFromTags(r.tags) === 'preferences.favorite.anime' &&
        favoriteValueSlugFromContent(r.content) === 'naruto',
    ),
  )
})

test('TEST 9b — Preview Test C punctuation variants → successor assert', async () => {
  const variants = [
    'Naruto non è più il mio anime preferito; il mio anime preferito è Dragon Ball.',
    'Naruto non è più il mio anime preferito. Il mio anime preferito è Dragon Ball.',
    'Naruto non è più il mio anime preferito, il mio anime preferito è Dragon Ball.',
    'Il mio anime preferito non è più Naruto; il mio anime preferito è Dragon Ball.',
    'Naruto is no longer my favorite anime; my favorite anime is Dragon Ball.',
    // Fragile separators that previously fell through to revoke-only:
    'Naruto non è più il mio anime preferito;il mio anime preferito è Dragon Ball.',
    'Naruto non è più il mio anime preferito；il mio anime preferito è Dragon Ball.',
    'Naruto non è più il mio anime preferito: il mio anime preferito è Dragon Ball.',
  ]
  for (const msg of variants) {
    const correction = resolveSameTurnCorrection(msg)
    assert.equal(correction.mode, 'rewrite', msg)
    assert.equal(correction.kind, 'favorite_successor', msg)
    const facts = extractDurableFacts(msg)
    assert.ok(
      facts.some(
        (f) => f.factKey === 'preferences.favorite.anime' && /Dragon Ball/i.test(f.content),
      ),
      msg,
    )
    assert.ok(!facts.some((f) => f.operation === 'revoke'), msg)
    const db = createFakeSupabase([
      seedRow({
        id: 'fav-1',
        factKey: 'preferences.favorite.anime',
    ...
    await pipeline(db, msg)
    assert.equal(activeFavoriteAnime(db.rows), 'dragon_ball', msg)
  }
})

test('TEST 9c — revoke-only (no full successor) still revokes', async () => {
  const db = createFakeSupabase([
    seedRow({
      id: 'fav-1',
      factKey: 'preferences.favorite.anime',
  ...
  const msg = 'Naruto non è più il mio anime preferito.'
  assert.equal(resolveSameTurnCorrection(msg).mode, 'none')
  const facts = extractDurableFacts(msg)
  assert.ok(facts.some((f) => f.operation === 'revoke'))
  await pipeline(db, msg)
  assert.equal(activeFavoriteAnime(db.rows), null)
})
