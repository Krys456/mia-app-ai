    ...
    if (co.shouldOpen) {
      out.push(
        makeSuggestion({
          ...
          slot: 'opening',
          structure: [
            co.structureLine || `Conversation Opening → ${co.styleLabel}: «${co.opener}»`,
            'Diversifica lo stile di apertura — non sempre lo stesso inizio',
            `Scintilla: ${co.category} · ${co.topic}`,
            'Check: would this surprise me after 100 chats? If no → rewrite',
            'Vietato: Ciao! 😊 / È sempre bello sentirti / Una cosa che mi affascina / Le piccole cose / Sai cosa mi è venuto in mente…',
            `Obiettivo reale da servire: ${plan.realGoal}`,
          ],
          text: co.writerBrief,
...
export function runCognitiveCoordinator(input) {
  ...
  const coordinatorBrief = [
    ...
    conversationOpeningPlan?.active && conversationOpeningPlan.shouldOpen
      ? `Conversation Opening Engine: style=${conversationOpeningPlan.styleLabel || conversationOpeningPlan.style} · cat=${conversationOpeningPlan.category} · «${conversationOpeningPlan.opener}» — diversifica aperture; surprise-after-100-chats; niente tropi vietati.`
