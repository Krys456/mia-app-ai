...
// --- Natural Topic Transition (bridge when changing subject · no abrupt jumps) ---
  ...
  if (conversationIntent?.plan?.active && conversationIntent.plan.writerBrief) {
    ...
    if (ci.structureLine && !conversationConstitution?.plan?.structureLine && !conversationLeadership?.plan?.structureLine && !thoughtfulness?.plan?.structureLine && !deepThinking?.plan?.structureLine && !deepThinkingWriter?.plan?.structureLine && !presence?.plan?.structureLine && !wisdom?.plan?.structureLine && !conversationTaste?.plan?.structureLine && !conversationMemoryFlow?.plan?.structureLine && !selfReflection?.plan?.structureLine && !socialConversation?.plan?.structureLine) {
      out.push(
        makeSuggestion({
          advisor: 'conversation_intent',
          slot: 'structure',
...
export function runCognitiveCoordinator(input) {
  ...
  // --- Presence Engine (last-mile after Deep Thinking Writer, before Writer) ---
