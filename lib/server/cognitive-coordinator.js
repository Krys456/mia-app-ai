...
export function collectAdvisorSuggestions(input) {
  ...
  // --- Worth Reading Protocol (FINAL Writer craft — after Constitution / HCS) ---
...
export function runCognitiveCoordinator(input) {
  ...
  // --- Human Conversation Simulator (before Worth Reading / Writer) ---
  ...
  // --- Worth Reading Protocol (LAST craft before Writer — after HCS) ---
  ...
  // Worth Reading Protocol — LAST unshift → top craft cue (constitution stays in baseDirective)
  if (worthReadingPlan?.active && worthReadingPlan.writerBrief) {
 ...
 * Conversation Constitution + Worth Reading Protocol are baked into the base so budget cannot drop them.
 * @param {object} plan
 * @param {string | null} realGoal
 * @param {object | null} [constitutionPlan]
 * @param {object | null} [worthReadingPlan]
 */
function buildBaseWriterDirective(plan, realGoal, constitutionPlan = null, worthReadingPlan = null) {
