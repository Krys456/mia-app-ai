...
export function collectAdvisorSuggestions(input) {
  ...
  if (conversationIntent?.plan?.active && conversationIntent.plan.writerBrief) {
    ...
    if (ci.structureLine && !conversationLeadership?.plan?.structureLine && !deepThinking?.plan?.structureLine && !presence?.plan?.structureLine && !wisdom?.plan?.structureLine && !conversationTaste?.plan?.structureLine) {
      out.push(
        makeSuggestion({
          advisor: 'conversation_intent',
          slot: 'structure',
...
export function runCognitiveCoordinator(input) {
  ...
  // Conversation Taste last-mile — after Wisdom, before HCS
  if (conversationTastePlan?.active && conversationTastePlan.writerBrief) {
