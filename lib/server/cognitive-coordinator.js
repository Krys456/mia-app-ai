...
export function collectAdvisorSuggestions(input) {
  ...
  if (conversationIntent?.plan?.active && conversationIntent.plan.writerBrief) {
    ...
    if (ci.structureLine && !conversationLeadership?.plan?.structureLine && !deepThinking?.plan?.structureLine && !presence?.plan?.structureLine && !wisdom?.plan?.structureLine && !conversationTaste?.plan?.structureLine && !conversationMemoryFlow?.plan?.structureLine && !selfReflection?.plan?.structureLine) {
      out.push(
        makeSuggestion({
          advisor: 'conversation_intent',
          slot: 'structure',
