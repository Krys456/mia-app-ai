...
export function collectAdvisorSuggestions(input) {
  ...
  if (conversationIntent?.plan?.active && conversationIntent.plan.writerBrief) {
    ...
    if (ci.structureLine && !conversationLeadership?.plan?.structureLine && !thoughtfulness?.plan?.structureLine) {
      out.push(
        makeSuggestion({
          advisor: 'conversation_intent',
          slot: 'structure',
...
export function runCognitiveCoordinator(input) {
  ...
  // --- Thoughtfulness Engine (last-mile before Writer: best conversational contribution) ---
  ...
  // Thoughtfulness last-mile — craft contribution before HCS intent layer
  if (thoughtfulnessPlan?.active && thoughtfulnessPlan.writerBrief) {
