...
export function resolveConflicts(deduped, state = {}) {
  ...
  for (const s of deduped) {
    ...
    // Coda: exclusive — curiosity / momentum / intellectual_initiative / next_ask bridge — only ONE
    if (s.slot === 'coda') {
...
export function collectAdvisorSuggestions(input) {
  ...
  // --- Topic Leadership Engine (user delegated topic choice) ---
  if (topicLeadership?.plan?.shouldLead && topicLeadership.plan.chosen && topicLeadership.plan.writerBrief) {
    ...
    out.push(
      makeSuggestion({
        ...
        slot: 'opening',
        structure: [
          `Prendi iniziativa: un solo tema «${pick.title}»`,
          `Perché (breve): ${pick.why}`,
          `Primo insight coinvolgente — poi al massimo UNA domanda aperta`,
          'Vietato: liste di temi, “di cosa vuoi parlare?”, far riscegliere l’utente',
          `Obiettivo reale da servire: ${plan.realGoal}`,
        ],
        text: tl.writerBrief,
        confidence: tl.confidence === 'high' ? 0.93 : 0.82,
        baseValue: ADVISOR_BASE_VALUE.topic_leadership,
        reasons: [`topic_${pick.id}`, ...(tl.reasons || []).slice(0, 3)],
      }),
    )
    out.push(
      makeSuggestion({
        ...
        confidence: tl.confidence === 'high' ? 0.93 : 0.82,
        baseValue: ADVISOR_BASE_VALUE.topic_leadership,
        reasons: ['topic_leadership_brief'],
      }),
    )
  }
        ...
        makeSuggestion({
          ...
          structure:
            continuation.plan.intent === 'compliment_go_deeper'
              ? [
                  'Ack caldo in mezza frase max — NON rispondere solo con “grazie”',
                  `Premia la curiosità: UNA idea più profonda (${continuation.plan.continuationStyle || 'advanced'}) sul filo`,
                  'Il complimento = segnale di voler andare più a fondo',
          ...
          reasons:
            continuation.plan.intent === 'compliment_go_deeper'
              ? ['continuation_compliment_deeper']
