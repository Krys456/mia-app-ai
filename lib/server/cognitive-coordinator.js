...
export function collectAdvisorSuggestions(input) {
  ...
  if (conversationIntent?.plan?.active && conversationIntent.plan.writerBrief) {
    ...
    if (ci.structureLine && !conversationConstitution?.plan?.structureLine && !conversationLeadership?.plan?.structureLine && !deepThinking?.plan?.structureLine && !presence?.plan?.structureLine && !wisdom?.plan?.structureLine && !conversationTaste?.plan?.structureLine && !conversationMemoryFlow?.plan?.structureLine && !selfReflection?.plan?.structureLine) {
      out.push(
        makeSuggestion({
          advisor: 'conversation_intent',
          slot: 'structure',
  ...
  // --- Deep Thinking Engine (after Leadership, before Writer: explore directions) ---
...
export function runCognitiveCoordinator(input) {
  ...
  if (!presencePlan?.active || !presencePlan?.writerBrief) {
    try {
      const rerun = runPresenceEngine({
        ...
        thoughtfulness: input.thoughtfulness || null,
        session: input.session || input.conversation?.memory || null,
      })
  ...
  // --- Human Conversation Simulator (immediately before Writer) ---
  ...
  // Conversation Constitution — IMMUTABLE LAW last unshift → stays first; also baked into baseDirective
  if (conversationConstitutionPlan?.active && conversationConstitutionPlan.writerBrief) {
 ...
 * Conversation Constitution is baked into the base so budget cannot drop it.
 * @param {object} plan
 * @param {string | null} realGoal
 * @param {object | null} [constitutionPlan]
 */
function buildBaseWriterDirective(plan, realGoal, constitutionPlan = null) {
