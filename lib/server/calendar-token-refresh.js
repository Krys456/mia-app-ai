...
async function doGetValidGoogleAccessToken(opts) {
  ...
  if (stillValid) {
    const dec = await decryptToken(row.access_token_enc, opts.env.SHINKAIDO_CALENDAR_ENCRYPTION_KEY)
  ...
  const refreshDec = await decryptToken(
    ...
    opts.env.SHINKAIDO_CALENDAR_ENCRYPTION_KEY,
  )
  ...
  const accessEnc = await encryptToken(newAccess, opts.env.SHINKAIDO_CALENDAR_ENCRYPTION_KEY)
  ...
  if (newRefresh) {
    const enc = await encryptToken(newRefresh, opts.env.SHINKAIDO_CALENDAR_ENCRYPTION_KEY)
