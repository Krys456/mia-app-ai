...
const GREETING_BANK = [
  // —— warm_only ——
  {
    ...
    line: ({ name }) =>
      `Buongiorno${name ? ` ${name}` : ''}. Stamattina mi è venuto in mente un dettaglio piccolo ma utile — te lo lascio subito.`,
  },
  {
    id: 'wo-aft-1',
    styles: ['warm', 'calm'],
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Eccoti${name ? `, ${name}` : ''}. C’è un angolo sul filo di oggi che merita due frasi — parto da lì.`,
  },
  {
    id: 'wo-eve-1',
    styles: ['warm', 'calm'],
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Buonasera${name ? ` ${name}` : ''}. Niente frenesia: un pensiero leggero, concreto, e poi vediamo.`,
  },
  {
    id: 'wo-night-1',
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Hey${name ? ` ${name}` : ''}. Ora tarda — restiamo su qualcosa di leggero che valga davvero.`,
  },
  {
    id: 'wo-pause-1',
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Di nuovo qui${name ? `, ${name}` : ''}. Il filo è ancora caldo — riprendo dal pezzo che contava, senza ricapitolare tutto.`,
  },
  {
    id: 'wo-first-1',
    styles: ['warm', 'bright'],
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Ciao${name ? ` ${name}` : ''}. Parto con un’osservazione concreta — così abbiamo già un filo, non un menu.`,
  },
  {
    id: 'wo-first-2',
    styles: ['calm', 'terse'],
    ...
    encounters: ['first'],
    strategies: ['warm_only'],
    line: () => `Ciao. Un’idea chiara per iniziare, senza fretta — poi pieghiamo dove serve.`,
  },
  {
    id: 'wo-stressed-1',
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Hey${name ? ` ${name}` : ''}. Andiamo piano. Un pezzo alla volta — niente pressione.`,
  },
  {
    id: 'wo-formal-1',
    styles: ['formal'],
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Buongiorno${name ? ` ${name}` : ''}. Un punto di partenza sobrio: un fatto utile, poi il resto.`,
  },
  ...
  // —— resume_project ——
  {
    ...
    line: ({ name, memory }) =>
      `Eccoti${name ? `, ${name}` : ''}. Su ${memory} c’è un pezzo concreto che vale riprendere.`,
  },
  {
    id: 'rp-2',
    styles: ['warm', 'bright'],
    ...
    strategies: ['resume_project'],
    line: ({ memory }) => `Bentornato. ${memory} è ancora vivo — riparto da lì con un passo chiaro.`,
  },
  {
    id: 'rp-3',
    styles: ['terse', 'calm'],
    ...
    encounters: ['returning', 'pause_resume'],
    strategies: ['resume_project'],
    line: ({ memory }) => `Ciao. Riprendo ${memory} da dove l’avevamo lasciato.`,
  },
  {
    id: 'rp-4',
    styles: ['formal', 'calm'],
    ...
    strategies: ['resume_project'],
    line: ({ name, memory }) =>
      `Buongiorno${name ? `, ${name}` : ''}. Riprendo ${memory} con un passo netto.`,
  },
  {
    id: 'rp-pause-1',
    ...
    encounters: ['pause_resume'],
    strategies: ['resume_project'],
    line: ({ memory }) => `Riprendo il filo su ${memory} — un pezzo concreto, subito.`,
  },
  ...
  // —— celebrate_progress ——
  {
    ...
    line: ({ name, memory }) =>
      `Hey${name ? ` ${name}` : ''} — bel pezzo su ${memory}. Il passo dopo è già chiaro.`,
  },
  {
    id: 'cp-2',
    styles: ['warm', 'calm'],
    ...
    encounters: ['returning'],
    strategies: ['celebrate_progress'],
    line: ({ memory }) => `Ho ancora fresco il progresso su ${memory}. Costruisco sopra, senza reset.`,
  },
  {
    id: 'cp-3',
    styles: ['bright'],
    ...
    encounters: ['returning', 'pause_resume'],
    strategies: ['celebrate_progress'],
    line: ({ memory }) => `Ottimo lavoro su ${memory}. Ecco il pezzo successivo, concreto.`,
  },
  {
    id: 'cp-soft-1',
    styles: ['calm', 'warm'],
    ...
    encounters: ['returning', 'pause_resume'],
    strategies: ['celebrate_progress'],
    line: ({ memory }) => `Hai già fatto strada con ${memory}. Facciamo solo il prossimo pezzetto.`,
  },
  ...
  // —— suggest_next ——
  {
    ...
    line: ({ name, memory }) =>
      memory
        ? `Ciao${name ? ` ${name}` : ''}. Su ${memory} porto un pezzo concreto e utile, subito.`
        : `Ciao${name ? ` ${name}` : ''}. Porto un’idea concreta — poi pieghiamo dove serve.`,
  },
  {
    id: 'sn-2',
    styles: ['terse', 'calm'],
    ...
    line: ({ memory }) =>
      memory
        ? `Ciao. Su ${memory} vado dritto al pezzo che conta.`
        : `Ciao. Parto io con un filo — poi pieghiamo dove serve.`,
  },
  {
    id: 'sn-3',
    styles: ['formal'],
    ...
    line: ({ name, memory }) =>
      memory
        ? `Buongiorno${name ? ` ${name}` : ''}. Su ${memory} avanzo con un passo chiaro.`
        : `Buongiorno${name ? ` ${name}` : ''}. Un punto di partenza concreto, senza menu.`,
  },
  {
    id: 'sn-first-1',
    styles: ['warm', 'bright'],
    ...
    strategies: ['suggest_next'],
    line: ({ name }) =>
      `Ciao${name ? ` ${name}` : ''}. Porto io la prima idea — concreta, non un’intervista.`,
  },
  ...
  // —— warm_handoff (substance already in the message) ——
  {
    ...
    line: () => `Ci entro subito dal pezzo che conta.`,
  },
  {
    id: 'wh-2',
    styles: ['terse', 'calm'],
  ...
  },
  {
    ...
    encounters: ['any'],
    strategies: ['warm_handoff'],
    line: () => `Bello — partiamo dalla sostanza.`,
  },
  {
    id: 'wh-4',
    styles: ['formal'],
    ...
    encounters: ['any'],
    strategies: ['warm_handoff'],
    line: () => `Entriamo nel merito senza preamboli.`,
  },
  {
    id: 'wh-5',
    styles: ['calm', 'warm'],
    ...
    encounters: ['any'],
    strategies: ['warm_handoff'],
    line: () => `Con calma — un pezzo chiaro alla volta.`,
  },
]
...
export function suggestNextStep(opts) {
  ...
  if (strategy === 'resume_project' && memory?.label) {
    return enthusiasm === 'terse'
      ? `Riprendere ${memory.label} con un pezzo concreto`
      : `Riprendere ${memory.label} da dove l’avevamo lasciato — afferma, non chiedere permesso`
  }
  if (strategy === 'celebrate_progress' && memory?.label) {
    return `Costruisci il prossimo pezzo su ${memory.label} (affermazione, non “vuoi continuare?”)`
  }
  if (strategy === 'suggest_next') {
    if (memory?.label) return `Una direzione concreta su ${memory.label} — sviluppala subito`
    if (encounterKind === 'first') {
      return 'Apri con osservazione/idea/curiosità/insight come affermazione — non “Dimmi pure.” / “Come posso aiutarti?” / “Hai domande?”'
    }
    return 'UNA direzione ad alto valore, sviluppata — niente aperture da sportello'
  }
  return null
}
...
export async function buildWelcomePlan(input) {
  ...
  const strategyGuide = {
    warm_only:
      'Calore umano + presenza da partner: se il filo è vuoto, apri con osservazione/idea/curiosità/insight — niente aperture a basso valore (“dimmi pure”, “come posso aiutarti”, “priorità”, “hai domande”, “fammi sapere”, “sono qui se ti serve”).',
    resume_project: memory
      ? `Riprendi naturalmente UN filo: «${memory.label}».`
      : 'Riprendi la relazione senza elencare memorie; offri un’idea se manca un filo.',
    ...
    suggest_next:
      'Dopo il saluto, UNA sola direzione proposta da te e sviluppata — non un’intervista sulle priorità.',
    warm_handoff: 'Ack caldo brevissimo, poi servi subito la richiesta — niente digressioni.',
  }

  const writerBrief = [
    'WELCOME EXPERIENCE ENGINE: partner di conversazione di fiducia — non macchina Q&A, non sportello ticket.',
    `Incontro: ${encounterKind}. Orario: ${timeOfDay}. Umore: ${mood}. Stile: ${enthusiasm}.`,
    `Strategia: ${strategy}. ${strategyGuide[strategy]}`,
    `Intento di apertura (tono + direzione — NON copiare/parafrasare come script): «${greeting.line}»`,
    'Scrivi un’apertura originale ad alto valore: osservazione, idea, curiosità, storia breve, insight, fatto sorprendente o collegamento. Afferma, non intervistare.',
    'Vietato: presentarti (“Sono LAIfe…”), menu di scelte, “Dimmi pure.”, “Come posso aiutarti?”, “Hai domande?”, “Fammi sapere.”, “Sono qui se ti serve.”',
    ...
    nextStep
      ? `Un solo next step di valore (proposto da te, non chiesto): ${nextStep}.`
      : 'Nessun next step forzato — ma non restare passivo: calore + un’idea leggera già in mano.',
    'Su saluto/incertezza: prendi responsabilità; inizia una conversazione interessante, non un’intervista.',
    'Varietà essenziale: suona come qualcuno che ha già un pensiero, non come un template.',
...
Regole:
- Partner di conversazione, non macchina Q&A / chatbot da sportello
- Mai aperture a basso valore (“Dimmi pure.”, “Come posso aiutarti?”, “Qual è la tua priorità?”, “Cosa vuoi sapere?”, “Hai domande?”, “Fammi sapere.”, “Sono qui se ti serve.”)
...
- Su saluto/incertezza: prendi responsabilità; porta un’idea
