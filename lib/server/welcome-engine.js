...
const GREETING_BANK = [
  ...
  },
  {
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Eccoti${name ? `, ${name}` : ''}. C’è un filo che mi incuriosisce — se ti va, ci entro.`,
  },
  {
    id: 'wo-eve-1',
    styles: ['warm', 'calm'],
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Buonasera${name ? ` ${name}` : ''}. Piano piano — ho un filo a cui dare energia, se ti va.`,
  },
  {
    id: 'wo-night-1',
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Hey${name ? ` ${name}` : ''}. Ancora sveglio — se vuoi, restiamo su qualcosa di leggero.`,
  },
  {
    id: 'wo-pause-1',
  ...
  },
  {
    ...
    strategies: ['warm_only'],
    line: ({ name }) =>
      `Ciao${name ? ` ${name}` : ''}. Sono LAIfe — piacere di conoscerti.`,
  },
  {
    id: 'wo-first-2',
    styles: ['calm', 'terse'],
    ...
    encounters: ['first'],
    strategies: ['warm_only'],
    line: () => `Ciao. Sono LAIfe — partiamo da qualcosa di concreto, senza fretta.`,
  },
  {
    id: 'wo-stressed-1',
  ...
  // —— suggest_next ——
  {
    ...
    line: ({ name, memory }) =>
      memory
        ...
        : `Ciao${name ? ` ${name}` : ''}. Parto da un’idea concreta — poi pieghiamo dove serve.`,
  },
  {
    id: 'sn-2',
    styles: ['terse', 'calm'],
    ...
    line: ({ memory }) =>
      memory
        ? `Ciao. Prossimo passo su ${memory} — ci vado dritto.`
  ...
  },
  {
    ...
    line: ({ name, memory }) =>
      memory
        ...
        : `Buongiorno${name ? ` ${name}` : ''}. Ti propongo un punto di partenza concreto.`,
  },
  {
    id: 'sn-first-1',
    styles: ['warm', 'bright'],
    ...
    strategies: ['suggest_next'],
    line: ({ name }) =>
      `Ciao${name ? ` ${name}` : ''}. Partiamo da una cosa concreta che vale la pena — io porto la prima idea.`,
  },
...
export function suggestNextStep(opts) {
  ...
  if (strategy === 'suggest_next') {
    ...
    if (encounterKind === 'first') {
      return 'Apri con osservazione/idea/curiosità/insight (non “Dimmi pure.” / “Come posso aiutarti?” / “Qual è la tua priorità?” / “Hai domande?”)'
    }
    return 'Proponi UNA direzione ad alto valore e sviluppala — non aperture a basso valore'
  }
  return null
}
