...
describe('#303A reminder validation units', () => {
  ...
  it('#380C rejects Etc/GMT* on create; accepts normal IANA zones', async () => {
    const mod = await import(pathToFileURL(path.join(root, 'lib/server/reminders.js')).href)
    const timeMod = await import(pathToFileURL(path.join(root, 'lib/server/reminder-time.js')).href)
    const now = new Date('2026-08-18T12:00:00.000Z')

    assert.equal(timeMod.isValidIanaTimeZone('Europe/Rome'), true)
    assert.equal(timeMod.isValidIanaTimeZone('America/New_York'), true)
    assert.equal(timeMod.isValidIanaTimeZone('Asia/Tokyo'), true)
    assert.equal(timeMod.isValidIanaTimeZone('Europe/London'), true)
    assert.equal(timeMod.isUnreliableReminderTimeZone('Etc/GMT'), true)
    assert.equal(timeMod.isUnreliableReminderTimeZone('Etc/GMT+12'), true)
    assert.equal(timeMod.isUnreliableReminderTimeZone('Etc/GMT-5'), true)
    assert.equal(timeMod.isValidIanaTimeZone('Etc/GMT'), false)
    assert.equal(timeMod.isValidIanaTimeZone('Etc/GMT+12'), false)
    assert.equal(timeMod.isValidIanaTimeZone('Etc/GMT-5'), false)

    for (const bad of ['Etc/GMT', 'Etc/GMT+12', 'Etc/GMT-5']) {
      const rejected = mod.validateReminderCreateInput(
        {
          title: 'Chiamare Marco',
          fire_at: '2026-08-19T15:00:00.000Z',
          timezone: bad,
      ...
      assert.match(String(rejected.errors.timezone || ''), /IANA|timezone/i, bad)
    }

    const updateBad = mod.validateReminderUpdateInput(
      {
        timezone: 'Etc/GMT+12',
      },
      { status: 'pending', fire_at: '2026-08-19T15:00:00.000Z' },
      now,
    )
    assert.equal(updateBad.ok, false)
    assert.match(String(updateBad.errors.timezone || ''), /IANA|timezone/i)
  })
