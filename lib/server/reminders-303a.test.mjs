...
describe('#303A reminder validation units', () => {
  ...
  it('status transitions and feature gate', async () => {
    ...
    assert.equal(limits.canTransitionReminderStatus('delivered', 'delivered'), false)
    assert.equal(limits.isIdempotentReminderStatusAck('delivered'), true)
    assert.equal(limits.isIdempotentReminderStatusAck('completed'), false)
  ...
  it('#380E delivered→delivered is idempotent; invalid transitions still fail', async () => {
    const mod = await import(pathToFileURL(path.join(root, 'lib/server/reminders.js')).href)
    const now = new Date('2026-08-23T19:48:00.000Z')
    const deliveredAt = '2026-08-23T19:48:21.723Z'

    const first = mod.validateReminderUpdateInput(
      { status: 'delivered' },
      { status: 'pending' },
      now,
    )
    ...
    assert.equal(first.data.delivered_at, now.toISOString())
    assert.notEqual(first.idempotent, true)

    const dup1 = mod.validateReminderUpdateInput(
      { status: 'delivered' },
      { status: 'delivered' },
      new Date('2026-08-23T19:55:00.000Z'),
    )
    assert.equal(dup1.ok, true)
    assert.equal(dup1.idempotent, true)
    assert.deepEqual(dup1.data, {})
    assert.equal('delivered_at' in dup1.data, false)
    assert.equal('status' in dup1.data, false)

    const dup2 = mod.validateReminderUpdateInput(
      { status: 'delivered' },
      { status: 'delivered' },
      new Date('2026-08-23T19:55:22.000Z'),
    )
    assert.equal(dup2.ok, true)
    assert.equal(dup2.idempotent, true)
    assert.deepEqual(dup2.data, {})

    const dup3 = mod.validateReminderUpdateInput(
      { status: 'delivered' },
      { status: 'delivered' },
      new Date('2026-08-23T19:56:00.000Z'),
    )
    assert.equal(dup3.ok, true)
    assert.equal(dup3.idempotent, true)

    // Duplicate ack must not invent a new delivered_at (empty patch ⇒ API keeps row as-is).
    assert.ok(deliveredAt)

    const cancelledToDelivered = mod.validateReminderUpdateInput(
      { status: 'delivered' },
      { status: 'cancelled' },
      now,
    )
    assert.equal(cancelledToDelivered.ok, false)
    assert.equal(cancelledToDelivered.errors.status, 'status_transition_invalid')

    const completedToPending = mod.validateReminderUpdateInput(
      { status: 'pending' },
      { status: 'completed' },
      now,
    )
    assert.equal(completedToPending.ok, false)
    assert.equal(completedToPending.errors.status, 'status_transition_invalid')

    // completed → completed is NOT idempotent (only delivered ack).
    const completedDup = mod.validateReminderUpdateInput(
      { status: 'completed' },
      { status: 'completed' },
      now,
    )
    assert.equal(completedDup.ok, false)
    assert.equal(completedDup.errors.status, 'status_transition_invalid')
  })
  ...
  it('#380E client maps nested status_transition_invalid (not generic failure)', () => {
    assert.match(
      reminderApi,
      /errors\?\.status\s*===\s*['"]status_transition_invalid['"]/,
    )
    assert.match(reminderApi, /Questa azione non è consentita/)
    // Idempotent path must short-circuit before updateReminder write on empty patch.
    assert.match(apiId, /idempotent/)
    assert.match(apiId, /Object\.keys\(validated\.data\)\.length === 0/)
  })
})
