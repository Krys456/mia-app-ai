...
function generateCandidates(args) {
  ...
  raw.push({
    ...
    seed: `Una conseguenza concreta e poco nota di ${label} (entra diretto nel contenuto — non annunciare “un fatto interessante”).`,
  ...
  raw.push({
    ...
    seed: `Una scena breve e concreta di ${label} nella vita reale (niente checklist, niente “ad esempio tipico”).`,
    valueLift: 4.5,
    surprise: 3.0,
    relevance: 4.6,
    novelty: 3.2,
  ...
  raw.push({
    ...
    seed: `La convinzione sbagliata più comune su ${label}, corretta in una frase netta (senza dire “misconcezione”).`,
    valueLift: 4.4,
    surprise: 4.0,
    relevance: 4.2,
    novelty: 3.8,
  ...
  raw.push({
    ...
    seed: `Un precedente storico breve che dà prospettiva a ${label} (come dettaglio, non come lezione).`,
  ...
  raw.push({
    ...
    seed: `Perché ${label} “funziona” o fallisce nelle persone — un meccanismo umano, detto semplice.`,
  ...
  raw.push({
    id: 'compare',
    kind: 'comparison',
    seed: `${label} vs l’alternativa più vicina: una sola differenza che cambia la scelta.`,
    valueLift: 4.1,
    surprise: 3.4,
    relevance: 4.3,
    novelty: 3.5,
  ...
  raw.push({
    ...
    seed: `Dove ${label} appare oggi nel mondo reale — un’applicazione tangibile, non astratta.`,
    valueLift: 4.4,
    surprise: 3.2,
    relevance: 4.5,
    novelty: 3.3,
  ...
  raw.push({
    ...
    seed: `Una conseguenza futura plausibile di ${label} (concreta, senza hype né annunci).`,
  ...
  if (Array.isArray(openQuestions) && openQuestions[0]) {
    ...
    if (q.length >= 8) {
      raw.push({
        id: 'open-apply',
        kind: 'real_world_application',
        seed: `Chiudi il filo aperto con un’applicazione concreta (affermazione): ${q}`,
        valueLift: 4.7,
        surprise: 2.8,
        relevance: 4.8,
        novelty: 3.0,
...
function buildAddBrief(insight) {
  return [
    'INTELLECTUAL INITIATIVE: sì — un solo insight ad alto valore, intrecciato naturalmente.',
    `Contenuto da portare (non etichettarlo): ${insight.seed}`,
    'Entra nel contenuto come qualcuno che ha notato qualcosa — mai “Ecco una cosa interessante…” né etichette di genere.',
    'Lunghezza: 1–3 frasi max; non allungare il resto della risposta.',
    'Vietato: liste di extras, emoji di sezione, ripetere il corpo, domande generiche di chiusura.',
  ].join(' ')
}
...
export function formatIntellectualInitiativeForWriter(plan) {
  ...
  const action =
    plan.shouldAdd && plan.chosen
      ? `AZIONE: aggiungi ESATTAMENTE UN insight (${plan.chosen.kind}) — «${plan.chosen.seed}». Forma naturale (niente template fissi). 1–3 frasi.`
