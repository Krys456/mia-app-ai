...
// Optional tools passthrough (image_generation) — still reasoning.none, no temperature
{
  ...
  assertNoTemperatureKey(params, 'gpt-5.6-sol+tools')
  assert.deepEqual(params.reasoning, { effort: 'none' })
  assert.equal(params.stream, false)
...
// api/chat.ts still uses a single responses.create (no second generation)
{
  ...
  assert.doesNotMatch(chatSrc, /requestProbe/)
