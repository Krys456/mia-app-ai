...
export function detectOverviewLanguage(message) {
  const raw = normalizeOverviewText(message)
  if (/\b(?:what|remember|know|about|tell)\b/i.test(raw) && !/\b(?:cosa|ricordi|sai|di\s+me)\b/i.test(raw)) {
    return 'en'
  }
  if (/\b(?:what|remember|know|tell)\b/i.test(raw) && /\babout\s+me\b/i.test(raw)) {
    return 'en'
  }
  return 'it'
}
...
export function ackOverviewEmpty(lang) {
  return lang === 'en'
    ? "I don't currently have any saved information about you."
    : 'Al momento non ho informazioni salvate su di te.'
}

export function ackOverviewUnauthenticated(lang) {
  return lang === 'en'
    ? "I can't inspect saved memories for this session without a signed-in account."
    : 'Non posso ispezionare i ricordi salvati per questa sessione senza un account autenticato.'
}
