  ...
  try {
    ...
    if (isCalendarError(err)) {
      if (err.code === 'not_connected') {
        return { status: 'disconnected', items: [], fetchedAt, timeZone: tz, failureCode: err.code }
      }
      if (err.code === 'calendar_disabled') {
        return { status: 'disabled', items: [], fetchedAt, timeZone: tz, failureCode: err.code }
      }
      if (err.code === 'reconnect_required' || err.code === 'google_unauthorized') {
        return { status: 'reconnect_required', items: [], fetchedAt, timeZone: tz, failureCode: err.code }
      }
      return {
        status: 'error',
        items: [],
    ...
    return { status: 'error', items: [], fetchedAt, timeZone: tz, failureCode: 'unknown' }
  }
}
