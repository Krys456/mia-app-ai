...
import {
  ...
  loadNarutoRelatedPreferenceRows,
  loadRowsForFactKeyAnyStatus,
  logSpecificForgetTrace,
  snapshotForgetRow,
  truncateOwnerId,
} from './memory-specific-forget-trace.js'
...
/**
 * Narrow meta / example / negation-of-request guards (no NLP engine).
 * @param {string} message
 */
...
export function isSpecificForgetIntent(message) {
  ...
  // Questions / third-party / meta must never mutate via Specific Forget.
...
export function familyAwareForgetLabel(row, lang = 'it') {
  ...
  if (key.startsWith('preferences.like.')) {
    return lang === 'en' ? `that you like ${favValue}` : `che ti piace ${favValue}`
  }
  if (key.startsWith('preferences.dislike.')) {
    return lang === 'en' ? `that you don't like ${favValue}` : `che non ti piace ${favValue}`
  }
  if (key.startsWith('preferences.interest.')) {
    return lang === 'en'
      ? `your interest in ${favValue}`
      : `il tuo interesse per ${favValue}`
  }
  ...
  if (key.startsWith('projects.')) {
    return lang === 'en' ? `your project ${favValue}` : `il tuo progetto ${favValue}`
  }
...
export async function tryHandleSpecificForget(input) {
  const userMessage = typeof input?.userMessage === 'string' ? input.userMessage.trim() : ''
  const lang = detectForgetLanguage(userMessage)
  const traceOn = isSpecificForgetTraceEnabled()
  ...
  if (traceOn) {
    logSpecificForgetTrace('specific_forget_classification', {
      truncatedUserId: truncateOwnerId(userId),
      strippedTarget: String(target || '').slice(0, 120),
      kind: classified.kind,
      value: classified.value || null,
      valueSlug: classified.valueSlug || null,
      factKey: classified.factKey || null,
      factKeyPrefix: classified.factKeyPrefix || null,
      subject: classified.subject || null,
  ...
  // Preview forensic: owner-scoped before snapshot (active+obsolete) for like / Naruto overlap.
  if (traceOn && (classified.kind === 'like' || classified.valueSlug === 'naruto')) {
    const before = await loadNarutoRelatedPreferenceRows(supabase, userId, {
      valueSlug: classified.valueSlug || 'naruto',
    })
    logSpecificForgetTrace('specific_forget_before', {
      truncatedUserId: truncateOwnerId(userId),
      error: before.error,
      rows: before.rows,
    })
  ...
  if (
    ...
    resolutionPath = `typed_exact:${classified.kind}`
    if (!factKey) {
      return {
        handled: true,
        status: 'not_found',
    ...
    if (traceOn) {
      logSpecificForgetTrace('specific_forget_exact_target', {
        truncatedUserId: truncateOwnerId(userId),
        requestedFactKey: factKey,
        resolutionPath,
        usedScoreForgetCandidates: false,
        usedBareEntity: false,
        usedFavoriteValueGate: false,
        usedInterestResolver: classified.kind === 'interest',
        listError: listed.error || null,
        exactActiveRows: (listed.rows || []).map(snapshotForgetRow),
        selectedIds: (listed.rows || []).map((r) => String(r.id)),
      })
    }
    if (listed.error) {
      return {
    ...
    if (toObsolete.length === 0) {
      if (traceOn) {
        logSpecificForgetTrace('specific_forget_exact_target_empty', {
          requestedFactKey: factKey,
          truncatedUserId: truncateOwnerId(userId),
        })
      }
    ...
    resolutionPath = 'typed_favorite_value'
    ...
    resolutionPath = 'typed_cofavorite_set'
    ...
    resolutionPath = 'unsupported_search_fallback'
    const active = await listActiveMemoriesForOwner(supabase, userId)
    if (active.error) {
      return {
        handled: true,
  ...
  if (traceOn) {
    const perRow = (toObsolete || []).map((row) => {
      const id = String(row.id)
      const ok = (mutation.obsoletedIds || []).map(String).includes(id)
      const failed = (mutation.failedIds || []).map(String).includes(id)
      return {
        id,
        factKey: readFactKeyFromTags(row.tags || []) || factKey,
        success: ok,
        failed,
        zeroRowSuspected: !ok && !failed,
      }
    })
    logSpecificForgetTrace('specific_forget_obsolete_result', {
      truncatedUserId: truncateOwnerId(userId),
      resolutionPath,
      mutationOk: mutation.ok === true,
  ...
  if (!mutation.ok || mutation.obsoletedIds.length === 0) {
    if (traceOn) {
      const afterFail = await loadNarutoRelatedPreferenceRows(supabase, userId, {
        valueSlug: classified.valueSlug || 'naruto',
      })
      logSpecificForgetTrace('specific_forget_after', {
        truncatedUserId: truncateOwnerId(userId),
        outcome: 'error_or_empty',
        rows: afterFail.rows,
        likeExact: (
          await loadRowsForFactKeyAnyStatus(
            supabase,
            userId,
            classified.factKey || 'preferences.like.naruto',
          )
        ).rows,
        favoriteAnimeExact: (await loadRowsForFactKeyAnyStatus(supabase, userId, 'preferences.favorite.anime'))
          .rows,
      })
    }
    return {
  ...
  if (traceOn) {
    const after = await loadNarutoRelatedPreferenceRows(supabase, userId, {
      valueSlug: classified.valueSlug || 'naruto',
    })
    const likeKey = classified.kind === 'like' ? factKey : `preferences.like.${classified.valueSlug || 'naruto'}`
    const likeExact = await loadRowsForFactKeyAnyStatus(supabase, userId, likeKey || 'preferences.like.naruto')
    const favExact = await loadRowsForFactKeyAnyStatus(supabase, userId, 'preferences.favorite.anime')
    logSpecificForgetTrace('specific_forget_after', {
      truncatedUserId: truncateOwnerId(userId),
      outcome: 'forgotten',
      resolutionPath,
      relatedRows: after.rows,
      LIKE: {
        factKey: likeKey,
        rows: likeExact.rows,
        statusSummary: (likeExact.rows || []).map((r) => r.status),
      },
      FAVORITE: {
        factKey: 'preferences.favorite.anime',
        rows: favExact.rows,
        statusSummary: (favExact.rows || []).map((r) => r.status),
        content: (favExact.rows || []).map((r) => r.content),
      },
    })
  }
