...
export async function tryHandleSpecificForget(input) {
  ...
  if (factKey) {
    ...
    if (isVagueForgetReference(target)) {
      ...
      // Same display name on primary + generic project must not silently pick one.
      // Example: projects.laife and projects.primary both say "LAIfe" → ask which.
      const primaryGenericCollision =
        ...
        (() => {
          ...
          const keyed = scored
            .map((item) => ({
              key: String(
                item.row.factKey || readFactKeyFromTags(item.row.tags || []) || '',
              ),
