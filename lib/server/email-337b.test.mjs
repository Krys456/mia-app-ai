...
describe('#337B write-path guardrails', () => {
  it('does not create api/email.ts and keeps vercel.json at 12 functions', () => {
  ...
  it('does not touch any calendar-* file (git diff against origin/main)', () => {
    ...
    // #388D intentionally gates calendar-oauth-start with the shared entitlement
    // Edge module (parallel to email-oauth-start). That co-change is allowed;
    // any other calendar path still fails this isolation guardrail.
    const unexpected = diff.filter(
      (p) => p !== 'supabase/functions/calendar-oauth-start/index.ts',
    )
    assert.deepEqual(unexpected, [], `calendar files were modified: ${unexpected.join(', ')}`)
  })
