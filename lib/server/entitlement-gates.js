 ...
 *   route?: string | null
 *   decisionStage?: 'offered' | 'required' | 'invoked' | string | null
 *   skip?: boolean
 * }} opts
 */
export function maybeLogEntitlementShadow(opts) {
  ...
  return logEntitlementShadow({
    ...
    decisionStage: opts.decisionStage,
    env,
  })
}
...
export function decideRouteEntitlement(opts) {
  ...
  maybeLogEntitlementShadow({
    ...
    route: opts.route,
    decisionStage: opts.decisionStage || 'required',
  })
  ...
  if (loaded.lookupError && enforcementEnabled) {
    return {
      ...
      body: {
        ...
        entitlement: String(opts.entitlement || ''),
      },
    }
  }
...
/**
 * #388G — Advanced memory gate (auto-write / overview / manage CRUD).
 * Soft-skip when enforcement ON and unentitled; shadow observes either way.
 * Forget/privacy delete must NOT call this helper.
 *
 * @param {{
 *   entitlements: import('./entitlements.js').EntitlementSet
 *   planId?: import('./entitlements.js').PlanId | string
 ...
 * }} opts
 */
export function decideAdvancedMemoryEntitlement(opts) {
  const env = opts.env ?? process.env
  maybeLogEntitlementShadow({
    shadowEnabled: opts.shadowEnabled,
    env,
    feature: 'advancedMemory',
    entitlements: opts.entitlements,
    planId: opts.planId,
...
export function decideWebSearchTools(opts) {
  ...
  // Shadow: required when explicit search; offered when tools merely attached.
  const requesting =
  ...
  if (requesting) {
    maybeLogEntitlementShadow({
      ...
      requestId: opts.requestId,
      route: '/api/chat',
      decisionStage: explicitRequire ? 'required' : 'offered',
    })
  }
