...
function pickContinuation(args) {
  ...
  if (intent === 'how_to' || intent === 'problem_solving') {
    return {
      kind: 'practical_next',
      hint: `Avanza di un passo concreto su ${label} (una sola mossa utile — affermazione, non checklist né “vuoi…?”)`,
    }
  }
  ...
  if (intent === 'advice' || intent === 'comparison') {
    return {
      kind: 'edge_caution',
      hint: `Un rischio o trade-off concreto da non ignorare su ${label} (detto diretto)`,
    }
  }
  ...
  if (teachingLikely || intent === 'explanation') {
    if (alreadyExplainedCount >= 4) {
      return {
        kind: 'edge_caution',
        hint: `L’errore comune ad alto segnale su ${label} — una frase netta`,
      }
    }
    return {
      kind: 'deeper_bridge',
      hint: `Il pezzo successivo naturale di ${label} — ponte conciso, senza rifare la lezione`,
    }
  }

  return {
    kind: 'practical_next',
    hint: `Una sola continuazione di valore sul filo (${label}) — afferma, non intervistare`,
  }
}
