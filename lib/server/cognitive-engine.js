...
export async function runCognitiveEngine(input) {
  try {
    ...
    // Welcome Engine: new conversation → new/returning, project recall, unique warm greeting.
    ...
    if (welcome.plan?.active && welcome.plan.writerBrief) {
      plan.writerDirective = [plan.writerDirective, welcome.plan.writerBrief]
        .filter(Boolean)
        .join(' ')
      if (welcome.plan.mode === 'full_welcome') {
        plan.responseStructure = [
          `Apertura calda unica (seed): ${welcome.plan.greetingSeed}`,
          welcome.plan.project
            ? `Menziona naturalmente al massimo UN progetto: ${welcome.plan.project.label}`
            : 'Nessun progetto da elencare',
          welcome.plan.continuation
            ? `Una sola continuazione rilevante: ${welcome.plan.continuation}`
            : 'Invito breve e concreto, non generico',
          'Niente “Come posso aiutarti?” / aperture da helpdesk',
        ...
        plan.responseStructure = [
          ...
          welcome.plan.project
            ? `Al massimo un cenno opzionale a «${welcome.plan.project.label}» se non distrae`
            : 'Niente digressioni',
          `Obiettivo reale da servire: ${plan.realGoal}`,
        ]
      }
    ...
    if (voice.plan?.active && voice.plan.writerBrief) {
      plan.writerDirective = [plan.writerDirective, voice.plan.writerBrief]
        .filter(Boolean)
        .join(' ')
      if (voice.plan.interruptKind === 'hard' || voice.plan.interruptKind === 'soft') {
        ...
        plan.responseStructure = [
          ...(plan.responseStructure || []).slice(0, 3),
          'Modalità voce: frasi corte, pause, poca ripetizione',
        ]
      }
    }
    ...
    if (expertTeacher.plan) {
      ...
      if (expertTeacher.plan.enabled && expertTeacher.plan.structureHints?.length) {
        // Don't override short-ack continuation stop responses; refine teaching turns.
        const shortStop =
          continuation.plan?.isShortMessage && !continuation.plan?.shouldContinue
        if (!shortStop) {
          if (continuation.plan?.isShortMessage && continuation.plan?.shouldContinue) {
            const nextLayer = expertTeacher.plan.layersThisTurn[0]
            plan.responseStructure = [
            ...
            plan.writerDirective = [plan.writerDirective, expertTeacher.plan.writerBrief]
              .filter(Boolean)
              .join(' ')
          }
        }
    ...
    if (
      ...
      if (continuation.plan?.isShortMessage) {
        ...
        if (continuation.plan.shouldContinue) {
          const nextLayer = plan.expertTeacher?.enabled
            ? plan.expertTeacher.layersThisTurn?.[0]
            : null
          plan.responseStructure = [
            'Ack naturale in mezza frase (opzionale) — senza “Perfetto!” ripetitivo',
            nextLayer
              ? `Prossimo layer didattico (Expert Teacher): ${nextLayer.label} — ${nextLayer.writerHint}`
              : `Una sola continuazione significativa (${continuation.plan.continuationStyle || continuation.plan.additionKind || 'utile'}) sul filo corrente`,
        ...
        plan.writerDirective = [
          ...
          continuation.plan.writerBrief,
          plan.expertTeacher?.enabled && continuation.plan.shouldContinue
            ? plan.expertTeacher.writerBrief
            : '',
        ]
          .filter(Boolean)
      ...
      if (continuation.plan.shouldContinue) {
        plan.responseStructure = [
          ...
          `Stile di continuazione: ${continuation.plan.continuationStyle || 'utile'}`,
    ...
    if (
      curiosity.plan?.shouldExtend &&
      curiosity.plan.chosen &&
      ...
      plan.writerDirective = [plan.writerDirective, curiosity.plan.writerBrief]
        .filter(Boolean)
        .join(' ')
    } else if (
      ...
      // Soft reminder to avoid generic closers even when not extending
      plan.writerDirective = [plan.writerDirective, curiosity.plan.writerBrief]
        .filter(Boolean)
        .join(' ')
    }
    ...
    if (
      ...
      plan.writerDirective = [plan.writerDirective, momentum.plan.writerBrief]
        .filter(Boolean)
        .join(' ')
    }

    // Re-assert spoken constraints after other engines (voice should still feel spoken).
    if (voice.plan?.active && voice.plan.spokenStyleBrief) {
      plan.writerDirective = [plan.writerDirective, voice.plan.spokenStyleBrief]
        .filter(Boolean)
        .join(' ')
    }
    ...
    if (multiStep.plan?.active && multiStep.plan.writerBrief) {
      ...
      plan.writerDirective = [plan.writerDirective, multiStep.plan.writerBrief]
        .filter(Boolean)
        .join(' ')
      if (multiStep.pendingAction) {
        plan.pendingAction = multiStep.pendingAction
      }
      ...
      if (actionEngine.plan.phase === 'awaiting_confirmation') {
        ...
        actionEngine.plan.phase === 'executed' ||
        actionEngine.plan.phase === 'unavailable' ||
        actionEngine.plan.phase === 'denied' ||
        actionEngine.plan.phase === 'error' ||
        actionEngine.plan.phase === 'skipped'
      ) {
        plan.responseStructure = [
      ...
      plan.writerDirective = [plan.writerDirective, actionEngine.plan.writerBrief]
        .filter(Boolean)
        .join(' ')
      // Stash pending confirmation payload on the plan for the API/session layer
      if (actionEngine.plan.pendingActionPayload) {
        plan.pendingAction = actionEngine.plan.pendingActionPayload
      }
    }
