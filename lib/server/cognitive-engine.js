...
import {
  formatPrioritizationForWriter,
  prioritizationWriterBrief,
  prioritizeToolResults,
  needsExternalKnowledge,
} from './info-prioritization.js'
...
export async function runCognitiveEngine(input) {
  try {
    ...
    if (plan.toolOrder.length > 0) {
      try {
        toolResults = await executeTools(plan.toolOrder, {
          userMessage: input.userMessage,
          attachments: input.attachments,
          memoryEnabled: input.memoryEnabled,
        })
        const ordered = prioritizeToolResults(toolResults)
        toolContext = buildOrchestratorContext(ordered)
      } catch {
        toolContext = ''
        toolResults = []
      }
    ...
    const hasMemory = toolResults.some((r) => r.tool === 'memory' && r.status === 'ok')
    const hasExternal = toolResults.some(
      (r) =>
        (r.tool === 'web' || r.tool === 'weather') &&
        (r.status === 'ok' || r.status === 'empty'),
    )
    const prioritizationBlock = formatPrioritizationForWriter({
      hasConversation: Boolean(conversationContext),
      hasMemory,
      hasExternal: hasExternal || needsExternalKnowledge(input.userMessage),
    })

    // Context order mirrors information priority for the Writer
    const context = [
      prioritizationBlock,
      writerBlock,
      conversationContext,
