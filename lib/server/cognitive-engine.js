...
/**
 * Phases 3–4 — Tool need + order / skips (smart selection).
...
export function decideTools(input, understanding) {
  ...
  // Re-apply refinement with cognitive urgency / intent cues
  let toolOrder = refineToolSelection(planned, {
    userMessage: input.userMessage,
    attachments: input.attachments,
    memoryEnabled: input.memoryEnabled,
  })
  ...
  // Pure greetings / thanks → no tools (internal knowledge / social only)
  if (
