 ...
 * 0. Conversation Intent — why the user wrote (guides all later planning)
...
export function buildCognitivePlan(
  ...
  // Conversation Intent → Leadership → Deep Thinking → Presence guide structure.
  ...
  // Keep Intent + Leadership + Deep Thinking + Presence (up to 8 pre-plan lines).
  if (expertTeacher.plan?.enabled && expertTeacher.plan.structureHints?.length) {
    responseStructure = [
      ...prePlanStructure.slice(0, 8),
  ...
  // Guarantee Deep Thinking + Presence craft lines survive structure rewrites.
  const craftStructure = [...deepThinkingStructure, ...presenceStructure]
...
export async function runCognitiveEngine(input) {
  try {
    ...
    // Deep Thinking Engine (after Leadership, before Writer): explore directions, pick best.
