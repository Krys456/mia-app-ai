...
export function buildCognitivePlan(
  ...
  // Social → Intent → Leadership → Thoughtfulness → Deep Thinking → Presence → Wisdom → Taste guide structure.
...
export async function runCognitiveEngine(input) {
  try {
    ...
    // Presence Engine (after Deep Thinking, before Writer): organic aliveness / style / ending.
    ...
    // Wisdom Engine (after Presence, before Writer): useful / appropriate / meaningful.
    ...
    // Natural Dialogue already ran (post mode / pre-directives) and feeds askQuestion/length.
    ...
    // Reaction-only dialogue beats: force short / no-question in directives when present.
    if (
      naturalDialogue.plan?.active &&
      naturalDialogue.plan.reactionOnly &&
      plan.writerDirectives
    ) {
      try {
        // Soft overlay via writerBrief — frozen object stays authoritative language/mode.
        plan.writerDirective = [
          naturalDialogue.plan.writerBrief,
          `Natural Dialogue overlay: askQuestion=false · reactionOnly · length=short · move=${naturalDialogue.plan.move}.`,
