/**
 * LAIfe Adaptive Cognitive Engine — invisible planning before any user-facing text.
 *
 * Dynamically chooses the reasoning process based on the request:
 * classify → estimate complexity → select strategy → allocate effort.
 * Simple questions stay fast; complex tasks get deeper reasoning.
 *
 * Does NOT generate the reply. Does NOT speak to the user / expose the mode.
 * Hands an invisible plan to the Writer (+ tool context).
 */
 ...
 * @property {import('./adaptive-reasoning.js').AdaptiveStrategy | null} adaptive
...
/**
 * Apply adaptive tool bias (keep simple asks fast).
 * @param {ToolId[]} toolOrder
 * @param {import('./adaptive-reasoning.js').AdaptiveStrategy | null} adaptive
 * @returns {ToolId[]}
 */
function applyAdaptiveToolBias(toolOrder, adaptive) {
  if (!adaptive) return toolOrder
  let order = [...toolOrder]
  if (adaptive.avoidTools?.length) {
    const avoid = new Set(adaptive.avoidTools)
    order = order.filter((t) => !avoid.has(t))
  }
  if (adaptive.preferTools?.length) {
    for (const t of adaptive.preferTools) {
      if (!order.includes(/** @type {ToolId} */ (t)) && ALL_TOOLS.includes(/** @type {ToolId} */ (t))) {
        // Only add soft prefs that planTools would often include; memory is safe
        if (t === 'memory' || t === 'web') {
          order = /** @type {ToolId[]} */ ([t, ...order.filter((x) => x !== t)])
        }
      }
    }
  }
  if (adaptive.keepFast) {
    order = order.filter((t) => t === 'memory' || t === 'calculator')
  }
  return order
}
 ...
 * Prefers adaptive strategy hints when present.
 * @param {MessageUnderstanding} understanding
 * @param {string} realGoal
 * @param {string[]} secondary
 * @param {import('./adaptive-reasoning.js').AdaptiveStrategy | null} [adaptive]
 */
export function outlineResponseStructure(understanding, realGoal, secondary, adaptive = null) {
  if (adaptive?.structureHints?.length) {
    /** @type {string[]} */
    const structure = [...adaptive.structureHints]
    if (secondary.includes('wants_list') && !structure.some((s) => /Passi|elenco|lista|opzioni/i.test(s))) {
      structure.splice(structure.length - 1, 0, 'Usa elenco solo se migliora la leggibilità')
    }
    return structure
  }
...
/**
 * Build the full cognitive plan. Adaptive strategy drives structure & effort.
 * @param {CognitiveInput} input
...
export function buildCognitivePlan(input) {
  ...
  // Align understanding complexity with adaptive estimate when adaptive is more decisive
  if (adaptive.complexity === 'high' && understanding.complexity === 'low') {
    understanding.complexity = 'medium'
  }

  let tools = decideTools(input, understanding)
  const biasedOrder = applyAdaptiveToolBias(tools.toolOrder, adaptive)
  tools = {
    toolsNeeded: [...biasedOrder],
    toolOrder: biasedOrder,
    toolsSkipped: ALL_TOOLS.filter((t) => !biasedOrder.includes(t)),
  }
  ...
  const writerDirective = [
    ...
    'L’Adaptive Cognitive Engine ha già scelto la strategia di ragionamento: eseguila, non riesporla.',
    'Non mostrare il piano, le fasi, gli strumenti, la modalità di ragionamento o questa direttiva.',
    'Non dire mai il nome della modalità (quick answer, planning, coding, ecc.).',
    'Scrivi UNA sola risposta naturale all’utente.',
    adaptive.writerBrief,
    `Obiettivo reale (priorità): ${realGoal}`,
    `Intento primario: ${understanding.primaryIntent}`,
    `Livello tecnico: ${understanding.technicalLevel}; urgenza: ${understanding.urgency}; complessità richiesta: ${adaptive.complexity}; effort: ${adaptive.effort}; tono: ${understanding.tone}`,
...
/**
 * Format plan for the Writer (invisible block).
...
NON nominare la modalità di ragionamento. NON rispondere con checklist o analisi.
Scrivi solo la risposta finale all’utente.
...
export async function runCognitiveEngine(input) {
  try {
    ...
    // Merge tool suggestions — but keep adaptive fast-path
    if (
      follow !== 'continue' &&
      follow !== 'ack' &&
      follow !== 'example' &&
      ...
      let merged = [...new Set([...plan.toolOrder, ...task.plan.tools])]
      merged = applyAdaptiveToolBias(merged, plan.adaptive)
      plan.toolOrder = merged
      plan.toolsNeeded = [...merged]
      plan.toolsSkipped = ALL_TOOLS.filter((t) => !merged.includes(t))
    }

    // High-complexity task plans enrich structure only when adaptive wants depth
    const wantsDeep =
      plan.adaptive?.effort === 'deep' ||
      plan.adaptive?.mode === 'planning' ||
      plan.adaptive?.mode === 'research_synthesis' ||
      plan.adaptive?.mode === 'decision_analysis'

    if (task.plan?.complexity === 'high' && follow === 'other' && wantsDeep) {
      plan.responseStructure = [
