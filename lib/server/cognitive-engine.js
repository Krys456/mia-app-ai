...
export async function runCognitiveEngine(input) {
  try {
    ...
    // Knowledge Level Estimator: continuous per-topic estimate → calibrate depth/terms/examples/pacing.
    ...
    if (knowledge.plan?.level) {
      ...
      if (knowledge.plan.writerBrief) {
        plan.writerDirective = [plan.writerDirective, knowledge.plan.writerBrief]
          .filter(Boolean)
          .join(' ')
      }
    ...
    if (behavior.plan?.active && behavior.plan.writerBrief) {
      plan.writerDirective = [plan.writerDirective, behavior.plan.writerBrief]
        .filter(Boolean)
        .join(' ')
      ...
      if (behavior.plan.memoryHelpful === false && Array.isArray(plan.toolOrder)) {
        // Soft preference: skip memory when it would not improve this turn
