 ...
 * Conversation Intelligence (short-term) → plan → tools → Writer handoff.
 * Fail-soft: never throws to the caller for planning/tool failures.
 *
 * @param {CognitiveInput} input
 * @returns {Promise<{ plan: CognitivePlan, writerBlock: string, toolContext: string, conversationContext: string, context: string }>}
 */
export async function runCognitiveEngine(input) {
  try {
    ...
    const context = [conversationContext, writerBlock, toolContext]
      .filter(Boolean)
      .join('\n\n')
