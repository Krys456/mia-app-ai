...
import {
  runNaturalConversationEngine,
  naturalConversationStructureHints,
} from './natural-conversation-engine.js'
import {
  runCognitiveAuthorityEngine,
  cognitiveAuthorityStructureHints,
} from './cognitive-authority-engine.js'
import {
  runConversationDiversityEngine,
  conversationDiversityStructureHints,
} from './conversation-diversity-engine.js'
import {
...
import {
  runSocialContextEngine,
  socialContextStructureHints,
} from './social-context-engine.js'
import {
...
import {
  runConversationQualityEngine,
} from './conversation-quality-gate.js'
import {
...
export function detectEmotionalTone(text) {
  const t = String(text || '')
  if (
    /^(non\s+mi\s+interessa|non\s+mi\s+piace|lasciamo\s+stare|not\s+interested|i\s+don'?t\s+care)/i.test(
      t.trim(),
    )
  ) {
    // Rejection of a direction — not curiosity / not general distress
    return 'neutral'
  }
  if (
...
function detectPrimaryIntent(text) {
  ...
  if (
    /^(non\s+mi\s+interessa|non\s+mi\s+piace|lasciamo\s+stare|lascia\s+stare|not\s+interested|i\s+don'?t\s+care(\s+about\s+that)?|i'?m\s+not\s+interested)([\s!.]*)$/i.test(
      t,
    )
  ) {
    return 'rejection'
  ...
  if (
    /^(come\s+stai|come\s+va|how\s+are\s+you|how'?s\s+it\s+going|tutto\s+bene\??)([\s?!.]*)$/i.test(t)
  ) {
    return 'how_are_you'
  }
...
export function inferRealGoal(userMessage, understanding, opts = null) {
  ...
  if (
    ...
    understanding.primaryIntent === 'how_are_you'
  ) {
    return {
      surfaceAsk: surface,
      candidates: [],
  ...
  if (understanding.primaryIntent === 'rejection') {
    return {
      surfaceAsk: surface,
      candidates: [],
      primary: null,
      ...
      assumptions: [],
      realGoal:
        'Accettare il rifiuto della direzione proposta — non approfondirla né forzare una nuova curiosità.',
    }
  }
...
export function decideWebSearch(userMessage, understanding, plannedTools) {
  ...
  if (
    ...
    understanding.primaryIntent === 'thanks' ||
    understanding.primaryIntent === 'how_are_you' ||
    understanding.primaryIntent === 'rejection'
  ) {
    return { needed: false, reason: 'saluto/ringraziamento/presenza: nessuna ricerca' }
  }
...
export function buildCognitivePlan(
  ...
  const writerDirective = [
    ...
    // conversationQualityPlan may be passed via buildCognitivePlan later; soft string when available on input extras is optional
    languageAwarenessPlan?.active
    ...
    conversationIntentPlan?.active && conversationIntentPlan.inference?.whySummary
      ? `Conversation Intent Engine: primary=${conversationIntentPlan.inference.primaryIntent || 'unknown'}; ${conversationIntentPlan.inference.whySummary} — rispondi all’intenzione, non solo al letterale. Strategy=${conversationIntentPlan.inference.responseStrategy || 'direct'}.`
    ...
    conversationLeadershipPlan?.active && conversationLeadershipPlan.move
      ? `Conversation Leadership Engine: move=${conversationLeadershipPlan.move} · owner=${conversationLeadershipPlan.ownership?.owner || 'shared'} · phase=${conversationLeadershipPlan.buildPhase || 'hold'}${conversationLeadershipPlan.preserveMomentum ? ' · preserva momentum' : ''} · qBudget=${conversationLeadershipPlan.questionBudget ?? 0} — porta la conversazione, non outsourcarla; build-first se owner=assistant; niente invite precoce.`
 ...
 *   naturalConversation?: object | null,
 *   naturalConversationContext?: string,
 *   cognitiveAuthority?: object | null,
 *   cognitiveAuthorityContext?: string,
 *   conversationDiversity?: object | null,
 *   conversationDiversityContext?: string,
 ...
 *   socialContext?: object | null,
 *   socialContextContext?: string,
 ...
 *   conversationQualityContext?: string,
...
export async function runCognitiveEngine(input) {
  try {
    ...
    // Social Context Engine (AFTER Social, BEFORE Intent): HOW they are saying it.
    const socialContext = runSocialContextEngine({
      userMessage: input.userMessage,
      messages: input.messages,
      session: conversation.memory,
      socialConversation,
    ...
    // Conversation Quality Gate (MANDATORY high-priority — after Worth Reading, before Writer).
    const conversationQuality = runConversationQualityEngine({
      userMessage: input.userMessage,
      messages: input.messages,
      session: conversation.memory,
      conversationMemoryMap: input.conversationMemoryMap || null,
    ...
    // Natural Conversation (AFTER Personal Voice): share the world — do not impress.
    const naturalConversation = runNaturalConversationEngine({
      userMessage: input.userMessage,
      messages: input.messages,
      session: conversation.memory,
      languageAwareness,
    ...
    if (naturalConversation.plan?.active) {
      const ncHints = naturalConversationStructureHints(naturalConversation.plan)
      if (ncHints.length) {
        plan.responseStructure = [
          ...ncHints,
          ...(plan.responseStructure || []).filter(
            (line) => !ncHints.some((h) => h === line),
          ),
        ]
      }
    }

    // Cognitive Authority (AFTER Natural Conversation): hard gate awareness —
    // weak responses will be REJECTED post-Writer; greeting panel mandatory.
    const cognitiveAuthority = runCognitiveAuthorityEngine({
      userMessage: input.userMessage,
      messages: input.messages,
      session: conversation.memory,
      languageAwareness,
    ...
    if (cognitiveAuthority.plan?.active) {
      const caHints = cognitiveAuthorityStructureHints(cognitiveAuthority.plan)
      if (caHints.length) {
        plan.responseStructure = [
          ...caHints,
          ...(plan.responseStructure || []).filter(
            (line) => !caHints.some((h) => h === line),
          ),
        ]
      }
    }

    // Conversation Diversity (AFTER Cognitive Authority awareness): vary form/rhythm —
    // avoid structural repetition; change the experience, not only words.
    const conversationDiversity = runConversationDiversityEngine({
      userMessage: input.userMessage,
      messages: input.messages,
      session: conversation.memory,
      languageAwareness,
    ...
    if (conversationDiversity.plan?.active) {
      const cdHints = conversationDiversityStructureHints(conversationDiversity.plan)
      if (cdHints.length) {
        plan.responseStructure = [
          ...cdHints,
          ...(plan.responseStructure || []).filter(
            (line) => !cdHints.some((h) => h === line),
          ),
        ]
      }
    }
    ...
    const naturalConversationContext = naturalConversation.context || ''
    const cognitiveAuthorityContext = cognitiveAuthority.context || ''
    const conversationDiversityContext = conversationDiversity.context || ''
    ...
    const socialContextContext = socialContext.context || ''
    ...
    const conversationQualityContext = conversationQuality.context || ''
    ...
    const context = [
      ...
      worthReadingContext,
      conversationQualityContext,
      socialConversationContext,
      socialContextContext,
    ...
    return {
      ...
      naturalConversation: naturalConversation.plan,
      cognitiveAuthorityContext,
      cognitiveAuthority: cognitiveAuthority.plan,
      conversationDiversityContext,
      conversationDiversity: conversationDiversity.plan,
      humanImperfectionContext,
      ...
      socialContextContext,
      socialContext: socialContext.plan,
      conversationIntentContext,
      conversationIntentPlan: conversationIntent.plan,
      conversationIntent: conversationIntent.plan,
      conversationLeadershipContext,
      ...
      conversationQualityContext,
      conversationQuality: conversationQuality.plan,
      languageAwarenessContext,
    ...
    return {
      ...
      personalVoice: null,
      naturalConversationContext: '',
      naturalConversation: null,
      cognitiveAuthorityContext: '',
      cognitiveAuthority: null,
      conversationDiversityContext: '',
      conversationDiversity: null,
      humanImperfectionContext: '',
