  ...
  try {
    ...
    if (
      ...
      plan.writerDirective = [plan.writerDirective, curiosity.plan.writerBrief]
        .filter(Boolean)
        .join(' ')
    } else if (curiosity.plan?.writerBrief && !continuation.plan?.isShortMessage) {
    ...
    if (!continuation.plan?.isShortMessage && momentum.plan?.writerBrief) {
    ...
    // Universal Action Engine: real-world actions via modular plugins (never platform-hardcoded).
    const actionEngine = await runUniversalActionEngine({
      userMessage: input.userMessage,
      messages: input.messages,
    ...
    if (actionEngine.plan?.actionRequired && actionEngine.plan.writerBrief) {
