...
export function buildCognitivePlan(
  ...
  const writerDirective = [
    ...
    conversationOpeningPlan?.active && conversationOpeningPlan.shouldOpen
      ? `Conversation Opening Engine: style=${conversationOpeningPlan.styleLabel || conversationOpeningPlan.style} · cat=${conversationOpeningPlan.category} · «${conversationOpeningPlan.opener}» — diversifica le prime aperture; surprise-after-100-chats check; niente tropi vietati.`
...
export async function runCognitiveEngine(input) {
  try {
    ...
    // Conversation Opening Engine (AFTER Intent + Language, BEFORE Writer): diversify first 1–2 opens.
