...
export function buildCognitivePlan(
  ...
  // Conversation Intent → Leadership → Thoughtfulness guide structure before other advisors reshape it.
  ...
  // Keep Intent + Leadership + Thoughtfulness (up to 6 pre-plan lines).
  if (expertTeacher.plan?.enabled && expertTeacher.plan.structureHints?.length) {
    responseStructure = [
      ...prePlanStructure.slice(0, 6),
  ...
  // Guarantee Thoughtfulness craft lines survive structure rewrites.
  if (thoughtfulnessStructure.length) {
    const missing = thoughtfulnessStructure.filter(
  ...
  const writerDirective = [
    ...
    thoughtfulnessPlan?.active && thoughtfulnessPlan.contribution
      ...
      : 'Cerca il contributo più thoughtful, non solo la risposta corretta.',
...
export async function runCognitiveEngine(input) {
  try {
    ...
    // Thoughtfulness Engine (after Leadership, before Writer): best conversational contribution.
