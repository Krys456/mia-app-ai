    ...
    if (welcome.plan?.active && welcome.plan.writerBrief) {
      ...
      if (welcome.plan.mode === 'warm_handoff' || strategy === 'warm_handoff') {
        ...
        plan.responseStructure = [
          ...
          'A volte basta salutare — non forzare un progetto',
          'Niente “Come posso aiutarti?”',
          `Obiettivo reale da servire: ${plan.realGoal}`,
        ]
      } else {
        plan.responseStructure = [
          ...
          welcome.plan.nextStep
            ? `Un solo next step: ${welcome.plan.nextStep}`
            : 'Niente next step forzato',
          'Assistente personale di fiducia — varietà, niente script',
          `Obiettivo reale da servire: ${plan.realGoal}`,
        ]
      }
    }
