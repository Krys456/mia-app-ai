 ...
 * @returns {CognitivePlan}
 */
export function buildCognitivePlan(input, session = null, conversationIntentPlan = null) {
  ...
  // Conversation Intent guides structure before other advisors reshape it.
  const intentStructure = conversationIntentStructureHints(conversationIntentPlan)
  if (intentStructure.length) {
    responseStructure = [
      ...intentStructure,
      ...responseStructure.filter(
        (line) => !intentStructure.some((h) => h === line),
      ),
    ]
  }
...
export async function runCognitiveEngine(input) {
  try {
    ...
    const plan = buildCognitivePlan(input, session, conversationIntent.plan)
