...
export function buildConversationOwnershipPlan(input = {}) {
  ...
  const takeLead =
    ...
    (userMessage.length <= 18 && !/[?]/.test(userMessage) && !/\b(aiut|help|spieg|explain|perch|why|come)\b/i.test(userMessage))
...
export function analyzeConversationOwnershipDraft(input = {}) {
  ...
  if (takeLead && (onlyAck || ackThenQuestion || HELP_DESK.test(draft))) {
    issues.push({
      id: 'passive_ack',
      detail:
  ...
  if (takeLead && (GENERIC_QUESTION.test(draft) || (endsWithQ && sents.length <= 2 && !/[.!]/.test(draft.replace(/\?+$/, ''))))) {
    issues.push({
      id: 'generic_question',
  ...
  if (takeLead && draft.length < 70 && qCount === 0 && !/[—–…]/.test(draft)) {
    issues.push({
      id: 'thin_lead',
