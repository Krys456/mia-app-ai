...
export function buildV2DebugInfo(pipelineResult) {
  ...
  // Compact Conversation Signals diagnostics (no regex traces).
  if (
    pipelineResult.conversationSignals &&
    typeof pipelineResult.conversationSignals === 'object'
  ) {
    const sig = /** @type {any} */ (pipelineResult.conversationSignals)
    debug.conversationSignals = {
      boredom: Number(sig.affect?.boredom) >= 0.55,
      excitement: Number(sig.affect?.excitement) >= 0.45,
      frustration: Number(sig.affect?.frustration) >= 0.45,
      seriousness: Number(sig.affect?.seriousness) >= 0.45,
      playfulness: Number(sig.affect?.playfulness) >= 0.45,
      wantsBrief: Boolean(sig.style?.wantsBrief),
      wantsDetailed: Boolean(sig.style?.wantsDetailed),
      wantsSimple: Boolean(sig.style?.wantsSimple),
      wantsTechnical: Boolean(sig.style?.wantsTechnical),
      wantsProfessional: Boolean(sig.style?.wantsProfessional),
      allowsEmojis: sig.style?.allowsEmojis ?? null,
      topicChangeCue: Boolean(sig.interaction?.topicChangeCue),
      correctionCue: Boolean(sig.interaction?.correctionCue),
      continuationCue: Boolean(sig.interaction?.continuationCue),
      stopCue: Boolean(sig.interaction?.stopCue),
      activeFollowUp: Boolean(sig.engagement?.activeFollowUp),
      lowEffortReply: Boolean(sig.engagement?.lowEffortReply),
      language: sig.language?.code || null,
    }
  }
  // Compact reference / repair diagnostics (no candidate scoring traces).
  {
    const cs =
      (pipelineResult.nextConversationState &&
      typeof pipelineResult.nextConversationState === 'object'
        ? /** @type {any} */ (pipelineResult.nextConversationState)
        : null) ||
      (pipelineResult.conversationState && typeof pipelineResult.conversationState === 'object'
        ? /** @type {any} */ (pipelineResult.conversationState)
        : null)
    if (cs?.referenceResolution) {
      debug.referenceResolution = {
        status: cs.referenceResolution.status || 'unresolved',
        type: cs.referenceResolution.referentType || null,
        confidence: cs.referenceResolution.confidenceBand || null,
      }
    }
    if (cs?.repair) {
      debug.repair = {
        active: Boolean(cs.repair.active),
        type: cs.repair.active ? cs.repair.type || null : null,
      }
    }
  }
  ...
  if (
    ...
    const hard = Array.isArray(ce.hardViolations)
      ? ce.hardViolations.map((v) => (typeof v === 'string' ? v : v.code))
      : Array.isArray(ce.violations)
        ? ce.violations.filter((v) => v.severity === 'hard').map((v) => v.code)
        : []
    const soft = Array.isArray(ce.softViolations)
      ? ce.softViolations.map((v) => (typeof v === 'string' ? v : v.code))
      : Array.isArray(ce.violations)
