...
══════════════════════════════════════
L’utente ha DELEGATO la scelta del tema. È responsabilità tua — non ridarla indietro.
OBBLIGO:
1. Scegli ESATTAMENTE UNA direzione: «${t.title}»
2. COMMIT: non offrre alternative; non chiedere conferma della scelta
3. Sviluppa: perché breve (${t.why}) + insight (${t.insight}) + approfondisci (${t.develop})
4. Continua tu il filo — afferma, approfondisci, collega
VIETATO:
...
Regola d’oro: delegated choice = delegated responsibility. Never give control back.
...
function buildWriterBrief(plan) {
  ...
  const t = plan.chosen
  return [
    'TOPIC LEADERSHIP · NEVER GIVE CONTROL BACK: l’utente ha ceduto la scelta — è responsabilità tua.',
    `Una sola direzione: «${t.title}». Commit. Sviluppa. Non ridare il controllo.`,
    `Perché (1 frase): ${t.why}.`,
    `Insight: ${t.insight}`,
    `Sviluppa: ${t.develop}`,
    'VIETATO: liste di argomenti; domande aperte di scelta; “di cosa vuoi parlare?”; “preferisci…?”; far riscegliere.',
  ].join(' ')
}
