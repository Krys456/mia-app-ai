...
function resolveAskQuestion(input, mode, social) {
  ...
  const intent = input.conversationIntent?.inference || input.conversationIntent || null
  const naturalDialogue = input.naturalDialogue?.plan || input.naturalDialogue || null
  const conversationalPragmatics =
    input.conversationalPragmatics?.plan || input.conversationalPragmatics || null

  // Conversational pragmatics (playful / reaction-only) — hard no questions
  if (
    conversationalPragmatics?.active &&
    (conversationalPragmatics.reactionOnly || conversationalPragmatics.playful)
  ) {
    return { askQuestion: false, source: 'conversation_intent', reason: 'pragmatics_no_q' }
  }

  // Natural dialogue reaction-only / reaction-first — hard no questions
  if (naturalDialogue?.active && (naturalDialogue.reactionOnly || naturalDialogue.allowQuestion === false)) {
    return { askQuestion: false, source: 'conversation_intent', reason: 'natural_dialogue_no_q' }
  }
...
function resolveLength(input, mode, social, leadConversation) {
  const conversationalPragmatics =
    input.conversationalPragmatics?.plan || input.conversationalPragmatics || null
  if (conversationalPragmatics?.active && conversationalPragmatics.reactionOnly) {
    return {
      responseLength: /** @type {ResponseLength} */ ('short'),
      source: 'conversation_intent',
      reason: 'pragmatics_reaction_only',
    }
  }
  const naturalDialogue = input.naturalDialogue?.plan || input.naturalDialogue || null
  if (naturalDialogue?.active && naturalDialogue.reactionOnly) {
    return {
      responseLength: /** @type {ResponseLength} */ ('short'),
      source: 'conversation_intent',
      reason: 'natural_dialogue_reaction_only',
    }
  }
...
export function buildWriterDirectives(input = {}) {
  ...
  const conversationPlanner =
    input.conversationPlanner?.plan ||
    input.conversationPlannerPlan?.plan ||
    input.conversationPlanner ||
    input.conversationPlannerPlan ||
    null
  ...
  // askQuestion / lead / topic (raw resolves — Planner may override below)
  const askRaw = resolveAskQuestion(packed, mode, social)
  const leadRaw = resolveLeadConversation(packed, mode, social)
  const topicHoldRaw = resolveContinueTopic(packed)
  const initiativeRaw = resolveInitiative(leadRaw.leadConversation, mode, social)

  // Planner / rejection authority — outranks generic ownership/warm/presence lead defaults.
  const resolved = applyPlannerAuthority(packed, leadRaw, initiativeRaw, topicHoldRaw, askRaw)
  const ask = resolved.ask
  const lead = resolved.lead
  const topicHold = resolved.topicHold
  const initiative = resolved.initiative
  const authorityResolution = {
    ...resolved.authorityResolution,
    plannerAskQuestion: ask.askQuestion,
    finalAskQuestion: ask.askQuestion,
    finalInitiative: initiative.initiative,
  }
  ...
  if (conversationalPragmatics?.active && conversationalPragmatics.playful) {
    emotionalTone = 'playful'
    sources.emotionalTone = `pragmatics:${conversationalPragmatics.force}`
    reasons.push('tone_playful_pragmatics')
  } else if (authorityResolution?.rejection) {
    // Rejection is not curiosity — keep calm/neutral presence
    emotionalTone = emotionalTone === 'curious' ? 'neutral' : emotionalTone
    sources.emotionalTone = `tone:${emotionalTone}:rejection`
    reasons.push('tone_not_curious_on_rejection')
  } else {
  ...
  // 7. Writer style (length / initiative) — length uses post-authority lead
  ...
  return Object.freeze({
    ...
    authorityResolution: Object.freeze({ ...authorityResolution }),
  })
}
...
export function validateDraftAgainstDirectives(draft, directives) {
  ...
  // askQuestion — trailing OR unauthorized conversational invite
  if (!directives.askQuestion) {
    const trimmed = text.replace(/\s+/g, ' ').trim()
    if (/[?？]\s*$/.test(trimmed) || /\?\s*[😊🙂😉🔥✨]*\s*$/.test(trimmed)) {
      checks.askQuestion = false
      failures.push('ends_with_question')
    } else if (draftHasUnauthorizedConversationalQuestion(trimmed)) {
      checks.askQuestion = false
      failures.push('unauthorized_conversational_question')
    }
  }
  ...
  if (!checks.askQuestion) {
    briefParts.push(
      'askQuestion=false: remove conversational follow-up questions to the user (no “Vuoi…?”, “Cosa hai in mente…?”, “And you?”); end with a statement or warm beat. Do not invent a new curiosity hook.',
    )
  }
...
export function softEnforceDirectives(draft, directives) {
  ...
  // askQuestion=false: strip unauthorized conversational questions / trailing invites
  if (!directives.askQuestion) {
    text = stripUnauthorizedQuestions(text)
