...
export function inferTopicOwnership(turns, userMessage = '') {
  ...
  // Prefer user words; never promote assistant relational filler to topic.
  let topicHint = null
  let owner = /** @type {TopicOwner} */ ('shared')
  {
    const explicitUser =
      /^(?:parliamo\s+(?:di|dei|delle|del|della|degli)|let'?s\s+talk\s+about|vorrei\s+parlare\s+(?:di|dei|delle|del))\s+(.+)$/i.exec(
        userMessage,
      ) ||
      /^(?:parliamo\s+(?:di|dei|delle|del|della|degli)|let'?s\s+talk\s+about|vorrei\s+parlare\s+(?:di|dei|delle|del))\s+(.+)$/i.exec(
        lastU,
    ...
    if (explicitUser) {
      topicHint = String(explicitUser[1] || '')
        .replace(/[?.!]+$/, '')
        .trim()
        .slice(0, 120)
      owner = 'user'
      ...
      const clean = (s) =>
        String(s || '')
          ...
          .join(' ')
      const filler =
        /\b(stimolante|scambiare\s+idee|interesting\s+to\s+(chat|talk)|lovely\s+to\s+(chat|talk)|nice\s+to\s+(chat|talk)|piacere\s+(parlare|chiacchierare))\b/i
      const greetingOrSocial =
        /^(ciao|hey|hi|hello|come\s+stai|how\s+are\s+you|ok+|grazie|thanks)([\s?!,.]*)$/i
      const fromUser = clean(userMessage || lastU)
      if (fromUser && !greetingOrSocial.test(userMessage) && !greetingOrSocial.test(lastU)) {
        topicHint = fromUser
        owner = 'user'
      } else {
        const fromAssistant = clean(lastA)
        if (fromAssistant && !filler.test(lastA) && !filler.test(fromAssistant)) {
  ...
  // Recompute owner flags when not already explicit user topic
  if (owner !== 'user') {
