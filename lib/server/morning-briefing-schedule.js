 ...
 * @param {{ userId: string, requireExplicitUserId?: boolean }} scope
 */
export async function getMorningBriefingSchedule(scope) {
 ...
 * @param {{ userId: string, requireExplicitUserId?: boolean }} scope
 */
export async function upsertMorningBriefingSchedule(input, scope) {
  const userId = requireOwnerUserId(scope)
  const validated = validateMorningBriefingScheduleInput(input)
  if (validated.ok === false) return validated

  await ensureAuthUserRow(getServiceSupabase(), userId)
  const supabase = getServiceSupabase()
  const row = {
    user_id: userId,
 ...
 * @param {{ userId: string, requireExplicitUserId?: boolean }} scope
 */
export async function disableMorningBriefingSchedule(scope) {
  const userId = requireOwnerUserId(scope)
  const supabase = getServiceSupabase()
  const existing = await getMorningBriefingSchedule(scope)
  if (!existing.exists) {
