...
export const DOCUMENT_ONLY_MODEL_NUDGE =
  'Analyze this document and briefly explain what it is about.'
...
const DOCUMENT_ONLY_NUDGE_BY_LANG = {
  it: 'Analizza questo documento e dimmi in breve di cosa tratta.',
  en: DOCUMENT_ONLY_MODEL_NUDGE,
  es: 'Analiza este documento y explícalo brevemente.',
  fr: 'Analyse ce document et explique brièvement de quoi il parle.',
  de: 'Analysiere dieses Dokument und erkläre kurz, worum es geht.',
}
...
export function sanitizeFileAttachment(raw) {
  ...
  const expiresAt =
    typeof att.expiresAt === 'number' && Number.isFinite(att.expiresAt)
      ? att.expiresAt
      : typeof att.expires_at === 'number' && Number.isFinite(att.expires_at)
        ? att.expires_at
        : null
 ...
 * #313 — optional reuseDocument injects active file_id onto the last text-only user turn.
 * @param {SanitizedChatMessage[]} messages
 * @param {{
 *   browserLocale?: string | null
 *   reuseDocument?: { fileId: string, mimeType?: string } | null
 * }} [opts]
 */
export function mapMessagesToResponsesInput(messages, opts = {}) {
  ...
  const reuse =
    opts.reuseDocument &&
    typeof opts.reuseDocument.fileId === 'string' &&
    isSafeOpenAiFileId(opts.reuseDocument.fileId)
      ? opts.reuseDocument
      : null

  // Find last user index for #313 document reuse injection
  let lastUserIdx = -1
  ...
  return limited.map((msg, idx) => {
    ...
    if (msg.role !== 'user' || !msg.attachments?.length) {
      ...
      if (msg.role === 'user') {
        ...
        const shouldReuseDoc =
          Boolean(reuse) && isLastUser && !isFileAttachmentMsg(msg) && !isImageAttachmentMsg(msg)

        if (prior.length || shouldReuseDoc) {
          /** @type {Array<Record<string, unknown>>} */
          const parts = []
          const caption = msg.content.trim()
          parts.push({
            type: 'input_text',
            text: caption || (shouldReuseDoc ? documentOnlyNudge : imageOnlyNudge),
          })
          ...
          if (shouldReuseDoc && reuse) {
            const mime = String(reuse.mimeType || '')
            if (mime === SERVER_PDF_MIME) {
              parts.push({
                type: 'input_file',
                file_id: reuse.fileId,
                detail: 'low',
              })
            } else {
              parts.push({
