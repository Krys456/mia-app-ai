...
/**
 * @param {unknown} raw
 * @param {{ requireGeneratedSource?: boolean }} [opts]
 * @returns {{
 *   type: 'image'
 *   mimeType: string
 *   dataUrl: string
...
export function sanitizeImageAttachment(raw, opts = {}) {
  ...
  // Assistant history replay: only trust server-marked generated/edited images.
  // Clients cannot invent arbitrary assistant images without this marker.
  if (opts.requireGeneratedSource) {
  ...
  if (typeof att.id === 'string' && att.id.trim()) out.id = att.id.trim().slice(0, 120)
  return out
}

/**
 * Sanitize image attachments on an assistant message (session replay of
 * server-generated results only — never arbitrary client spoof without source).
 ...
 * }}
 */
export function sanitizeAssistantImageAttachments(rawList) {
  if (rawList == null) {
  ...
  for (const rawAtt of rawList) {
    ...
    if (!isImage || isFile) {
      // Assistant messages may only carry trusted generated/edited images — never PDFs.
      return {
        ok: false,
    ...
    const cleaned = sanitizeImageAttachment(rawAtt, { requireGeneratedSource: true })
    if (!cleaned) {
      return {
        ok: false,
