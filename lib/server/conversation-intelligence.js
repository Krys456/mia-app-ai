...
function buildContinuityDirective(opts) {
  const lines = [
    ...
    'Sei lo stesso assistente dall’inizio di questa chat: stessa voce, stesso contesto, stessi impegni.',
  ...
  if (opts.openQuestions?.length) {
    lines.push(
      `Domande ancora aperte (rispondi solo se ancora rilevanti): ${opts.openQuestions.join(' · ')}`,
    )
  }
