...
describe('#303C Edge worker contracts', () => {
  ...
  it('#380G due reminder push uses Urgency high; TTL 3600; payload/retry unchanged', () => {
    // Send options: high urgency (Android/FCM), TTL one hour.
    ...
    // Payload shape unchanged (reminderId / title / url / tag).
    assert.match(edge, /reminderId/)
    assert.match(edge, /url:\s*`\/\?reminder=/)
    assert.match(edge, /tag:\s*reminderId/)
    // Acceptance / claim / retry semantics unchanged.
    assert.match(edge, /code:\s*['"]push_accepted['"]/)
    assert.match(edge, /push_sent_at:\s*new Date\(\)\.toISOString\(\)/)
    assert.match(edge, /p_outcome:\s*['"]retry['"]/)
    assert.match(edge, /push_subscription_gone|disable:\s*true/)
    // #384E — Morning briefing also uses high urgency in its own Edge file (not this one).
    const morning = read('supabase/functions/morning-briefing-dispatch/index.ts')
    assert.match(morning, /urgency:\s*['"]high['"]/)
    assert.doesNotMatch(morning, /urgency:\s*['"]normal['"]/)
  })
})
