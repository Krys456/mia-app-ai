...
/**
 * Strip invisible format chars that break `\b` / `\s` matching (ZWSP, ZWNJ, etc.).
 * Mobile/IME/paste often inserts these between Italian tokens like "cosa" + "ho".
 * @param {unknown} text
 * @returns {string}
 */
export function normalizeCalendarIntentText(text) {
  return String(text || '')
    .normalize('NFKC')
    // Treat zero-width / soft-hyphen as separators so "cosa​ho" → "cosa ho"
    .replace(/[\u200B-\u200D\uFEFF\u2060\u00AD]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
}
...
export function detectCalendarChatIntent(text) {
  const raw = normalizeCalendarIntentText(text)
  if (!raw) return 'none'
