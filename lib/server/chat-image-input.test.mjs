...
import { signImageArtifact } from './image-artifact-proof.js'
import {
...
// --- PDF with embedded bytes rejected (fileId-only wire format) ---
{
  const r = sanitizeMultimodalMessages([
    {
      role: 'user',
  ...
  assert.equal(r.ok, false)
  assert.equal(r.code, 'invalid_file')
}
  ...
  const r = sanitizeMultimodalMessages([
    {
      role: 'user',
      content: 'x',
      attachments: [{ type: 'audio', mimeType: 'audio/mpeg', dataUrl: 'data:audio/mpeg;base64,aaa' }],
    },
  ])
  assert.equal(r.ok, false)
...
// --- assistant cannot spoof with source alone (needs artifactProof) ---
{
  const r = sanitizeMultimodalMessages([
    { role: 'assistant', content: 'hi', attachments: [imgAtt({ source: 'generated', id: 'ig_x' })] },
  ])
  assert.equal(r.ok, false)
  assert.equal(r.code, 'assistant_image_forbidden')
}

// --- assistant sealed generated image allowed for session replay ---
{
  const secret = 'unit-test-assistant-seal-secret'
  process.env.LAIFE_IMAGE_ARTIFACT_SECRET = secret
  const id = 'ig_ok'
  const dataUrl = tinyJpeg
  const proof = signImageArtifact({ id, source: 'generated', dataUrl }, secret)
  const r = sanitizeMultimodalMessages([
    {
      role: 'assistant',
      content: 'Ecco',
      attachments: [imgAtt({ source: 'generated', id, artifactProof: proof })],
    },
  ])
  assert.equal(r.ok, true)
  assert.equal(r.messages[0].attachments?.[0].source, 'generated')
  assert.equal(r.messages[0].attachments?.[0].artifactProof, proof)
}
...
// --- image-only: model nudge is SERVER-ONLY (not visibleUserText) ---
{
  ...
  // No sticky thread → default English nudge (IMAGE_ONLY_MODEL_NUDGE).
  assert.equal(mapped[0].content[0].text, IMAGE_ONLY_MODEL_NUDGE)
  assert.equal(mapped[0].content[1].type, 'input_image')
}

// --- #274 follow-up: image-only nudge follows Italian sticky (not synthetic EN) ---
{
  const itHist = [
