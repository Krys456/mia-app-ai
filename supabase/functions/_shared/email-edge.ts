/**
 * #337B — Deno shared helpers for Email Edge Functions.
 * Mirrors supabase/functions/_shared/calendar-edge.ts — kept as a SEPARATE
 * module (different table, different env vars, different CORS allowlist).
 */

import { createClient, type SupabaseClient } from 'npm:@supabase/supabase-js@2'
import { redactTokenFields } from './email-token-crypto.ts'
...
export function isEmailEnabled(): boolean {
  return isTruthy(env('EMAIL_ENABLED'))
}
...
export function resolveCorsOrigin(req: Request): string | null {
  ...
  for (const entry of env('EMAIL_RETURN_URL').split(',')) {
    const base = entry.trim().replace(/\/+$/, '')
    if (!base) continue
    try {
      allow.add(new URL(base).origin)
    } catch {
      /* ignore */
    }
  }
...
/** Never pass subject/from/snippet/body through this — redactTokenFields also strips those keys. */
...
export function toPublicConnection(row: Record<string, unknown> | null): EmailConnectionPublic {
  if (!row) {
    return {
      status: 'disconnected',
      provider: 'google',
