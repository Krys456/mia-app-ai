/**
 * #311 — AES-256-GCM token encryption (server / Edge only).
 *
 * Ciphertext format (versioned):
 *   v1.<iv_b64url>.<ciphertext_b64url>.<tag_b64url>
 *
 * Key: EMAIL_TOKEN_ENCRYPTION_KEY — 32 raw bytes as base64 or 64-char hex.
...
/**
 * Parse EMAIL_TOKEN_ENCRYPTION_KEY into 32 raw bytes.
...
export async function encryptToken(plaintext, keyEnv = undefined) {
  ...
  const parsed = parseEncryptionKey(
    keyEnv !== undefined ? keyEnv : typeof process !== 'undefined' ? process.env.EMAIL_TOKEN_ENCRYPTION_KEY : '',
  )
