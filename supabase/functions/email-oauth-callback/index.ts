/**
 * #311 — email-oauth-callback
 *
 * Google redirects here (no user JWT). Validates signed state + pending nonce,
 * exchanges code server-side, encrypts tokens, upserts email_connections,
 * redirects to HMAC-bound return origin (allowlisted; no open redirects).
 *
 * verify_jwt = false — ownership from HMAC state + DB nonce.
 */
...
function failRedirect(
  returnBase: string,
  code: string,
  returnOrigin: string | null = null,
) {
  const safe = resolveOAuthCallbackReturnUrl({
    returnOrigin,
    allowedBases: returnBase,
    flag: 'error',
  })
  if (!safe.ok) {
...
Deno.serve(async (req) => {
  ...
  const boundOrigin =
    typeof verified.returnOrigin === 'string' ? verified.returnOrigin : null
