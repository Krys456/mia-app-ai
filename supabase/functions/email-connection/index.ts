...
import { revokeGoogleToken } from '../_shared/email-gmail.ts'
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const { data, error } = await supabase
        ...
        .select('status, provider, account_email, scopes, last_error_code, disconnected_at, updated_at')
        .eq('user_id', userId)
        .eq('provider', 'google')
        .maybeSingle()
