/**
 * #311 — email-oauth-start
 *
 * Verifies ShinkAIdo JWT, binds PKCE + signed state (+ return origin) to auth.uid(),
 * stores pending nonce on email_connections, returns Google authorize URL.
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const signed = await createSignedOAuthState(
      { userId, nonce, codeVerifier, returnOrigin },
      encKey,
    )
    if (!signed.ok) {
