-- #311 — Google Gmail OAuth connection foundation (repo-only; do NOT auto-apply)
--
-- ARCHITECTURE:
...
--
-- Phase 1 is READ-ONLY (gmail.readonly). No send/reply/delete/archive/labels.
-- Do NOT store email bodies permanently in this table.
...
COMMENT ON TABLE public.email_connections IS
  '#311: Google Gmail OAuth connection metadata + encrypted tokens. Owner = auth.uid(). Service-role / Edge only. READ-ONLY.';

COMMENT ON COLUMN public.email_connections.access_token_enc IS
  '#311: AES-256-GCM ciphertext (versioned). Never plaintext. Never client-readable.';
...
-- See supabase/migrations/README-311-EMAIL.md
