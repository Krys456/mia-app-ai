/**
 * BrAIn memory runtime helpers for Vercel serverless.
 *
 * Kept in one lowercase module so NFT/static tracing is reliable.
 * Uses lazy dynamic import of @supabase/supabase-js (node_modules) so
 * local helper evaluation cannot crash function cold-start.
 */
...
export async function searchMemories(
  ...
  const rows: MemoryRecord[] = (data ?? []).map((row: Record<string, unknown>) => ({
    id: String(row.id),
    category: String(row.category ?? ''),
    title: String(row.title ?? ''),
    content: String(row.content ?? ''),
